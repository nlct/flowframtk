...
\FloatFig
  ...
  {\includeimg[height=0.4\textheight]{configureDialog-controls}}
...
\FloatFig
  ...
  {\includeimg[height=0.4\textheight]{configureDialog-startdir}}
...
\FloatFig
  ...
  {\includeimg[height=0.4\textheight]{configureDialog-jdrsettings}}
...
\FloatFig
  ...
  {\includeimg[height=0.4\textheight]{configureDialog-initsettings}}
...
configuration directory (\sectionref{sec:configdir}).  Mappings,
...
\FloatFig
  ...
  {\includeimg[height=0.4\textheight]{configureDialog-bitmap}}
...
\FloatFig
  ...
  {\includeimg[height=0.4\textheight]{configureDialog-apppaths}}
...
(\sectionref{sec:configure} covers image settings
and \sectionref{sec:configureui} covers user interface settings.)
...
frames (\section{sec:flowframe}) that use a non-standard paragraph
shape also require this information.

\FloatFig
 {fig:normalsize}
 {\includeimg[width=0.9\linewidth]{normalsizeandclass}}
...
Note that the largest normal size listed
(25pt\dash this value is actually 24.88pt, but
\appname\ lists it as 25pt) is for use
with the \cls{a0poster} class file. Remember that for very large
or very small fonts, you will need to use scalable fonts in your
document to prevent font size substitutions.
Available values, along with the corresponding value of
\gls{baselineskip} and the file in which they are defined,
are listed in \tableref{tab:normalsize}.
...
\FloatFig
 {fig:flfsettings}
 {\includeimg[width=0.9\linewidth]{flfsettings}}
...
\FloatFig
  ...
  {\includeimg[width=0.9\linewidth]{configureDialog-texsettings}}
...
To add a new mapping, \gls{click} on the green plus button to
the side of the relevant table.
TODO
...
You can also use this tab to choose between enabling (left icon) and
disabling (right icon) hotspots along the \glspl{bbox}. 
