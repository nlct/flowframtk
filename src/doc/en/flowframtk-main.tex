...
If you \filefn{export} your image to PDF, EPS or \gls{svg}
...
\section{The Preamble and Document Text Pane}\label{sec:thepreamblepane}

The preamble code and document text pane is, by default, on the right of the \gls{canvas},
which may be used to specify code that should go in the document
preamble or (where applicable) the \env{document} environment of exported files. 
(This includes exports that use \LaTeX\ as an intermediate step.)

This area is split into three tabs for the early preamble code,
the mid-preamble code, and the late preamble code, and a fourth tab
for the content of the \env{document} environment (only applicable
with \gls{flowframe} exports that include the \env{document}
environment).
...
for all images and to reposition the preamble code and document text pane.
...
This will open the \dialog{export} dialog box, which
provides a way to specify the name and location of the file as well
as what format to export to. 

\widgetdef{export.title}

At the top of the \dialog{export} dialog is a field for the path to
the desired export file. You can either type in the full path or use
the \btn{selectfile} button at the side to open the file chooser.
...
The file format for the exported file
...
Select the \widget{export.use_alpha} box if you want a \gls{transparent}
background. If the box isn't checked, the background will be white.

Select the \widget{export.bounds.image} box if you want the image cropped
...
package. In this case, you can specify the \LaTeX\ code to add the
\env{document} environment.

If the document content is empty, blank pages will be created. The
number depends on the page lists of all objects that have
\gls{flowframe} data. If all page lists are simply set to \qt{all},
then only one page will be created. If some page lists are set to
\qt{even} then two pages will be created, otherwise the number of
pages will be determined by the maximum end range of all page lists.
If a page list has an open-ended range then the end will be treated
as the minimum valid number for that range.
...
exporting \gls{flowframe} data to a multi-paged PDF.
...
\meta{late-preamble} \meta{document body} \meta{class name}
...
the \gls{document-body} is also stored.
\begin{enumerate}
...
\meta{document body}, for version 2.1 onwards.
