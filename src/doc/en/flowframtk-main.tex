...
You can assign a space-separated list of tags to \selected\
\glspl*{object} using the \menu{edit.object_tag} menu item.
This will display the \dialog{tag} dialog box. Type the space
separated tag list into the text field and click on \btn{okay}
or press \keys{\keyref{return}}. The list will automatically be
trimmed.

\begin{important}
Tag lists are restricted to alphanumerics, spaces, and the following
...
the tags will be combined into a single list.
\end{information}

The tag list will not appear in the image but may be included in
object markup when \hyperref[sec:exportimage]{exporting images},
depending on the export options.

\begin{important}
If you assign a tag list to a \gls{group},
