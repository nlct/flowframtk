...
\widget{flowframe.margin.left} and
\widget{flowframe.margin.top} fields. Additionally, you
...
menu item to open the \dialog{fillcolour} dialog box.  The windows
...
the \dialog{fillcolour} dialog box. I also added two
...
show the \gls{preamble} panel. In the \widget{preamble.early}
...
\widget{flowframe.margin.left} and
\widget{flowframe.margin.top} margins to the nearest
...
This should alter the \widget{flowframe.margin.right} field.
...
\widget{flowframe.margin.left} and
\widget{flowframe.margin.top} margins to 40\gls{pt} and the
\widget{flowframe.margin.right} and
\widget{flowframe.margin.bottom} margins to 10\gls{pt}.
...
\widget{flowframe.margin.left} margin to 10\gls{pt} and the
\widget{flowframe.margin.right} margin to 40\gls*{pt}.
...
\widget{flowframe.margin.left} to 50\gls{pt} and set the remaining
...
do \mnemonic{controls} to select it. Then do
\mnemonic{graphics.storage_unit} to move the focus to the
...
the \dialog{fillcolour} dialog box shown in \figureref{fig:housenomouse2}.
...
\item To select the \widget{colour.single} radio button,
either do \mnemonic{colour.single} or press \keys{\keyref{tab}} until
the \widget{colour.single} button has the focus, and then
...
\item Press \mnemonic{colour.rgb} to select the
\widget{colour.rgb} panel and set the
\widget{colour.red} field to 100, the
\widget{colour.green} field to 100, the
\widget{colour.blue} field to 0 and the
\widget{colour.alpha} field to 100.

\item Press \mnemonic{colour.cmyk} to select the
\widget{colour.cmyk} panel and set the
\widget{colour.cyan} field to 0, the
\widget{colour.magenta} field to 0, the
\widget{colour.yellow} field to 100, the
\widget{colour.black} field to 0 and the
\widget{colour.alpha} field to 100.
\end{itemize}
...
\item Open the \dialog{fillcolour} dialog box using the menu mnemonic
\mnemonictrail{edit.fill_colour}.

\item Use \mnemonic{colour.single} to select the
\widget{colour.single} radio button.
...
\widget{colour.rgb} tab and set the
\widget{colour.red} field to 100, the
\widget{colour.green} and \widget{colour.blue}
fields to 0 and the \widget{colour.alpha} field to 100.
...
{Additional Paper Size Identifiers (JDR v1.3 onwards)}
  ...
  however the \meta{major} unit is an \type{integer} not a
...
\meta{end-preamble} \meta{class name}
...
and \meta{end-preamble} for version 1.9 onwards.
...
\gls{JDR} file, the outermost implicit group will contain
...
flowframe specifications \meta{flowframe-specs} should follow
...
Recall from above that the image is stored with an implied outer grouping. The
\meta{flowframe-specs} for this implicit group represents the image
typeblock and the \meta{description} represents the image description.
...
{Additional Marker IDs (Version 1.6 Onwards)}
  ...
  \meta{id} \meta{specs}
  \end{syntaxline}
  where \meta{id} is a \type{char} representing the segment type. This
  can be one of: \code{B} (cubic \gls{Bezier-curve}), \code{L} (line) or
  \code{M} (move). For versions below 1.8, the co-ordinates are
