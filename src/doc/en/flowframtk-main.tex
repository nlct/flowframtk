...
Set the \manmsg{colour} for the line of symmetry
\glspl*{controlpt} according to whether they are selected or
unselected.

\widgetdef{render.control.patternanchor}

Set the \manmsg{colour} for the pattern anchor
\glspl*{controlpt} according to whether they are selected or
unselected.

\widgetdef{render.control.patternadjust}

Set the \manmsg{colour} for the pattern adjustment
\glspl*{controlpt} according to whether they are selected or
unselected.
...
have been selected, then the \menu{selected.fill_colour} menu item
...
\inlineglsdef{goto.title} dialog in which you can enter $x$ and $y$
...
\gls{Bezier-curve}) to another, first \select\ the \gls*{path}, and
then either click on the \gls{menu.edit.path.edit} icon or select
\menu{edit.path.edit}.  Note that the \gls{menu.edit.path.edit} menu item
is only available if exactly one \gls{path} and no other
...
\widget{graphics.title} panel will ignore the \gridlock\ for the
...
\glslink{symmetricshape}{symmetry}.
...
the adjacent curvature control points, the corresponding curvature
control on the other segment will be adjusted to maintain
continuity. An anchor image will appear in the control joining the
...
used to change the starting gradient of the \gls{Bezier-curve} to make
...
again used to change the end gradient of the \gls{Bezier-curve} to
...
Symmetry can be applied to an existing \gls{shape} in 
...
to add or remove symmetry. Adding symmetry will replace the existing
\gls{shape} with a \gls{symmetricshape}. This has extra
\glspl{controlpt} that govern the line of symmetry.
Removing symmetry will replace the existing \gls{symmetricshape}
...
path with symmetry applied to it. There are now two extra
\glspl{controlpt} (\manmsg{coloured} blue). These points govern the line
of symmetry. In \figureref{fig:symmetricc}, these two controls
have been moved.

\begin{information}
The line of symmetry extends infinitely though the two controls,
...
determine whether or not the reflection is joined to the underlying
path at its end point. If this option is on, the end \gls{controlpt}
will be constrained to the line of symmetry.  That is, the end
...
end control was then moved away from the line of symmetry.
...
the join has been changed to a curve. Unlike the
\glspl{Bezier-curves}
in the non-symmetric paths, this curve only has one curvature
control.
...
to determine whether or not the reflection is joined to the
underlying path at is start point.  If this option is on, the start
\gls{controlpt} will be constrained to the line of symmetry.  This
function is only available if
\gls{menu.editpath.symmetry.has_symmetry} is on and the
underlying \gls{path} is closed.

For example, \figureref{fig:closedsymmetrica}
shows a closed symmetric path (a closed version of
\figureref{fig:symmetrice}). The anchor constraint on the first control
was then removed and the control was moved to the left
(\figureref{fig:closedsymmetricb}). As with the join segment
(above) the closing segment between the start control and its
reflection can be changed to a curve with
one curvature control (\figureref{fig:closedsymmetricc}).
