...
\DeclareTOCStyleEntry[numwidth=2.5em]{tocline}{section}
\DeclareTOCStyleEntry[numwidth=3.5em]{tocline}{figure}

% Filenames are obtained from the label following to TOC entry
% If no label provided, a generic filename is used.
% This is automatically done for the table of contents and
% glossaries/index.

% Adding a label for the other unnumbered chapters:

\AfterTOCHead[lof]{\label{listoffigures}}
\setbibpreamble{\label{references}}
...
\newcommand{\rendered}{\glslink{rendering}{rendered}}
...
\begin{prefrontmatterblock}{notes}{Notes}
...
\gls{gui}.  The main purpose of \FlowframTk\ is 
to generate \sty{pgf} picture drawing code, and 
to generate \LaTeX\ packages or classes that use the \sty{flowfram} package. 
As a side-effect, it can also be
...
(binary) or \gls{AJR} (plain text) file formats.

Images can be imported from \gls{svg} and the
\gls{Acorn-DrawFile-format}, although not all features for either
format are supported.  \FlowframTk's experimental mode may provided
additional import options, but they may not be fully functional.

Additionally, images can be exported (\sectionref{sec:exportimage}) as:
...
\begin{itemize}
    ...
    \item a PDF file (requires \TeX\ installation);

    \item (only if \widget{processes.support_svg} is on)
     \pgls{svg} file.

    \item (only if \widget{processes.support_eps} is on)
     \pgls{eps} file.
\end{itemize}

Note that some export functions, such as export to PDF use external
processes, such as \app{pdflatex}.

\begin{important}
All export formats lose information so even if \FlowframTk\ supports
both export and import for a particular file format, you will not
get back the same image. It's therefore important to also save your
image using \FlowframTk's native \gls{JDR} or \gls{AJR} file
formats.
\end{important}
...
to write manually. (Also, \gls{JpgfDraw} was written before the
introduction of the \sty{tikz} package.)

\FlowframTk\ was tested with version 3.0.0 of the \sty{pgf}
package, version 2.0 of the \sty{flowfram} package and version
2.2 of the \sty{shapepar} package. Files created by \FlowframTk\
may not work with earlier versions of those packages.
...
be assumed to mean a~\gls{primaryclick}. Keystrokes are represented
in the form \keys{\keyref{return}} (the Return\slash Enter key).
...
\menu{navigate.goto} or \menu{navigate.find}). This is done via the \emph{\gls{robot}}.
...
\tabularnewline
&
In some tabbed panes: &
\tabularnewline
&
\leftquadpar{Switch focus to tab} 
\tabularnewline
%
\keys{\actualkey{F7}} &
Select mode: &
...
\code{in}, \code{mm}, \code{cm}, \code{\gls+{pc}},
\code{\gls+{dd}} and \code{\gls+{cc}}. If the unit is omitted,
...
\begin{information}
The \widget{grid.offset} setting is only supported in
\gls{JDR}\slash\gls{AJR} file format 2.1 onwards.
\end{information}
...
\subfigref{fig:grid-radial-origin} radial grid (origin-based).}
...
(\figureref{fig:grid-rect}). This grid has four parameters:
\widget{grid.major.x} and \widget{grid.major.y} (which identifies the grid unit) and
\widget{grid.sub_divisions.x} and \widget{grid.sub_divisions.y} which 
indicate the number of minor tick marks between each major tick.

\begin{important}
A value of zero for either of the major divisions will be replaced
with one. The subdivision widget won't go lower than one.
In general, it's best to avoid very small major divisions and very
large sub-divisions as it can slow down \gls{rendering} and make it
hard to see the objects.
\end{important}

The \includeimg{linked} \slash\ \includeimg{unlinked} button toggles between
fixing the $y$ parameter to the $x$ or allowing a different $y$
value to be specified. The separate $y$ settings are only supported
in \gls{JDR}\slash\gls{AJR} file format 2.2 onwards.

\widgetdef{grid.iso}

The isometric grid is likewise a left-handed co-ordinate system but in this case
the major divisions \widget{grid.major} indicate the length of each
side of the equilateral triangles forming the grid
(\figureref{fig:grid-iso}). The \widget{grid.sub_divisions}
indicates the number of minor tick marks between each major tick. 
There is no separate $x$ and $y$ value for the parameters.
...
\subfigref{fig:grid-radial-origin-shift} radial grid (origin-based).}
...
\begin{information}
The \widget{grid.radial.centre.origin} setting is only supported in
\gls{JDR}\slash\gls{AJR} file format 2.1 onwards.
\end{information}
...
The shift also affects the page margins, which mark the printer
margins around the border of the page.

For example, \figureref{fig:grid-radial-page-margins-portrait} shows an image
in \FlowframTk\ that has a page-\manmsg{centred} radial grid with
the \glspl{printer-margin} showing (\menu{settings.paper.margins}).
The paper size is set to A5 portrait.
The \gls{origin} for the internal storage co-ordinates is at the top
left corner of the \gls{canvas} (the origin shift is the
default zero), so the paper margins are in their
usual place around the border of the \gls{canvas} but the radial
grid is drawn with its \manmsg{centre} coinciding with the
\manmsg{centre} of the page.

The image was then printed to a file using \menu{file.print} with
the output destination set to a file instead of to a printer.
The result is shown in
\figureref{fig:grid-radial-page-margins-printout}.
(Scaled and border added to show bounds.)
The page orientation was then changed to landscape.
(The \menu{file.page} dialog needs to be used to update the 
\glspl{printer-margin}.) % TODO
The radial grid is still \manmsg{centred} on the page but the
\glspl{object} on the \gls{canvas} haven't moved because the
locations of their \glspl{controlpt} are always in terms of the
left-handed co-ordinate system based on the top left corner of the
page (see \figureref{fig:grid-radial-page-margins-landscape}).

Compare this with \figureref{fig:grid-radial-origin-margins}, which
is similar to \figureref{fig:grid-radial-page-margins} but uses the
radial grid \manmsg{centred} on the \gls{origin}, which has been
shifted to the \manmsg{centre} of the \gls{canvas}. The
\glspl{object} on the \gls{canvas} look the same as in the previous
example, but have different co-ordinates. The co-ordinates have been
translated by half the page width and half page height. This
combined with the origin shift makes them appear in the same
location. However, note that the \glspl{printer-margin} are no
longer bordering the \gls{canvas}. When the image is printed to a
file, only the content within the \glspl{printer-margin} appears in
the output. Note, too, that when the paper orientation is changed to
landscape (\figureref{fig:grid-radial-origin-margins-landscape}) the
grid is no longer \manmsg{centred} on the \gls{canvas}. This is
because changing the paper properties (orientation or size) doesn't
change the \gls{origin} shift.

\FloatSubFigs{fig:grid-radial-page-margins}
{
  {fig:grid-radial-page-margins-portrait}{\includeimg{grid-radial-page-margins}}{},
  {fig:grid-radial-page-margins-printout}{\includeimg[frame,scale=0.42]{grid-radial-page-margins-printout}}{},
  {fig:grid-radial-page-margins-landscape}{\includeimg{grid-radial-page-margins-landscape}}{}
}
[Page-Dependent Radial Grid with Paper Margins]
{Page-dependent radial grid with visible paper margins:
\subfigref{fig:grid-radial-page-margins-portrait} image showing in
\FlowframTk\ (the thick \manmsg{grey} area represents the margins);
\subfigref{fig:grid-radial-page-margins-printout} image printout
(border added for clarity);
\subfigref{fig:grid-radial-page-margins-landscape} image showing in
\FlowframTk, now in landscape orientation.}

\FloatSubFigs{fig:grid-radial-origin-margins}
{
  {fig:grid-radial-origin-margins-portrait}{\includeimg{grid-radial-origin-margins}}{},
  {fig:grid-radial-origin-margins-printout}{\includeimg[frame,scale=0.42]{grid-radial-origin-margins-printout}}{},
  {fig:grid-radial-origin-margins-landscape}{\includeimg{grid-radial-origin-margins-landscape}}{},
}
[Origin-Based Radial Grid with Shift and Paper Margins]
{Origin-based radial grid with shift and visible paper margins:
\subfigref{fig:grid-radial-origin-margins-portrait} image showing in
\FlowframTk\ (the thick \manmsg{grey} area represents the margins);
\subfigref{fig:grid-radial-origin-margins-printout} image printout
(border added for clarity);
\subfigref{fig:grid-radial-origin-margins-landscape} image showing in
\FlowframTk, now in landscape orientation.}
...
The printer margins mark the margin bounds around the current page.
Remember that if the \gls{grid-origin} is shifted, this effectively
shifts the page around on the \gls{canvas}. This means that the
paper margins will be shifted as well. (See, for example,
\figureref{fig:grid-radial-origin-margins} in
\sectionref{sec:gridmenu}.)

\begin{information}
If you change the paper size or orientation when the margins are
showing, you may need to use \menu{file.page} to update the margins.
\end{information}
...
\gls+{bp}, in, cm, mm, \gls+{pc}, \gls+{dd} or \gls+{cc}).
...
\widget{button.selectfile} button to select the required
directory.
\end{itemize}
...
\widgetdef{jdr.warn_load_old}

If the \widget{jdr.warn_load_old} setting is on, there will be a
warning if you load a \gls{JDR} or \gls{AJR} file that is in an
old format. When you save the file with \menu{file.save}, it will 
by default use the latest stable format. If you still want to use
the old format, you will need to use \menu{file.save_as} and select
...
\subsubsection{Clipping Tag}\label{sec:cliptag}

There are several ways of clipping paths. If you want to actually
modify a \gls{shape}, then you can either use the
\gls{cag.intersection} function (see
\sectionref{sec:pathintersect}) or the \gls{shapeclipping} function (see
\sectionref{sec:clippingshapes}). If you don't want to lose the
original \gls{path} but just want it clipped when you export the
image to PGF code, then set up a \inlineglstextdef{clippingtag}
instead.

\begin{important}
No clipping is applied on the \gls{canvas}, but clipping is applied
when the image is exported to a \LaTeX\ file where it is rendered
with \env{pgfpicture}.  Bear in mind that the image \gls{bbox} (used
for the \widget{export.bounds} \widget{export.bounds.image} setting)
is calculated as normal, without any clipping. This is because the
actual clipping is done by the \sty{pgf} package not by
\FlowframTk. (See \figureref{fig:cliptag}.)
\end{important}

Usually, when \FlowframTk\ writes a \gls{path} to a \LaTeX\ file, it
will write the line:
\begin{codebox*}
\gls{pgfusepath}\marg{stroke,fill}
\end{codebox*}
(omitting either stroke or fill, if applicable), to stroke and fill
the path. However, if a clipping tag is set and the \gls{path} has
that tag in its \gls{taglist}, then \FlowframTk\ will instead write:
\begin{codebox}
\gls{pgfusepath}\marg{clip}
\end{codebox}

The \gls{clippingtag} was designed specifically for use with export
to PGF. It's not supported in export formats that don't export to 
a \LaTeX\ file or use \LaTeX\ as an intermediate step.

\FloatFig
  {fig:configdialog-clipping}
  {\includeimg{configureDialog-clipping}}
  {Configuration Dialog Box (Clipping Tag)}

\widgetdef{clipping.title}

The \gls{clippingtag} is specified in the \dialog{clipping} tab of the
\dialog{config} dialog. You will then need to assign this tag to the
required \gls{path} (see \sectionref{sec:objecttag}).
\strong{The path and all other \glspl{object} will show normally on the \gls{canvas}.}

\widgetdef{clipping.tag}

Supply your preferred tag in the \widget{clipping.tag} text box (no
spaces). When you export your image, any \gls{path} with a
(space-separated) \gls{taglist} that contains this tag will be written to
the exported file as a clipping path rather than a normal path.

The \gls{clippingtag} is saved in the image file (\gls{JDR}\slash\gls{AJR}
v2.2+), so existing images will use whatever tag was set in the
file. If you want to prevent clipping, you can simply set the
\widget{clipping.tag} field to empty if you don't want to remove the
tag from all the \glspl{path} in the current image.

\widgetdef{clipping.set_as_default}
If the \widget{clipping.set_as_default} checkbox is selected, then
this tag will be set as the default for new images. Otherwise, the
setting will only be applied to the current image.

Bear in mind that all \glspl{object} are written to the exported file 
according to the \gls{stackingorder}, so the \glsdisp{backobject}{backmost} 
object is written first. This means that any
objects behind the clipping path will not be clipped.

\begin{information}
If you have multiple paths with the designated \gls{clippingtag}, the clipping area won't
be enlarged but may be reduced. The \sty{pgf} manual provides further information.
\end{information}

If you want to scope the effects of the clipping, you will need to
insert a \env{pgfscope} environment during export. The simplest way to do this is
to group all the objects that need to be within the scope and export
with one of the object markup settings 
(\widget{export.markup.paired} or \widget{export.markup.encap}).
You will then need to provide a definition of the applicable
commands that introduce scoping around groups.
For example, with the \widget{export.markup.encap} setting:
\begin{codebox}
\cmd{renewcommand}\gls{flowframtkencapobject}\oarg{8}\marg{\comment{}
 \gls{FlowframTkScopeIfGroup}\marg{\#2}\marg{\#8}\comment{}
}
\end{codebox}
(This will require \sty{flowframtkutils} v2.2+.)

The tag will have no effect on \glspl{object} that are not written
as \glspl{path} when exported. For example, if the designated tag is
added to a \gls{bitmap} then no clipping path will be created.  With
\glspl{textarea}, it will depend on whether or not the
\gls{textarea} is converted to a path during export. Setting the tag
on a \gls{compositeshape} may or may not have an effect, depending on how
the \gls{compositeshape} is exported and whether or not the tag was
added to the underlying path before the \gls{compositeshape} was
created.

\FloatSubFigs*{fig:cliptag}
{
 {fig:cliptaga}{\includegraphics[scale=0.5]{cliptaga}}{},
 {fig:cliptagb}{\includegraphics[scale=0.4]{cliptagb}}{},
 {fig:cliptagc}{\includegraphics[scale=0.4]{cliptagc}}{}
}
[Clipping Tag]
{Clipping Tag: 
 \subfigref{fig:cliptaga} image showing on the \gls{canvas};
 \subfigref{fig:cliptagb} image in \LaTeX\ document (no scoping);
 \subfigref{fig:cliptagc} image in \LaTeX\ document (with scoping)
}

For example, in \figureref{fig:cliptag} the image consists of a
red square (\glsdisp{backobject}{backmost}), a \gls{group}
containing an unfilled circle, a green ellipse and a cyan ellipse,
a purple square, an unfilled rectangle and a yellow ellipse.
The unfilled circle and unfilled rectangle were both given the tag
\qt{clip}. (This had to be done before the circle was added to the
group.) The \gls{clippingtag} has been set to \qt{clip} (as shown in
\figureref{fig:configdialog-clipping}). 
All shapes are \rendered\ normally on the \gls{canvas}
(\figureref{fig:cliptaga}).

For both \figuresref{fig:cliptagb,fig:cliptagc},
the PGF export had the \widget{export.markup.encap} setting on.
However, in \figureref{fig:cliptagb}, no adjustment was made to the encap
command, which means the clipping isn't scoped. The clipping area
established by the unfilled circle
isn't expanded by the unfilled rectangle clipping. The top left
corner of the purple square is within the circle's clipping area
even though it's in front of the group. The yellow ellipse is
omitted as it's completely outside of the clipping area.

In \figureref{fig:cliptagc}, \gls{flowframtkencapobject} was redefined as
shown earlier, so clipping within the group is now scoped.
The unfilled rectangle can now set up a new clipping area as it's
outside of the scope. The purple square isn't clipped as it's now
outside of any clipping as it's rendered after the end of the scope
when the circle clipping was reset and it's before the unfilled
rectangle sets up a new clipping area. The yellow ellipse (the
\gls{frontobject}) is now clipped by the rectangle.

Note that in both cases, the \glsdisp{backobject}{backmost} red
square has not been clipped as it's rendered before any clipping
path is set. Also, the image \gls{bbox} doesn't take the clipping
into account. (You might want to
set up the \gls{image.typeblock} and use the
\widget{export.bounds.typeblock} export option.)

\subsubsection{External Processes (Application Paths)}\label{sec:processes}
...
\FloatFig
  ...
  {Configuration Dialog Box (Export Processes)}

If you \filefn{export} your image to PDF or if you export to 
PNG (or, if enabled, \gls{eps} or \gls{svg}) with the \widget{export.use_process} option on then
...
(\figureref{fig:configdialog-apppaths}) to set their locations.
...
\widgetdef{processes.support_eps}

The DVI format was still widely in use when \gls{JpgfDraw} was first
created (around 2005) so it was useful to be able to export to \gls{eps}.
However, the DVI format has mostly fallen out of use so, to reduce
clutter in the \dialog{export} dialog, the export to \gls{eps} is not
supported by default.  (The non-processor export to \gls{eps} is limited
and still a bit experimental.)

If you still require export to \gls{eps}, you need to enable support by
ensuring that the \widget{processes.support_eps} checkbox is
selected. This will enable additional fields for \app{latex} (DVI
format) and \app{dvips}.

\begin{warning}
The \code{-E} switch for \app{dvips} can be problematic with
graphical content. If you want \pgls{eps} file, 
it's simpler to just select the \widget{export.bounds.image} option 
for the \widget{export.bounds} setting in the \dialog{export} dialog.
\end{warning}

\widgetdef{processes.support_svg}

As with export to \gls{eps}, the external processors used to create
\pgls{svg} file also use \app{latex} (DVI format),
but will use \app{dvisvgm} instead of \app{dvips}. The \file{libgs} library is only
applicable with \app{dvisvgm}, so you only need it if you intend to
export to \gls{svg} use external processors.

Since the non-processor export to \gls{svg} can support most types of
\glspl{object}, support for export to \gls{svg} is on by default.
If you don't need this export format and you want to reduce the
clutter in the \dialog{export} dialog, then you can switch off this
setting by unchecking the \widget{processes.support_svg} checkbox.
...
The \menu{settings.configtex} menu item will open 
...
for all \filefn{export} functions that use the export to complete \LaTeX\
...
\widgetdef{flfsettings.pages_opt}
...
\manmsg{labelled} iii and the page \manmsg{labelled} 3.

Note that regardless of this setting, the page lists may only
contain plain integers not the formatted page number.
(See the \sty{flowfram} documentation for further details.) If you want to
...
\textattrpl{anchor} automatically adjusted. Similarly if you use the 
align to page function or the \hyperref[sec:aligntotypeblock]{align
to typeblock} function.
...
\FloatFig
  ...
  {LaTeX-Related Text Settings\dash \Glsentrytext{manual.maths}-Mode Mappings}
...
To restore all the original mappings, quit \FlowframTk\ and then
either delete or rename the mapping files \file{mathmappings.prop} and
...
image's \gls{early-preamble} code (\sectionref{sec:preamblepanels}), whenever the
mapping is applied.)
...
\href{http://www.dickimaw-books.com/latex/symbol-lookup.php}{\nolinkurl{dickimaw-books.com/latex/symbol-lookup.php}} which
...
complete \LaTeX\ documents or formats that use the export to
complete \LaTeX\ document function as an intermediate step (see
...
\file{preamble.tex} located in \FlowframTk's \gls{config_dir}. If you edit
this file outside of \FlowframTk\ while it's running, you'll need to use the reload
...
\widgetdef{preambleconfig.use_default_preamble}

If you don't want to use the default preamble, deselect the
\widget{preambleconfig.use_default_preamble}. This is simpler than
deleting the content from the file and then adding it back again if
you have an image where it's not applicable.
...
allows you to make adjustments to the way the image is
\rendered\ on the \gls{canvas}.
...
Each type of \gls{controlpt} has a particular \manmsg{colour}
depending on whether or not the \gls{controlpt} is selected.  The
\manmsg{colour} can be changed for each type and selection status.
Use the \btn{choose_colour} button next to the sample and state
(\widget{render.control.unselected} or
\widget{render.control.selected}) to open a \manmsg{colour} dialog
to choose a new \manmsg{colour}.
...
\widgetdef{annotations.scale}

If the \widget{annotations.scale} checkbox is on, the annotations will scale as you
increase or decrease the \gls{canvas} magnification 
(see \sectionref{sec:zoommenu}).

\widgetdef{annotations.annote_font}
The \widget{annotations.annote_font} is used for \gls{flowframe}
annotations and also for the file name of \glspl{bitmap} shown in draft
mode.

\Gls{flowframe} annotations can become quite cluttered if you have overlapping
\glspl{object}. The default is to show the \gls{typeblock} or frame
annotation at the bottom left (offset by the inner padding, if
applicable). This can be changed with the
\inlineglsdef{annotations.annote_typeblock_pos},
\inlineglsdef{annotations.annote_flow_pos},
\inlineglsdef{annotations.annote_static_pos}, and
\inlineglsdef{annotations.annote_dynamic_pos}
annotation settings. The options are:
\widget{annotations.annote_pos.none} (don't annotate),
\widget{annotations.annote_pos.bottom_left},
\widget{annotations.annote_pos.top_left},
\widget{annotations.annote_pos.top_right}, and
\widget{annotations.annote_pos.bottom_right}.

\widgetdef{annotations.frame_contents.show}
If the \widget{annotations.frame_contents.show} checkbox is
selected, any static or dynamic frames with contents set
will have the contents shown on the \gls{canvas}. This is
non-editable. You need to use \menu{tex.flowframe.set_frame}
if you want to change the contents (or use the \dialog{flfwizard}).

This can be distracting if there are many \glspl{object} on the
\gls{canvas}. In which case, unselect the 
\widget{annotations.frame_contents.show} to hide the content.

\widgetdef{annotations.frame_contents.font}
If the \widget{annotations.frame_contents.show} checkbox is
selected, the \widget{annotations.frame_contents.font} setting is
used to render the content.

This isn't the same as that used in the 
\TeX\ editor (see \sectionref{sec:texeditor}). Use the
\dialog{texeditorui} tab (see \sectionref{sec:texeditorui}) to
change that. There's no syntax highlighting in the annotation.

The frame contents annotation is the only setting in this panel
where you can also change the annotation text \manmsg{colour}.
Use the \btn{button.choose_colour} button to open the
\manmsg{colour} selector dialog.

\widgetdef{splash.title}
The \widget{splash.title} panel is for the fonts used on the splash
screen when \FlowframTk\ starts up.
...
individual \gls{statusbar} elements (see \figureref{fig:statusbar}
in \sectionref{sec:thestatusbar}).
...
\FlowframTk\ has small text editor (\sectionref{sec:texeditor})
that's opened when you want to edit the contents of a static or
dynamic frame.  The font used by this editor can be changed in the
\widget{texeditorui.title} tab (\figureref{fig:texeditorui}).  You
can also set the default width (in terms of approximate number of
characters per line) and the default height (in terms of lines) for
the editor when it's first created at startup.

The \gls{index.menu.texeditor} popup menu item
\menu{texeditor.settings} opens a dialog that has just the
\dialog{texeditorui} panel, without all the other UI configuration
options.

\widgetdef{texeditorui.num_columns}

The \widget{texeditorui.num_columns} setting controls the line
wrapping. The line width is calculated according to this value
multiplied by the width of the widest character (identified by
\verb|lang.widest_char| in the localisation file) rendered in the specified
font. If the editor width is narrower than this, scroll
bars will appear.

\widgetdef{texeditorui.highlight}

You can enable or disable syntax highlighting by selecting or deselecting
the button marked \widget{texeditorui.highlight}. If
selected, you can also set the \manmsg{colour} used for comments
(indicated by \TeX's \glssymbol{percent} comment character) or for commands.
(Use the \btn{button.choose_colour} button to open the
\manmsg{colour} selector dialog.) The
...
while you're editing the image. There are now multiple preamble
panes, and also a pane for the document body and magic comments.

These editor panes also use the \dialog{texeditorui} settings,
except for the default width and height, which is now governed
by the size of the \gls{canvas} window and the number of columns. 
You can move the divider between the \gls{canvas} and preamble area 
to resize the editor panes.

By default, the preamble pane is to the right of the
...
\widgetdef{lookandfeel.dialog.as_general}

The dialog buttons by defaylt have the same style as general
buttons, but if you uncheck the \widget{lookandfeel.dialog.as_general} 
checkbox you can select your preferred style for dialog buttons
using the settings that will become visible in the
\widget{lookandfeel.dialog_button_styles} area.

\widgetdef{lookandfeel.button.style}
...
For example, \figureref{fig:icontext} shows the
...
\item the \hyperref[sec:preamblepanels]{preamble} file \file{preamble.tex}
...
fonts used by the \hyperref[sec:annotations]{startup splash screen};
...
See \sectionref{sec:preamblepanels} for further details on how this
...
closes the dialog window. In some dialogs, you may also be able to
use \inlinekeystrokedef{button.alt_okay} as well, but sometimes an
active widget with the focus may override this behaviour.
...
The \widget{button.choose} button (\btntooltip{choose}) opens a new dialog in which you
can choose a sub-setting from the main dialog window.

\widgetdef{button.choose_colour}

The \widget{button.choose_colour} button (\btntooltip{choose_colour}) opens
a dialog in which you can choose a \manmsg{colour}.

\widgetdef{button.selectfile}

The \widget{button.selectfile} button (\btntooltip{selectfile}) opens a file chooser in
...
will load each file into \FlowframTk. If you \gls*{drag-and-drop} a
...
If you want to open an image in a new child window using \gls{drag-and-drop},
...
You can use the \menu{file.image_description} dialog box to set the
\inlineglsdef{imagedescription}. The description is not visible in the image,
...
The \gls{imagedescription} will also be used in the applicable
markup command if \widget{export.markup} is set to
\widget{export.markup.paired} or \widget{export.markup.encap}.
...
\gls{JDR} (binary) or \gls{AJR} (plain text) format using either
...
\gls{eps} or \gls{svg} (as applicable) first.
The default setting enables support for \gls{svg} but not for
\gls{eps} (see \sectionref{sec:processes}). 
...
The \menu{file.export} menu item will open the \dialog{export}
dialog box, which provides a way to specify the name and location of
the file as well as what format to export to and what settings to
apply.  The available widgets in the dialog vary depending on the chosen export
type.
...
\FloatFig
 {fig:export-row-file}
 {\includeimg[scale=0.6]{export-row-file}}
 {Export Dialog: File}

At the top of the \dialog{export} dialog is the
\inlineglsdef{export.file} field for the path to the desired export
file (\figureref{fig:export-row-file}). You can either type in the
full path or use the \btn{selectfile} button at the side to open
the file chooser.

\widgetdef{export.format}

\FloatFig
 {fig:export-row-format}
 {\includeimg[scale=0.6]{export-row-format}}
 [Export Dialog: Format]
 {Export Dialog: Format (SVG or EPS may not be present)}

Below the \widget{export.file} field is the \widget{export.format} area where you
can select the required export format (\figureref{fig:export-row-format}).
...
PDF file to the location specified in the file field. Since the
temporary files aren't deleted until you quit \FlowframTk, if any
errors occur, you can view the \LaTeX\ transcript file as long as \FlowframTk\ is
still running. (The \dialog{message} window will show the process
command and the directory in which it ran.)
\end{itemdesc}
...
The \gls{clippingtag} is ignored, just as it is on the \gls{canvas}.
...
\begin{itemdesc}
The export to \gls{eps} function is no longer available by default but can
be enabled with the \widget{processes.support_eps} setting in 
...
You can either get \FlowframTk\ to write the \gls{eps} content directly
...
\begin{itemdesc}
The export to \gls{svg} function is available if the
\widget{processes.support_svg} setting in the
\widget{processes.title} tab of the \dialog{config} dialog is
selected.

When using external processors, this function works in much the same
way as the \widget{export.eps} option.  In this case, \app{dvisvgm}
is used instead of \app{dvips}. Unless you need to use \LaTeX\
features, such as fonts or the \LaTeX\ alternative text, the
non-processor export to \gls{svg} produces neater code and any
\glspl{object} that have descriptions will have the description included
in a \xmlelem{title} element. However, the non-processor export
doesn't support the \gls{clippingtag}.
\end{itemdesc}
...
directory will be deleted when you quit \FlowframTk.
...
\subsection{Export Settings}\label{sec:exportsettings}

This section describes common settings shared by the different export functions.
...
\FloatFig[hbp]
 {fig:export-row-bbox}
 {\includeimg[scale=0.6]{export-row-bbox}}
 {Export Dialog: Bounding Box}

The \widget{export.bounds} panel isn't always available, but when it
is, it may be used to specify the bounds of the exported image
(\figureref{fig:export-row-bbox}).
It's not available for any of the \widget{export.format.flf_type}
functions. 

For export to PNG (non-processor), this establishes the image size.
For export to \LaTeX, this setting just establishes the \env{pgfpicture}
bounds with:
\begin{codebox*}
\gls{pgfusepath}\marg{use as bounding box}
\end{codebox*}
This doesn't automatically clip the content so anything outside of the designated
bounds will overflow the PGF bounding box. If you need to clip the
image, set up a \gls{clippingtag} (see \sectionref{sec:cliptag}).
However, when exporting to a complete \LaTeX\ document, this setting
will also govern the page geometry which will naturally clip
anything outside of the page boundary.
...
The bounds should be set to the image's \gls{bbox}.
...
are when drawn on the \gls{canvas}. The image \gls{bbox} doesn't
take the \gls{clippingtag} into account.
...
\gls{typeblock} set. This setting may be used with a
\gls{clippingtag} to both clip and set the bounds. For example,
create a rectangle at the \glsdisp{backobject}{back} of the image,
add the \gls{clippingtag} to its \gls{taglist} and set the
\gls{typeblock} to the same area (with
\widget{typeblock.compute_from_objects} in the \dialog{typeblock}
dialog).

\widgetdef{export.markup}

\FloatFig[hbp]
 {fig:export-row-markup}
 {\includeimg[scale=0.6]{export-row-markup}}
 {Export Dialog: Markup}

The \widget{export.markup} options
(\figureref{fig:export-row-markup}) are only applicable for the
...
step. The markup is a command or a pair of commands inserted into
the code in the body of the \env{pgfpicture} environment.  The
...
do nothing but may be redefined in conjunction with 
...
The \meta{Java class name} is the \gls{javaclass} simple name of the \gls{object}:
\inlineglsdef{JDRGroup} for a \gls{group}, \inlineglsdef{JDRPath} for a \gls{path},
\inlineglsdef{JDRBitmap} for a \gls{bitmap},
\inlineglsdef{JDRText} for a \gls{textarea},
\inlineglsdef{JDRTextPath} for a \gls{textpath},
\inlineglsdef{JDRRotationalPattern} for a rotational \gls{pattern},
\inlineglsdef{JDRScaledPattern} for a scaled \gls{pattern},
\inlineglsdef{JDRSpiralPattern} for a spiral \gls{pattern}, and
\inlineglsdef{JDRSymmetricPath} for a symmetric shape.

\begin{information}
Even if an export setting causes a \gls{textarea} or \gls{textpath}
to be converted into a path, the class name will still be
\gls{JDRText} or \gls{JDRTextPath}, as applicable, not
\gls{JDRPath}.
\end{information}

The \meta{description} is the \gls{imagedescription}
(\sectionref{sec:imagedescription}) when \meta{n} is 0 otherwise
it's the \gls{objectdescription}
(\sectionref{sec:objectdescription}). This argument will be empty if
no description has been set.

The \meta{tag} is the space-separated \gls{taglist} (\sectionref{sec:objecttag}).
This argument will be empty if no \gls{taglist} has been set.
(The \gls{imagetag} will always be empty.)
...
code. The \sty{flowframtkutils} package provides a configurable function
designed for slide overlays that
may be used in the definition of this command. You can enable it
with:
\cmddef{FlowFramTkUtilsSetOverlayEncap}
See the \sty{flowfram} documentation (v2.0+) for further details.

\begin{information}
The following settings indicate what to do if the export option
doesn't support a particular feature. This may result in the
\gls{object} being written to the exported file as a different type
of object. For example, a \gls{textarea} may be written as a filled
path. These settings don't change the image itself but just
determine how the object is exported. Note that if an object is
converted in this way, this may change how other settings affect it.
For example, if a \gls{textpath} is exported as a filled (glyph) path, then
the text gradient paint setting will no longer be applicable.
\end{information}

\widgetdef{export.markers}

\FloatFig[hbp]
 {fig:export-row-markers}
 {\includeimg[scale=0.6]{export-row-markers}}
 {Export Dialog: Markers}

The \widget{export.markers} setting (\figureref{fig:export-row-markers}) indicates how
\hyperref[sec:markers]{markers} should be exported.

\widgetdef{export.markers.separate}

If \widget{export.markers.separate} is selected, then any markers
will be drawn as separate path (after the actual shape's outline is
drawn).

\widgetdef{export.markers.stroked}

If \widget{export.markers.stroked} is selected, then the stroked
path will be converted to a filled outline and the markers will be
merged into it.

\widgetdef{export.markers.separate_or_stroked}

If \widget{export.markers.separate_or_stroked} is selected, then it
will behave like \widget{export.markers.separate} unless the shape's
\pathattr{line-paint} is a gradient paint, in which case it will
behave like \widget{export.markers.stroked}.

\widgetdef{export.strokeshading}

\FloatFig[hbp]
 {fig:export-row-stroke-gradient}
 {\includeimg[scale=0.6]{export-row-stroke-gradient}}
 {Export Dialog: Stroke Gradient Paint}

If the export format doesn't support stroke \dgls{gradient-paint}, 
the \widget{export.strokeshading} panel
(\figureref{fig:export-row-stroke-gradient}) provides options to indicate
what should be done if the image contains any gradient
\pathattr{line-paint}.

\widgetdef{export.strokeshading.average}

If \widget{export.strokeshading.average} is selected, any gradient
\pathattr{line-paint} will be exported with the paint set to the
average \manmsg{colour}.

\widgetdef{export.strokeshading.start}

If \widget{export.strokeshading.start} is selected, any gradient
\pathattr{line-paint} will be exported with the paint set to
the start \manmsg{colour}.

\widgetdef{export.strokeshading.end}

If \widget{export.strokeshading.end} is selected, any gradient
\pathattr{line-paint} will be exported with the paint set to
the end \manmsg{colour}.

\widgetdef{export.strokeshading.to_path}

If \widget{export.strokeshading.to_path} is selected, any
\glspl{shape} with a gradient \pathattr{line-paint} will be exported
with the stroke outline converted to a path.
(This won't affect the \gls{shape}['s] actual \pathattr{fill-paint} setting.)

\widgetdef{export.textualshading}

\FloatFig[hbp]
 {fig:export-row-text-gradient}
 {\includeimg[scale=0.6]{export-row-text-gradient}}
 {Export Dialog: Text Gradient Paint}
...
\widgetdef{export.textualshading.to_path}

If \widget{export.textualshading.to_path} is selected, any text
with a \dgls{gradient-paint} will be exported as though the
\gls{textarea} had been converted to a path (except that the 
\meta{Java class name} will be \gls{JDRText} or \gls{JDRTextPath}).

\widgetdef{export.textareaoutline}

\FloatFig[hbp]
 {fig:export-row-textarea-outlines}
 {\includeimg[scale=0.6]{export-row-textarea-outlines}}
 {Export Dialog: Text Area Outline}

The \widget{export.textareaoutline} panel
(\figureref{fig:export-row-textarea-outlines}) governs the way that
\glspl{textarea} with the \textattr{outline} attribute on are
exported.  Any \glspl{textarea} that don't have the
\textattr{outline} attribute on are unaffected by this export
setting.

\widgetdef{export.textareaoutline.ignore}

If \widget{export.textareaoutline.ignore} is selected, the
\textattr{outline} text attribute will be ignored for all
\glspl{textarea}.

\widgetdef{export.textareaoutline.to_path}

If \widget{export.textareaoutline.to_path} is selected, then any
\gls{textarea} with the \textattr{outline} setting on will be
exported as a path. This means that the \LaTeX\ alternative text
won't be used.

\widgetdef{export.textareaoutline.use_cmd}

If \widget{export.textareaoutline.use_cmd} is selected, any
\glspl{textarea} with the \textattr{outline} setting on will be
encapsulated with \gls{jdroutline} (which is defined by
\sty{flowframtkutils}). This option is unavailable for export
functions that don't require \LaTeX.

If you are using the \widget{export.pgfpicture} setting 
then you will need to remember to load \sty{flowframtkutils},
but be sure to pass the \opt{flowframtkutils.outline} option to enable support.
This will be done automatically if the export function creates a
complete \LaTeX\ document or a class or package.

\widgetdef{export.textpath}

\FloatFig[hbp]
 {fig:export-row-textpath}
 {\includeimg[scale=0.6]{export-row-textpath}}
 {Export Dialog: Text-Path}

The \widget{export.textpath} panel
(\figureref{fig:export-row-textpath}) indicates how \glspl{textpath}
should be exported. A \gls{textpath} with transformed text means
that the text transformation matrix is not the identity matrix or
that there is a shift introduced by the vertical \textattr{anchor}.

\widgetdef{export.textpath.pgf}

If \widget{export.textpath.pgf} is selected, the export function
will write code to decorate a path with text, as far as
any limitations of the export format allow. This will require the
\sty{decorations.text} PGF library. 
If you are using the \widget{export.pgfpicture} setting, you will
need to ensure the library is loaded in your document. This can
easily be done by loading \sty{flowframtkutils} with the 
\opt{flowframtkutils.textpath} 
package option, but you can also simply load it as any other PGF
library:
\begin{codebox*}
\gls{usepgflibrary}\marg{decorations.text}
\end{codebox*}
The text decoration is quite restricted. In particular, take care if
you have alternative \LaTeX\ text set that contains commands as they
may trigger errors. See the PGF manual for further details.

This export option doesn't support transformed text or outline text.
The horizontal \textattr{anchor} is supported, but not the vertical
anchor.

\widgetdef{export.textpath.split}

If \widget{export.textpath.split} is selected, the export function
will export any \gls{textpath} as though it had been split into
separate \glspl{textarea}. Bear in mind that if you set alternative
\LaTeX\ text, that will be ignored but mapping will be applied as
usual for the \gls{menu.transform.split} function.

\widgetdef{export.textpath.to_path}

If \widget{export.textpath.to_path} is selected, the export function
will export \glspl{textarea} as though they had been converted to
paths.

\widgetdef{export.textpath.split_if_transformed}

If \widget{export.textpath.split_if_transformed} is selected, then 
any \glspl{textpath} that have transformed text will be treated as
for \widget{export.textpath.split} otherwise they will be exported
as for \widget{export.textpath.pgf}.

\widgetdef{export.textpath.to_path_if_transformed}

If \widget{export.textpath.to_path_if_transformed} is selected, then 
any \glspl{textpath} that have transformed text will be treated as
for \widget{export.textpath.to_path} otherwise they will be exported
as for \widget{export.textpath.pgf}.

\widgetdef{export.textpathoutline}

\FloatFig[hbp]
 {fig:export-row-textpath-outlines}
 {\includeimg[scale=0.6]{export-row-textpath-outlines}}
 {Export Dialog: Text-Path Outlines}

If the export format doesn't support \gls{textpath} outlines, the
\widget{export.textpathoutline} panel
(\figureref{fig:export-row-textpath-outlines}) provides options to
indicate what should be done if the image contains any
\glspl{textpath} with the \textattr{outline} mode on.

\widgetdef{export.textpathoutline.ignore}

If \widget{export.textpathoutline.ignore} is selected, each
\glspl{textpath} will be exported as though the \textattr{outline} mode
was off if the \gls{textpath} is exported as a decoration.
This will be ignored if the \widget{export.textpath} setting results in
the \gls{textpath} being exported as \glspl{textarea} (in which case
the \widget{export.textareaoutline} setting will apply) or as
a path.

\widgetdef{export.textpathoutline.split}

If \widget{export.textpathoutline.split} is selected, all
\glspl{textpath} with the \textattr{outline} mode attribute set 
will be exported as though they had been split into separate
\glspl{textarea}. Bear in mind that if you set alternative
\LaTeX\ text, that will be ignored but mapping will be applied as
usual for the \gls{menu.transform.split} function.
This overrides the \widget{export.textpath} settings.

\widgetdef{export.textpathoutline.to_path}

If \widget{export.textpathoutline.to_path} is selected, all
\glspl{textpath} with the \textattr{outline} mode attribute set 
...
\FloatFig[hbp]
 {fig:export-row-doc-class}
 {\includeimg[scale=0.6]{export-row-doc-class}}
 {Export Dialog: Document Class}
...
\widgetdef{export.use_default_preamble}

This setting is the same as the
\widget{preambleconfig.use_default_preamble} setting in the
\widget{preambleconfig.title} tab of the \dialog{texconfig} dialog
(see \sectionref{sec:texconfigpreamble}).

\widgetdef{export.write_src_path}

For export options that create a \ext{tex}, \ext{sty} or \ext{cls}
file, the \widget{export.write_src_path} setting determines whether
or not to write the path of the original \ext{jdr}\slash\ext{ajr} of
the image (if it has been saved) as a comment near the start of the
exported file.

\widgetdef{export.write_date_comment}

For export options that create a \ext{tex}, \ext{sty} or \ext{cls}
file, the \widget{export.write_date_comment} setting determines whether
or not to write the file creation date as a comment near the start
...
option should be on a separate line. Any option that contains one or
more spaces will automatically be delimited, so don't include
double-quotes. The following placeholders are available:
\begin{deflist}
\itemtitle{\inlineglsdef{placeholder-inputfile}}

\begin{itemdesc}
The name of the input file. This is a file in the temporary
directory, not the current image file. It won't have the same
basename as the designated export file.
\end{itemdesc}
...
\FloatFig{fig:pdfprocessor}
 {\includeimg[scale=0.6]{pdfprocessor}}
 {The \pdfLaTeX\ Processor Panel (Default Setting)}

The default is to use the \app{pdflatex} application to create a PDF
file (\figureref{fig:pdfprocessor}). You can change this, for
example to \app{lualatex}, but don't change it to an application that
doesn't create a PDF file from \LaTeX\ source.
...
The \widget{export.flfpdf} function may
...
\app{latexmk}. Note the need in this case to add \code{-pdf} to the list of
options to ensure that a PDF file is created.

\FloatFig{fig:pdfprocessor-latexmk}
 {\includeimg[scale=0.6]{pdfprocessor-latexmk}}
...
The \gls{early-preamble} text (\sectionref{sec:preamblepanels}) is added before the
...
\sectionref{sec:preamblepanels}). For example, the default preamble shown in
\figureref{fig:texconfig-preamble} in \sectionref{sec:texconfigpreamble} has:
\begin{verbatim}
...
\hyperref[sec:preamblepanels]{image preamble}. This is done automatically
by the \gls{textarea-mapping} function, if it has been
\hyperref[mi:texmappings]{enabled}.  See
\sectionsref{sec:texconfigtext,sec:preamblepanels} for further details.
...
\widget{typeblock.compute_from_objects} button. Then delete the
...
\seealsorefs{sec:texconfig,sec:preamblepanels,sec:fontanchor}

\subsection{Export to SVG}\label{sec:exportsvg}

%\FloatFig{fig:exportsvgdialog}
 %{\includeimg{exportsvgdialog}}
 %{Export Dialog (SVG)}

The export to \gls{svg} function is only available if the
\widget{processes.support_svg} checkbox has been selected
...
function in that you can choose whether or not to use external tools to
...
The non-processor export to \gls{svg} produces cleaner code and will
include any \gls{object} descriptions in a \xmlelem{title} element.
However, this method doesn't support text outlines so the
\widget{export.textpathoutline} and \widget{export.textareaoutline}
panels will be available. If the text is converted to a path
then the content of the \xmlelem{title} element will be the original text.

If the \widget{export.use_process} checkbox is selected, a temporary
\LaTeX\ file will be created, as for \widget{export.imagepdf}, but
in this case \app{latex} not \app{pdflatex} is needed.  The
\widget{processes.latex} processor panel and the
\inlineglsdef{processes.dvisvgm} processor panel will be shown.
Additionally, the \widget{processes.libgs} panel will be shown where
you can optionally specify the path to the \file{libgs} library. You
may leave this blank if this information can be obtained from the
\gls{LIBGS} environment variable.

\begin{warning}
The DVI format is quite old and is more problematic than the newer
PDF format engines. Unless you need \LaTeX\ features, such as alternative
\LaTeX\ text for \glspl{textarea}, using \FlowframTk's native
non-processor SVG export is simpler and produces more readable code.
\end{warning}

\subsection{Export to EPS}\label{sec:exporteps}

\FloatFig{fig:exportepsdialog}
 {\includeimg{exportepsdialog}}
 {Export Dialog (EPS)}

The export to \gls{eps} function is only available if the
\widget{processes.support_eps} checkbox has been selected
(in the \widget{processes.title} tab of the \dialog{config} dialog).
...
exporting to \gls{eps} without an external tool is quite basic and may not
work correctly.

To create \pgls{eps} file of the current image, specify the target
...
If the image contains any \glspl{bitmap}, \FlowframTk\ will attempt to convert
them to \gls{eps}, where necessary, if the \widget{export.bitmaps_to_eps}.
...
As with the \widget{export.imagepdf} option, if the resulting
\gls{svg} or \gls{eps} file has unexpected results or fails to be
created, try exporting to a \LaTeX\ document
(\widget{export.imagedoc}) and then manually building the document.
...
\begin{information}
There are currently only two file format types that are supported in
normal (non-experimental) mode: \gls{svg} and the
\gls{Acorn-DrawFile-format}.
\end{information}

The original \gls{JpgfDraw} application was inspired by \gls{Acorn}['s] Draw
application. The \gls{Acorn-DrawFile-format} is a little-endian binary format that can
...
but can extract embedded JPEG data (if the
\widget{import.extract_bitmaps} checkbox is on);
...
in the \gls{grid};
\item \FlowframTk's triangle markers are different from
Draw's triangle markers but the best match will be chosen;
...
followed by three hexadecimal digits}) for the
\inlineglsdef{filter.acorndraw} filter in the file chooser.
\end{itemize}

When importing from \gls{svg}, \glspl{co-ordinate} or dimensions without
units are assumed to be \gls{bp} but they will then be converted to the
current \gls{storageunit}.  Only a limited subset of \gls{svg}
elements and attributes are supported by the \gls{svg} import
function. The following elements are recognised although they may
not support all attributes:
\begin{itemize}
\item \xmlelem{svg} (the main container, but clipping not supported);
\item \xmlelem{title} (used to set the image or object description);
\item \xmlelem{desc} (if \xmlelem{title} not present, used to set
the image or object description);
\item \xmlelem{image} (the referenced image file must be
local, either relative or prefixed with the \code{file} scheme);
\item \xmlelem{text} (may produce a single \gls{textualobject} or a
\gls{group} of \glspl{textualobject} depending on the attributes and
content);
\item \xmlelem{tspan} (may produce a single \gls{textarea} or a
\gls{group} that will be added to the encapsulating \xmlelem{text}
container's collection);
\item \xmlelem{a} (restricted to within \xmlelem{text}, but the
\LaTeX\ alternative text will be encapsulated with \gls{href},
provided the \xmlattr{href} attribute has a path);
\item \xmlelem{textPath} (offset and spacing ignored, this will
create a \gls{textpath});
\item shapes: \xmlelem{rect}, \xmlelem{ellipse},
\xmlelem{circle}, \xmlelem{line}, \xmlelem{polyline},
\xmlelem{polygon}, \xmlelem{path};
\item \xmlelem{linearGradient}, \xmlelem{radialGradient} and \xmlelem{stop}
(limited as \FlowframTk's \dgls{gradient-paint} only supports two
\manmsgpl{colour} with fixed starting points or directions);
\item \xmlelem{marker} (very limited);
\item \xmlelem{style} (limited CSS support);
\item \xmlelem{defs} and \xmlelem{use} (the referenced element must be
defined beforehand within the file);
\item \xmlelem{g} (will add a \gls{group} if its content causes more
than one \FlowframTk\ \gls{object} to be created, otherwise just the
...
\begin{itemize}
\item \gls{Acorn-DrawFile-format} (always available);
\item \gls{svg} (\ext+{svg}, always available).
\item \gls{eps} (\ext+{eps} or \ext+{ps}, only in experimental mode);
\end{itemize}
...
If a DrawFile doesn't have the \ext+{aff} suffix, you can just use
the \qt{All Files} filter. Other file types will need the
...
When importing from a \glslink{Acorn-DrawFile-format}{DrawFile}, any
text lines that have the Selwyn font will be converted to
\mathsmode, and the \mathsmode\ mappings will be used, otherwise
\mathsmode\ mappings will only be used if the text starts and end
with \gls{mshift} and doesn't contain any \glssymbol{mshift} in between.

When importing from \pgls{svg} file, \xmlelem{text} and \xmlelem{tspan}
are supported but may be converted into either a single
\gls{textarea} or a \gls{group} containing multiple
\glspl{textarea}. Each \gls{textarea} will have mappings
applied. If the text starts and ends with \gls{mshift} and doesn't
contain any \glssymbol{mshift} in between, then the \mathsmode\
mappings will be applied otherwise the \textmode\ mappings will be
applied (if \widget{import.use_mappings} is checked).

The \gls{svg} import setting only supports the \xmlelem{a} element
inside a \xmlelem{text} container. If found, mappings will be
applied as per \xmlelem{tspan} but the \LaTeX\ alternative text will
then be encapsulated with \gls{href}. The import function does not
support \xmlelem{a} outside of \xmlelem{text}.

\widgetdef{import.markers}

The \widget{import.markers} row is only available with imports from
\gls{svg}. The \gls{svg} format allows arbitrary shapes for markers,
which \FlowframTk\ doesn't currently support. There are three
choices as to what \FlowframTk\ should do if markers are encountered
in the imported file.

Select the \inlineglsdef{import.markers.ignore} button to ignore all
markers. Select the \inlineglsdef{import.markers.add_shapes}
button to convert all markers to \glspl{shape}. This means that each
marker will be a separate \gls{object} independent of the path they
were associated with in the \gls{svg} file and are not part of the
path's \gls{line-style}. These marker shapes will all have their tag
set to \qt{marker} to make them easier to find in the
\dialog{findbydescription} dialog box. 

Select the \inlineglsdef{import.markers.markers} button
to replace each marker with one of \FlowframTk's predefined 
\pathattrpl{marker}. Note that this may result in a very different
shape to the definition in the \gls{svg} file. For example, you may
simply end up with \gls{arrow.cross} markers. However, they will be
part of the path's \gls{line-style} and so can be changed when you
edit the path style.

\widgetdef{import.paper}

The \widget{import.paper} row is only available with imports from
\gls{svg}. The \gls{Acorn-DrawFile-format} includes the paper \gls{paper.size}
but \gls{svg} files typically have the \xmlattr{viewBox} attribute
set to the image bounds. Clipping isn't automatically implemented by \FlowframTk,
but you can choose how the paper \gls{paper.size} for the imported image is
determined.

\widgetdef{import.paper.current}

If the \widget{import.paper.current} button is selected, the current
paper setting will be used. Unlike the other two options, no
shifting is applied. This may result in some or all of the
imported \glspl{object} ending up outside the \gls{canvas} bounds.
You can use the \menu{navigate} menu to find them.

\widgetdef{import.paper.custom}

If the \widget{import.paper.custom} button is selected, the paper
\gls{paper.size} will be set to match the \xmlattr{viewBox}
attribute, if supplied, or the image \gls{bbox} otherwise.
The image content will be shifted so that its aligned with the top
and left of the page bounds.

\widgetdef{import.paper.predefined}

If the \widget{import.paper.predefined} button is selected, the paper
\gls{paper.size} will be set to one of \FlowframTk's predefined
paper \glspl{paper.size}. The smallest predefined paper that can
fully contain the image will be chosen, if possible, otherwise the
largest closest match will be chosen.
The image content will be shifted so that its aligned with the top
and left of the page bounds.
...
This setting is not applicable with \gls{svg} files, which use a
reference (\xmlattr{href}) in the \xmlelem{image} element, so the
\widget{import.extract_bitmaps} button and associated fields won't
be visible if the import type has been set to \gls{svg}.
...
The \widget{import.bitmap_dir} field will only be visible if the
\widget{import.extract_bitmaps} button is both visible and selected.
...
file's basename.
The \widget{import.bitmap_prefix} field will only be visible if the
\widget{import.extract_bitmaps} button is both visible and selected.
...
\menu{file.messages} item. This usually just lists the files that
have been opened, saved or exported, but warnings are sometimes
written there as well. This window will automatically open following a process
if any warnings or errors were reported.

Information about any processes will be included. The command echoed
in the window is just a simple concatenation of options for the
purpose of the message window. The actual process is created using
...
Line breaks are not supported in a \gls{textarea}. If you are using
...
\hyperref[sec:preamblepanels]{early-preamble}. The mapping function will
...
\begin{information}
The \dialog{symbolselector} dialog uses the same font as the
\gls{textarea}. Any characters that are not supported by the font
will be \rendered\ with the missing glyph character (often shown as a
rectangle). If you need one of those characters, you will have to change the font.
\end{information}
...
to the \hyperref[sec:preamblepanels]{early-preamble pane}, as shown in
...
export to \gls{eps} or \gls{svg} function, \FlowframTk\ will
attempt to create \pgls{eps} version of the bitmap (if it doesn't
...
You need to be in \selectmode\ with no selected
\glspl{object} if you want to use the \menu{tools.create_path_from_svg} menu
item. If this menu item is disabled, try \menu{tools.select}
followed by \menu{edit.deselect_all}.

\menudef{menu.tools.create_path_from_svg}

The \menu{tools.create_path_from_svg} menu item opens the
\dialog{svg_path} dialog (\figureref{fig:create-path-from-svg})
which provides a way to create a \gls+{path} from the same
syntax as the \gls+{svg} path \qtt{d} property. Note that this is
not the same as importing a complete \gls{svg} file.

\widgetdef{svg_path.title}

\FloatFig{fig:create-path-from-svg}
{\includeimg{create-path-from-svg}}
{Create Path from SVG Specs}
...
The \gls{svg} specifications use a left-handed \gls{co-ordinate} system,
which is the same as \FlowframTk's \gls{canvas}.
(The origin is at the top left and $y$ increases downwards.)
It's possible you may want to use the same path syntax but for a
right-handed \gls{co-ordinate} system where the origin is at the
bottom left with $y$ increasing upwards. This may be done
by selecting the \widget{svg_path.data_coords.righthand} button.
...
Once you're happy with the draft path, click on the \btn{okay}
...
This chapter describes how to select one or more \glspl{object},
move \glspl{object} up and down the \gls{stackingorder} (that is, in
front or behind other \glspl{object}), how to perform basic editing
such as grouping, alignment, cut, copy and paste, and how to set an \gls{object}['s]
description and tag. See \sectionref{sec:textareas} to edit
\glspl{textarea}, \sectionref{sec:affinetrans} to shift, rotate,
scale or shear an \gls{object},
\sectionref{sec:modshape} to modify \glspl{shape},
and \sectionref{sec:styles} to change path and text styles.
...
\gls*{drag-and-drop}) which will add all the objects from those files to
the current image. The new objects will be added to the current
selection. You can't \gls*{drag-and-drop} if a shape is being edited or
distorted.
...
In \selectmode, you can \gls{menuclick} to activate the
...
(see \sectionref{sec:objectdescription}).
This option will only be available if there is exactly one
\gls{object} selected.

\menudef{menu.selected.object_tag}

The \menu{selected.object_tag} menu item opens the \dialog{tag}
dialog from which you assign a tag or space-separated \gls{taglist} to all
\selected\ objects. This is the same as using the 
\menu{edit.object_tag} menu item from the main menu bar (see \sectionref{sec:objecttag}).
...
\menudef{menu.selected.align_to_page}

Move all selected \glspl{object} so that they are aligned relative
to the page.
This is the same as using the \menu{transform.align_to_page} menu item.

\menudef{menu.selected.flowframe}

The \menu{selected.flowframe} menu provides \gls{flowframe} related
menu items.

\menudef{menu.selected.flowframe.wizard}
Opens the \dialog{flfwizard} dialog (see
\sectionref{sec:flowframe}). This is the same as using the
\menu{tex.flowframe.wizard} menu item.

\menudef{menu.selected.flowframe.set_frame}
Opens the \dialog{flowframe} dialog (see \sectionref{sec:framedef}).
This is the same as using the \menu{tex.flowframe.set_frame} menu
item.
The \menu{selected.flowframe.scale_to_typeblock} menu item is only
available if the \gls{typeblock} has been set and a single
object is selected.

\menudef{menu.selected.flowframe.set_typeblock}
Opens the \dialog{typeblock} dialog (see
\sectionref{sec:typeblock}).
This is the same as using the \menu{tex.flowframe.set_typeblock}
menu item.

\menudef{menu.selected.flowframe.scale_to_typeblock}
Scales the selected object or objects to fit the \gls{typeblock}
(aspect ratio may change). This is the same as using the
\menu{tex.flowframe.scale_to_typeblock} menu item (see \sectionref{sec:scaletotypeblock}).
The \menu{selected.flowframe.scale_to_typeblock} menu item is only
available if the \gls{typeblock} has been set and one or more
objects are selected.

\menudef{menu.selected.flowframe.align_to_typeblock}
Aligns the selected object or objects to the \gls{typeblock}.
This sub-menu is the same as the
\menu{tex.flowframe.align_to_typeblock} sub-menu
(see \sectionref{sec:aligntotypeblock}).
The \menu{selected.flowframe.align_to_typeblock} menu item is only
available if the \gls{typeblock} has been set and one or more
objects are selected.
...
The \menu{selected.select_all}, \menu{selected.paste},
\menu{selected.flowframe.wizard} and
\menu{selected.flowframe.set_typeblock} common menu items are also available.
...
In addition to the general \gls{index.menu.selected} items,
...
\menu{edit.path.fill_colour} menu item from the main menu bar.
...
alternative, use \menu{selectedtext.edit} to open the
\dialog{edittext} dialog. This is the same as using
...
\menu{edit.textarea.fill_colour} menu item from the main menu bar.
...
\menudef{menu.selectedtext.reset_textmatrix}

To reset the \gls{textarea}['s] transformation matrix for all
selected \gls{textarea}, use \menu{selectedtext.reset_textmatrix}.
This is the same as using the \menu{transform.reset} menu item from
the main menu bar. The matrix can also be changed in the
\dialog{edittext} dialog.
...
If a \gls{textpath} has been selected then the
\gls{index.menu.selectedtextpath} popup menu will have a 
\menu{selected.path} sub-menu and a \menu{selected.textarea} 
sub-menu.
...
the general \menu{selected.object_description} menu item 
but has a more specific label.

\menudef{menu.selectedtextpath.textpath_tag}

To set or edit the \gls{textpath}['s] tag, use
\menu{selectedtextpath.textpath_tag}. This is the same as
the general \menu{selected.object_tag} menu item but has a more specific label.

The \menu{selected.path} menu has the same items as the
\dgls{menu.selectedpath} popup menu that govern the \dglspl{path-attribute}
but additionally has the \inlineglsdef{menu.selectedpath.textpathshow} toggle
item to toggle the \pathattr{textpathshow} attribute, which 
is the same as the \menu{edit.path.textpathshow} toggle menu
item.

The \menu{selected.textarea} menu has the same items as the
\dgls{menu.selectedtext} popup menu that govern the
\dglspl{text-attribute}.
...
\menudef{menu.selectedbitmap.reset_bitmapmatrix}

To reset the \gls{bitmap}['s]
transformation matrix, use \menu{selectedbitmap.reset_bitmapmatrix}. This is the
...
menu items as \gls{index.menu.selectedpath} that are specific to
\glspl{path}, but additionally has the \menu{selectedpath.textpathshow} toggle
item to toggle the \pathattr{textpathshow} attribute.
...
menu items as \gls{index.menu.selectedtext} that are specific to
\glspl{textarea}.

\menudef{menu.selected.bitmap}

The \menu{selected.bitmap} sub menu has the same
menu items as \gls{index.menu.selectedbitmap} that are specific to
\glspl{bitmap}.
...
in the viewport.
If the \gls{object} lies outside of the \gls{canvas}, the grid
\gls{origin} will be shifted to bring the \gls{object} into view.

If your system allows applications to move the
...
\widgetdef{findbydescription.image_objects}
...
description shown in the \widget{findbydescription.image_objects}
list. If an object doesn't have the description set, a
...
\begin{important}
The \btn{button.okay} can be activated with
\accelerator{button.okay} but not with
\accelerator{findbydescription.update} as that is used for the
\btn{findbydescription.update} button instead.
\end{important}
...
Once you have set the filter criteria, click on the
\widget{findbydescription.update} button to update the
list of matching objects.

\widgetdef{findbydescription.reset}
The \widget{findbydescription.reset} button will reset the filter
criteria back to its default settings. If you only want to reset the
\gls{object} type checkboxes, use the \btn{findbydescription.selectallitems}
button instead.

\widgetdef{findbydescription.selectallitems}
The \widget{findbydescription.selectallitems} button will 
select all the \gls{object} type checkboxes.

\widgetdef{findbydescription.deselectallitems}
The \widget{findbydescription.deselectallitems} button will 
deselect all the \gls{object} type checkboxes.
If you only want one selected, you may find it quicker to deselect
all and then select the one you want than to individually deselect the
unwanted ones.
...
\begin{information}
This section describes how to change the \gls{stackingorder}. If you want
to move an \gls{object} horizontally or vertically, see
\sectionref{sec:moveobjects}.
\end{information}
...
There are three methods of shifting \glspl{object} to align them
within a certain area.
The first is to justify \glspl{object} within a \gls{group} so that
they are aligned relative to the \gls*{group}['s] \gls{bbox}.
This can be done via the \menu{transform.justify} menu item.

The second method shifts all selected \glspl{object} to align them
relative to the page boundary. Note that this refers to the paper area which
may not necessarily coincide with the \gls{canvas} bounds if the
\gls{grid} has been shifted.
This can be done via the \menu{transform.align_to_page} menu item.

The third method is similar but only available if the
\gls{typeblock} has been set and is in the
\menu{tex.flowframe.align_to_typeblock}
sub-menu (see \sectionref{sec:aligntotypeblock}).

None of these methods recursively descend into sub groups so if you want
to shift several \glspl{object} while retaining their relative
positions to each other, you will need to group them first.
Bear in mind that a \gls*{shape}['s] \gls{line-style} contributes to the \gls{bbox}. 

\menudef{menu.transform.justify}

Objects within a \gls{group} can be aligned vertically or
horizontally using the \menu{transform.justify} sub menu.
Any selected \glspl{object} that are not groups will be ignored.
...
\menudef{menu.transform.align_to_page}

\Selected\ \glspl*{object} can be aligned vertically or
horizontally relative to the paper bounds using the
\menu{transform.align_to_page} sub menu.
Note that if the grid \gls{origin} has been shifted the \gls{canvas}
will not coincide with the paper bounds. The alignment is relative
to the paper not the \gls{canvas}.

If you have the \gls{typeblock} set and want to align to the
\gls{typeblock} instead of the paper bounds, use
\menu{tex.flowframe.align_to_typeblock} instead (see \sectionref{sec:aligntotypeblock}).

\menudef{menu.transform.align_to_page.left}

The \menu{transform.align_to_page.left} menu item will move all
selected objects so that the left edge of each object's \gls{bbox}
lies along the left edge of the paper (that is along the $y$-axis).

\menudef{menu.transform.align_to_page.centre}

The \menu{transform.align_to_page.centre} menu item will move all
selected objects so that the \manmsg{centre} of each object's \gls{bbox}
is horizontally \manmsg{centred} on the paper.

\menudef{menu.transform.align_to_page.right}

The \menu{transform.align_to_page.right} menu item will move all
selected objects so that the right edge of each object's \gls{bbox}
lies along the right edge of the paper.

\menudef{menu.transform.align_to_page.top}

The \menu{transform.align_to_page.left} menu item will move all
selected objects so that the top edge of each object's \gls{bbox}
lies along the top edge of the paper (that is along the $x$-axis).

\menudef{menu.transform.align_to_page.middle}

The \menu{transform.align_to_page.middle} menu item will move all
selected objects so that the \manmsg{centre} of each object's \gls{bbox}
is vertically \manmsg{centred} on the paper.

\menudef{menu.transform.align_to_page.bottom}

The \menu{transform.align_to_page.bottom} menu item will move all
selected objects so that the bottom edge of each object's \gls{bbox}
lies along the bottom edge of the paper.

\seealsorefs{
  sec:grouping,
  sec:texconfig,
  sec:aligntotypeblock,
  sec:neuronexample
}
...
The \menu{edit.cut} menu item is only available in \selectmode.
To cut a \selection\ of \glspl{object} to the
clipboard, use \menu{edit.cut} menu item. This copies the selection
as per \menu{edit.copy} and then removes the selection from the
\gls{canvas}.

\begin{information}
The \hyperref[sec:preamblepanels]{preamble panes} have
...
The \menu{edit.copy} menu item is only available in \selectmode.
To copy a \selection\ of \glspl{object} to the
clipboard, use \menu{edit.copy}. The selection is stored as binary
\gls{JDR} image data with the image tag set to the current filename
(which may be \widget{label.untitled}). If the content is then
pasted onto a \gls{canvas} with the same filename, the content will
be shifted (which makes it more obvious that the paste operation has
been successful), otherwise no shift will be applied (for example,
if a selection from one \gls{canvas} is pasted into another).

\begin{information}
If the selection is copied and pasted from and to the same
\gls{canvas} but the filename is changed in between the copy and
paste action, then no shift will be applied.
\end{information}

The selection will also be stored on the clipboard in \gls{svg}
format, which may be pasted into another graphics application that
recognises that format. Any text contained in the selection (such as
the displayed text for a \gls{textarea}) will also be stored as
plain text on the clipboard, which can be pasted into a context that
accepts plain text (such as a text editor). If you want to copy the
alternative \LaTeX\ text, you will need to use the
\hyperref[sec:edittext]{edit text area} function and copy the
applicable field (select the text, for example with
\accelerator{menu.textarea.select_all}, and copy with
\accelerator{menu.textarea.copy}).
...
The \menu{edit.paste} menu item is only available in \selectmode.
To paste a \selection\ of \glspl{object} from the clipboard, use
\menu{edit.paste}.

If the clipboard doesn't contain a \gls{JDR} image but does contain
plain text, then a new \gls{textarea} will be created for each
non-blank line of text. The text for each \gls{textarea} will be
trimmed. The pasted text will always start at the \gls{canvas}
origin, offset according to the font size. This is different from
using \gls{drag-and-drop} with the \texttool\ or \mathstool.

If you want to paste plain text into an existing
\gls*{textarea}, you will need to use the
\hyperref[sec:edittext]{edit text area} function.  

\begin{information}
The \hyperref[sec:preamblepanels]{preamble panes} have their own paste button.
\end{information}
...
An \inlineglsdef{objecttaglist} is a space-separated list of tags
that may be assigned to an object.

\menudef{menu.edit.object_tag}

You can assign a tag or \gls{taglist} to \selected\ \glspl*{object} using the
...
field as required. Each selected \gls{object} will then have its
tag list set to the combined list.
\end{information}
...
or \menu{navigate.by_description}.
...
You can designate a particular tag as a \gls{clippingtag} (see
\sectionref{sec:cliptag}). Any \gls{path} with this tag will be
exported as a clipping path instead of a normal path.
...
\widgetdef{paintselector.transparent}
...
\widgetdef{paintselector.single}

\FloatFig{fig:paint-solid}
{\includeimg{paint-solid}}
{Solid \Glsentrytext{manual.colour} Selector}
...
\inlineglsdef{index.colour.cmyk} (cyan, magenta, yellow, black) or
...
filled in. The final button in the swatch panel is actually the 
\btn{choose_colour} button which will open the
\inlineglsdef{choose_colour.title} dialog with
alternative ways of selecting a \manmsg{colour} (including
specifying the \manmsg{colour} as HSV or HSL).

\begin{information}
If you select a \manmsg{colour} using
either the \dialog{choose_colour} dialog or one of the swatch
buttons, the \manmsg{colour} model won't change. This means that if,
for example, you have the \widget{paintselector.grey} tab selected,
the selected \manmsg{colour} will be converted to
\dgls{colour.greyscale}.
\end{information}

\widgetdef{paintselector.gradient}
...
\inlineglsdef{index.gradient-paint.end-colour}, optionally
\manmsg{through} a \inlineglsdef{index.gradient-paint.mid-colour}.  There is a choice
...
\gls{index.gradient-paint.start-colour}) to another (the
\gls{index.gradient-paint.end-colour}). 

The \inlineglsdef{paintselector.start} and \inlineglsdef{paintselector.end} selectors
each have the same interface as the \widget{paintselector.single} panel.
The \inlineglsdef{paintselector.mid} checkbox will show or hide
a similar panel in which you can specify a
\dgls{gradient-paint.mid-colour}.
...
\widgetdef{paintselector.linear}
...
\widgetdef{paintselector.radial}
...
\inlineglsdef{index.colour.rgb} (red, green, blue),
\inlineglsdef{index.colour.cmyk} (cyan, magenta, yellow, black) and
\gls{index.colour.greyscale} \manmsg{colour} spaces.
Other ways of specifying a \manmsg{colour} are available with
the \dialog{choose_colour} dialog, which is opened with the \btn{choose_colour}
button, but the selected \manmsg{colour} will be converted
to a \manmsg{colour} space supported by \FlowframTk.
...
\section{Removing Translucency}\label{sec:removetranslucency}
...
alpha channel will be ignored if you \filefn{export} to \pgls{eps}.
\end{warning}
...
\chapter{Text}\label{sec:textareas}
...
document, click on the button marked \widget{edittext.latextext.different},
...
You can use the \widget{edittext.latextext.remap} button to generate
...
\widget{edittext.latextext.remap} button to generate a \mathsmode\
...
\widgetdef{edittext.textpath.title}
...
The matrix can also be changed in the \dialog{edittext} dialog.
...
Initially the underlying path will not be visible (except in \editpathmode).
...
The \inlineglsdef{index.path-attribute.textpathshow} attribute
determines whether or not the underlying path is \rendered.
The underlying path can be made visible by selecting the 
\menu{edit.path.textpathshow} toggle menu item. Deselecting this
checkbox will hide the underlying path again.

A \gls{textpath} has two sets of attributes: the text attributes to
govern the way that the text is drawn, which can be changed via the
\menu{edit.textarea} sub-menu, and the path attributes that
govern the way that the underlying path is drawn if the
\pathattr{textpathshow} setting is on, which can be changed via the 
\menu{edit.path} sub-menu.
...
If the \pathattr{textpathshow} setting is on, the text will still
follow the path defined by the \gls{underlying-shape} overlapping
the \rendered\ path. The text position is unaffected by
\dglspl{path-attribute} such as the \pathattr{line-width} or
\pathattrpl{marker}.

For example, \figureref{fig:textpathshowa}
has a path with a \pathattr{line-paint}, \pathattr{fill-paint}
and a \dgls{line-style} with a thick \pathattr{line-width} and a start marker.
The \gls{textarea} has the \textattr{outline} mode on with a 
\textattr{fill-paint} for the interior of each glyph.
In \figureref{fig:textpathshowb} the \gls{path} and \gls{textarea}
have been combined to form a \gls{textpath}. This is initialised
with the \pathattr{textpathshow} attribute off so the text follows
the \gls{underlying-shape} but the actual shape isn't \rendered.
In \figureref{fig:textpathshowc} the \pathattr{textpathshow}
attribute is on so the original path is now showing behind the text.

\FloatSubFigs{fig:textpathshow}
{
 {fig:textpathshowa}{\includeimg{textpathshowa}}{},
 {fig:textpathshowb}{\includeimg{textpathshowb}}{},
 {fig:textpathshowc}{\includeimg{textpathshowc}}{}
}
[Showing or Hiding the Underlying Shape of a Text-Path]
{Showing or Hiding the Underlying Shape of a Text-Path:
\subfigref{fig:textpathshowa}:  the original \gls{textarea} and \gls{path};
\subfigref{fig:textpathshowb}: the resulting \gls{textpath} with the
\pathattr{textpathshow} attribute off:
\subfigref{fig:textpathshowb}: the \gls{textpath} with the
\pathattr{textpathshow} attribute on}

The \gls{cag} functions, such as \transformfn{union}, are applied to
the underlying path and the text is adjusted to follow the new path.
Transformations using the \transformfn{rotate}, \transformfn{scale}
and \transformfn{shear} functions are applied to the underlying path
not the text. You can either transform the text using the
transformation functions before combining it with a path or
transform it after combining them into a \gls{textpath} by changing the text
\textattr{transformation-matrix}.
...
menu item. This means that instead of drawing the text normally,
each glyph is treated as a path that should be stroked.

For example, \figureref{fig:textoutlinea} shows a
standard \gls*{textarea} and \figureref{fig:textoutlineb} shows
that \gls*{textarea} rendered as an outline.

If the outline mode is set, you can apply a \gls{index.fill-colour} using
\menu{edit.textarea.fill_colour}. This fills in the paths defined by
the glyphs. Any \gls{textpath} that has the \pathattr{textpathshow}
attribute set will have a separate \pathattr{line-paint}, \pathattr{fill-paint}
and \dgls{line-style} for the underlying shape.
...
this and you will need to choose the appropriate \widget{export.textpathoutline}
setting in the \dialog{export} dialog.
The \widget{export.textareaoutline} setting can be used to adjust
the way that \gls{textarea} outlines are exported.
\end{warning}
...
The \affinetrans{rotation}, \affinetrans{scaling} and
\affinetrans{shearing} functions all have an anchor point.
This may be changed with the \inlineglsdef{anchor.title} settings:
\widgetdef{anchor.x}
The \widget{anchor.x} widget may be set to
\inlineglsdef{anchor.x.left} (the left edge of the \gls{bbox}),
\inlineglsdef{anchor.x.middle} (the horizontal middle of the \gls{bbox}), or
\inlineglsdef{anchor.x.right} (the right edge of the \gls{bbox}).

\widgetdef{anchor.y}
The \widget{anchor.y} widget may be set to
\inlineglsdef{anchor.y.top} (the top edge of the \gls{bbox}),
\inlineglsdef{anchor.y.middle} (the horizontal middle of the \gls{bbox}), or
\inlineglsdef{anchor.y.bottom} (the bottom edge of the \gls{bbox}).
...
\section{Moving an Object (Translation)}\label{sec:moveobjects}

To move an \gls{object} or \glspl*{object} horizontally or
vertically (\inlineglsdef{affine-transformation.translation}), first \select\ the
\glspl*{object} you want to move and then depress the
\glslink{primaryclick}{primary mouse button} somewhere inside 
one of the \glspl{bbox} in the selection, and drag the mouse. 
...
If you need to move by only a small amount or if you need to
move by an exact amount you can use the \menu{edit.moveby} menu item
instead of using the mouse to drag the \selected\ objects.
The \menu{edit.moveby} menu item will open the \dialog{moveby} dialog box.

\begin{information}
The \menu{edit.moveby} function is unaffected by the \gridlock.
\end{information}

\widgetdef{moveby.title}
The \dialog{moveby} dialog box (\figureref{fig:moveby}) provides a
way of specifying the horizontal and vertical displacement to move
\selected\ objects.
...
\FloatSubFigs{fig:moveby}
{
  {fig:movebya}{\includeimg{moveby}}{},
  {fig:movebyb}{\includeimg{movebychoice}}{}
}
[Move Selected Objects Dialog Box]
{Move Selected Objects Dialog Box:
\subfigref{fig:movebya} no typeblock;
\subfigref{fig:movebyb} typeblock set}

\widgetdef{moveby.calculate}

If you want to move by a proportion of the paper width or height,
you can use the \widget{moveby.calculate} area to calculate the
appropriate value. The percentage fields described below only allow
integer values.

If the \gls{typeblock} has been set, then you can also move by a
proportion of the \gls{typeblock} width or height.

\widgetdef{moveby.width}

If you want to move the selected \gls{object} (or \glspl{object})
horizontally by proportion of the paper width, enter the percentage
value in the \widget{moveby.width} number field. (You may
use a negative percentage.) For example, enter \code{50} for half
the paper width.

If the \gls{typeblock} has been set, a \gls{dropdown} will become
available. You can choose whether to calculate a percentage of the
paper width or a percentage of the \gls{typeblock} width.

\widgetdef{moveby.calc_x}

Once you have entered the desired percentage value, click on the
\widget{moveby.calc_x} button on the side. This will calculate
the percentage value of the paper or \gls{typeblock} width, as
applicable, and insert the value into
the \widget{coordinates.x} field.

\widgetdef{moveby.height}

If you want to move the selected \gls{object} (or \glspl{object})
vertically by proportion of the paper height, enter the percentage
value in the \widget{moveby.height} number field. (You may
use a negative percentage.) For example, enter \code{50} for half
the paper height.

If the \gls{typeblock} has been set, a \gls{dropdown} will become
available. You can choose whether to calculate a percentage of the
paper height or a percentage of the \gls{typeblock} height.

\widgetdef{moveby.calc_y}

Once you have entered the desired percentage value, click on the
\widget{moveby.calc_y} button on the side. This will calculate
the percentage value of the paper or \gls{typeblock} width, as
applicable, and insert the value into
the \widget{coordinates.y} field.
...
Notes:
\begin{itemize}
\item The \widget{anchor.title} is initialised to
\widget{anchor.x.middle}, \widget{anchor.y.middle}.

\item Individual \glspl{object}
will be rotated relative to the anchor of the \gls{object}'s \gls{bbox}

\item \Glspl*{object} within a
\gls{group} will be rotated relative to the anchor for the
group's \gls{bbox}.

\item A \gls*{shape}['s] \gls{line-style} contributes to the \gls{bbox}. 
...
In the \dialog{scale} dialog (see \figureref{fig:scaledialog}), you can specify the
\inlineglsdef{affine-transformation.scaling} factor.

\FloatFig{fig:scaledialog}
{\includeimg[scale=0.5]{scaledialog-keepaspect}}
{Scale Selection Dialog Box}

\widgetdef{scale.x}

Select the \widget{scale.x} radio button to scale just the
$x$ dimension. This will alter the aspect ratio.

\widgetdef{scale.y}

Select the \widget{scale.y} radio button to scale just the
$y$ dimension. This will alter the aspect ratio.
...
\widgetdef{scale.keep_aspect}
If the \widget{scale.keep_aspect} checkbox is selected (while the
\widget{scale.both} option is set), the scale
factor is the same for both dimensions and only one scale factor
field will be enabled. Otherwise the \inlineglsdef{scale.both.y}
number field next to the \widget{scale.keep_aspect} check box will
become available and you can set that to the required vertical
scaling. The \inlineglsdef{scale.both.x} field (next to the
\widget{scale.both} button) will be the horizontal scaling.
...
\item The lower \widget{scale.calculate} part of the \dialog{scale} dialog box
provides a way to calculate the scaling factors. This simply changes
the values in the upper part of the dialog. No scaling is actually
applied until the \btn{okay} button is clicked on.

\item The \widget{anchor.title} is initialised to
\widget{anchor.x.left}, \widget{anchor.y.top}.

\item Individual \glspl*{object} will be
scaled relative to the anchor of the object's \gls{bbox}.

\item \Glspl*{object} within a
\gls{group} will be scaled relative to the anchor
...
\widgetdef{scale.calculate}
The scaling factors can be calculated based on the dimensions of the
\selected\ object and either the paper size or, if set, the
\gls{typeblock} size. If more than one \gls{object} has been
selected, the \glsdisp{backobject}{backmost} one of the selection
will be used for the calculations.

\FloatSubFigs{fig:scaledialogcompute}
{
  {fig:scaledialog-notypeblock}{\includeimg[scale=0.5]{scaledialog-notypeblock}}{},
  {fig:scaledialog-typeblock}{\includeimg[scale=0.5]{scaledialog-typeblock}}{}
}
[Calculate Scale Factors]
{Calculate Scale Factors:
\subfigref{fig:scaledialog-notypeblock} no typeblock; \subfigref{fig:scaledialog-typeblock}
  typeblock set}

The object's type is also taken into account. If the object is a
\gls{shape} then the selected area is obtained by discounting the
line style, such as \pathattr{line-width} and \pathattrpl{marker}. 
For example, if the shape is a rectangle with
co-ordinates at (100\gls{bp},100\gls{bp}), (200\gls{bp},100\gls{bp})
(200\gls{bp},200\gls{bp}) and (100\gls{bp},200\gls{bp}) then the
selected area width and height are both 100\gls{bp}.
If the \gls{typeblock} width is 400\gls{bp} then the scale factor
required to make the rectangle's width equal to half the width of
the \gls{typeblock} is $(400 \times 0.5)/100 = 2$.

However, if the object is a group containing one or more
\glspl{shape}, then each shape's line style, if applicable, will be
included in the area. For example, if the rectangle is in the group
and the rectangle has 1\gls{bp} line width, then this extends the
rectangle bounds by 0.5\gls{bp} in each direction (left, right, top
and bottom), which adds 1\gls{bp} to the width and 1\gls{bp} to the
height. So even if all the other objects within the group are within
the rectangle bounds, the group's width and height will be slightly
larger than the actual rectangle co-ordinates which will change the
calculations.
For the above example, the scale factor
required to make the rectangle's width equal to half the width of
the \gls{typeblock} is now $(400 \times 0.5)/101 \approx 1.9802$.

The reason for this difference is that the calculation feature is mainly
provided to help quickly set up static, flow or dynamic frames (see
\sectionref{sec:flowframe}) with an area that is a percentage of the
paper or \gls{typeblock}. In this case, the selected object may
simply be a rectangle used as a guide for the frame and the border
should not be taken into account. An alternative is to use the
\menu{tex.flowframe.scale_to_typeblock} menu item (see \sectionref{sec:scaletotypeblock}).

\widgetdef{scale.width}
To calculate the $x$ scaling factor required to change the selected
object's width to a percentage of either the paper size or
\gls{typeblock}, enter the percentage value (an integer) into the
\widget{scale.width} field. If no \gls{typeblock} has been set
the percentage can only be of the paper width (see
\figureref{fig:scaledialog-notypeblock}). 
Otherwise, a \gls{dropdown} will be
available where you can choose either the paper width or
\gls{typeblock} width (see \figureref{fig:scaledialog-typeblock}).

\widgetdef{scale.calc_x}
Once you have entered the required percentage value in the
\widget{scale.width} field, click on the \btn{scale.calc_x} button
to calculate the $x$ scale factor.

If the \widget{scale.x} button is selected, the calculated value
will be placed in the \widget{scale.x} field. If the
\widget{scale.both} button is selected, the calculated value will be
placed in the field next to the \widget{scale.both} button.
If the \widget{scale.y} button is selected, the setting will
automatically be switched to the \widget{scale.x} setting.

\widgetdef{scale.height}
To calculate the $y$ scaling factor required to change the selected
object's height to a percentage of either the paper size or
\gls{typeblock}, enter the percentage value (an integer) into the
\widget{scale.height} field. If no \gls{typeblock} has been set the
percentage can only be of the paper height (see
\figureref{fig:scaledialog-notypeblock}).
Otherwise, a \gls{dropdown} will be available where you can choose
either the paper height or \gls{typeblock} height (see
\figureref{fig:scaledialog-typeblock}).

\widgetdef{scale.calc_y}
Once you have entered the required percentage value in the
\widget{scale.height} field, click on the \btn{scale.calc_y}
to calculate the $y$ scale factor.

If the \widget{scale.y} button is selected, the calculated value
will be placed in the \widget{scale.y} field.
If the \widget{scale.x} button is selected, the setting will
automatically be switched to the \widget{scale.y} setting.
If the \widget{scale.both} button is selected then
if the \widget{scale.keep_aspect} checkbox is selected, the value will be placed in the
field next to the \widget{scale.both} button, otherwise the calculated value will be
placed in the \widget{scale.both.y} field next to the \widget{scale.keep_aspect} checkbox.
...
\begin{itemize}
\item The \widget{anchor.title} is initialised to
\widget{anchor.x.left}, \widget{anchor.y.bottom}.

\item Individual \glspl*{object}
will be sheared relative to the anchor of the
object's \gls{bbox}.

\item \Glspl*{object} within a 
\gls{group} will be sheared relative to the anchor of the group's \gls*{bbox}.
...
The matrix for a \gls{textarea} or \gls{textpath} can also be reset
in the \dialog{edittext} dialog, and the matrix for a \gls{bitmap}
can also be reset in the \dialog{bitmap_properties} dialog.
...
(\figureref{fig:editPathPopup}).
The \menu{selectedpath.edit} menu item remains available to switch off path edit mode.
...
\gls{symmetry}. This sub menu is unavailable if there are any closed
sub-paths.
...
point). See \sectionref{sec:continuity} for further details.
...
\begin{warning}
Closed sub-paths are problematic with \glspl{compositeshape}
and, in particular, don't work with \glspl{symmetricshape} as they
cause the shape to lose its symmetry.
\end{warning}
...
or a \gls{Bezier-curve}). If the \gls{closing-move} is the final
segment of a closed path, use \menu{editpath.open_path.remove_last}
instead.
...
\dialog{segmentinfo} dialog box. See \sectionref{sec:segmentinfo}
for further details.
...
\menudef{menu.editpath.anchored_snap}

The \menu{editpath.anchored_snap} menu item will move the entire
shape so that the currently selected \gls{controlpt} is on the
nearest tick mark.
...
\subsection{Segment Details}\label{sec:segmentinfo}

\widgetdef{segmentinfo.title}

\FloatFig{fig:segmentinfo}
{\includeimg{segmentinfo}}
{Segment Details Dialog}

The \menu{editpath.info} menu item opens the \dialog{segmentinfo}
dialog (\figureref{fig:segmentinfo}), which describes the segment
type and provides numeric fields in which to adjust the co-ordinates
of the segment. Note that the end \gls{controlpt} won't be enabled
unless it's the end point of an open path or the end point of a
\gls{line-of-symmetry}.

You can also adjust the \glspl{controlpt} with the mouse in the
sample area. This behaves slightly differently to moving
\glspl{controlpt} on the \gls{canvas}: the \gridlock\ setting is
ignored, and you can only drag a \gls{controlpt} if you initiate the
drag from within the \gls{controlpt}['s] bounding box.

\begin{information}
The \dialog{segmentinfo} dialog allows you to focus in on a specific
segment without the \glspl{controlpt} from the other segments
getting in the way. The \gls{shape} shown in draft mode is just a
guide. Any constraints, such as \gls{line-of-symmetry} or
\gls{continuity} \glspl{anchor}, won't be applied to the
\gls{shape} until the segment changes are confirmed.
\end{information}
...
\subsection{Gradient Continuity}\label{sec:continuity}
...
\menu{editpath.break_path} \gls*{popupmenu} item in \editpathmode.
\end{information}
...
The direction of the \glspl{path} makes a difference, but sometimes
this difference can be subtle. The direction refers to the order of
the \glspl{controlpt} that define the path. The easiest way of
checking this is to add arrow \hyperref[sec:markers]{mid-markers}.

For example, both circles in \figureref{fig:mergedirection-samea}
have arrow mid-markers that show where the all the joining
(non-curvature) \glspl{controlpt} are, except for the start and end.
Each circle has the starting \gls{controlpt} on the right and they
continue down, round to the left, up and then back to the right
(that is, clockwise). \Figureref{fig:mergedirection-sameb} shows the
result of merging both \glspl{path}. The \pathattr{winding-rule} is currently
even-odd and there is a gap in the fill \manmsg{colour} where the
original two circles overlapped. The winding rule was then set to
non-zero and the result is shown in
\figureref{fig:mergedirection-samec}. The overlap area is now
filled.

\begin{information}
The merged path has extra mid-marker arrows. The horizontal one on
the right marks the move (\gls{gap}) from the end \gls{controlpt} of the original left
(backmost) circle to the start \gls{controlpt} of the original right
(frontmost) circle.
\end{information}

\FloatSubFigs{fig:mergedirection-same}
{
  {fig:mergedirection-samea}{\includeimg[scale=0.5]{mergedirection-samea}}{},
  {fig:mergedirection-sameb}{\includeimg[scale=0.5]{mergedirection-sameb}}{},
  {fig:mergedirection-samec}{\includeimg[scale=0.5]{mergedirection-samec}}{}
}
[Merging Two Clockwise Circles]
{Merging two clockwise circles:
\subfigref{fig:mergedirection-samea} two circles with mid-markers
showing their clockwise direction;
\subfigref{fig:mergedirection-sameb} the two circles from 
\subfigref{fig:mergedirection-samea} have been merged into a single
path (even-odd winding rule);
\subfigref{fig:mergedirection-samec} winding rule changed to
non-zero.}

The two circles in \figureref{fig:mergedirection-diff}, started with
the same two circles from \figureref{fig:mergedirection-samea}, but
the one on the left had its direction reversed with
\menu{transform.reverse}. The arrow mid-markers are now following
the anti-clockwise direction for the left circle.
\Figureref{fig:mergedirection-diffb} shows the
result of merging both \glspl{path}. The \pathattr{winding-rule} is currently
even-odd and there is a gap in the fill \manmsg{colour} where the
original two circles overlapped. The winding rule was then changed
to non-zero, but there is no visible difference. There is still a
gap in the overlap area.

\FloatSubFigs{fig:mergedirection-diff}
{
  {fig:mergedirection-diffa}{\includeimg[scale=0.5]{mergedirection-diffa}}{},
  {fig:mergedirection-diffb}{\includeimg[scale=0.5]{mergedirection-diffb}}{}
}
[Merging an Anti-Clockwise and a Clockwise Circle]
{Merging an anti-clockwise and a clockwise circle:
\subfigref{fig:mergedirection-diffa} two circles with mid-markers
showing their anti-clockwise and clockwise direction;
\subfigref{fig:mergedirection-diffb} the two circles from 
\subfigref{fig:mergedirection-diffa} have been merged into a single
path (same image for even-odd and non-zero winding rules)}
...
Path \gls{cag.intersection} can be used to clip a closed
\gls{shape}. For open shapes, you may prefer to use the
\gls{shapeclipping} function (see \sectionref{sec:clippingshapes}).
If you want to set up a clipping path, assign a \gls{clippingtag}
(see \sectionref{sec:cliptag}).
...
\section{Clipping Shapes}\label{sec:clippingshapes}

This section deals with altering the definition of a \gls{shape}
(either a \gls{path} or a \gls{compositeshape} or \gls{textarea}
that will be converted into a path).  If you want a
clipping path that will retain the original \glspl{object}, then set up a
\gls{clippingtag} instead (see \sectionref{sec:cliptag}).

The \gls{cag} functions such as \transformfn{intersect} work on
areas. This means that the resulting shape will be closed, even if
the original was open. If you want a closed shape to be clipped to
another closed shape, then the \transformfn{intersect} function is
the best method.

\begin{information}
The shape clipping function is still a little experimental.
\end{information}

The \inlineglsdef{shapeclipping} function alters \glspl{shape} by removing
segments outside of the desired area. This is intended as a more
convenient method than using the \menu{editpath.break_path} and
\menu{editpath.delete_control} functions to extract part
of an existing shape, which can then be edited.

\menudef{menu.transform.clip}

The \menu{transform.clip} menu item can clip selected \glspl{shape}
to lie within a rectangular clipping area. If a \gls{shape} lies
entirely within the clipping area then no change is made.
If a \gls{shape} lies entirely outside the clipping area, then the
shape will be removed.

If a \gls{shape} straddles the clipping area, then it will be
replaced by a new \gls{path} that has the same segments except for
those that lie outside the clipping area, which will be replaced by
a move (gap), and segments that cross the boundary will be split to
fit.
This splitting process doesn't always work correctly so you may need
to edit the path afterwards.

\begin{important}
As with the \gls{cag} functions, the original \glspl{shape} can't be
recovered (except via the \menu{edit.undo} function).
The \gls{line-style} is not taken into account by the clipping
function. The replacement path will be given the same
\gls{line-style} as the original.
\end{important}

Any \glspl{compositeshape} that need to be clipped will have the
clipping applied to the full path. The result will just be a
\gls{path} not a \gls{compositeshape}. Any selected objects that
can't be clipped will be ignored.

\widgetdef{clip.title}

The \menu{transform.clip} menu item opens the \dialog{clip} dialog.
The sample panel shows the selected paths in draft mode as well as the clipping
area, which has a similar appearance to the selection \gls{bbox}
but it has three hotspots: the right one will extend the width of
the clipping area to the right, the lower hotspot will extend the
height of the clipping area downwards, and the lower right hotspot
will extend both the width and the height. You can also click inside
the \gls{bbox} and drag it to reposition it. Alternatively, you can
enter the desired location and size in the length widgets below the
sample panel.

\widgetdef{clip.dotask}

Once you have positioned the clipping area, click on the
\btn{clip.dotask} button and the results will be calculated.
No change is made to the actual image at this point. If you are
happy with the results, click the \btn{okay} button otherwise you
can change the clipping area.

The original shapes are drawn in draft mode (their \gls{line-style}
attributes are ignored) to make it easier to compare their positions
with the results. The new shapes are drawn with the best matching
solid line or fill \manmsg{colour} (shading is ignore), 
but the line style attributes aren't applied in the sample area.
The full set of attributes will only be applied once the 
\btn{okay} button is pressed and the replacement shape is drawn on
the \gls{canvas}.
...
A \gls{textpath} has both \dglspl{text-attribute} (which govern the
way the text is styled) and \dglspl{path-attribute} (which govern
the way the underlying path is styled, if the
\pathattr{textpathshow} attribute is on).
...
The fill paint is used to paint the interior of a shape. The
interior (or \qt{inside}) of a shape is determined by the
\pathattr{winding-rule}. There are three types of fill paint:

\begin{itemize}
\item the paint used to fill the interior of a \gls{path} (or a path
that's the \gls{underlying-shape} for a \gls{compositeshape});

\item the paint used to fill the interior of glyphs for any text
with the \textattr{outline} setting on; 

\item the paint used to fill \pathattrpl{marker}. For markers, see
\sectionref{sec:markers}.

\end{itemize}

\menudef{menu.edit.path.fill_colour}

The \menu{edit.path.fill_colour} menu item opens a dialog with a
...
will need to be set to something other than
\widget{paintselector.transparent} in order for the \glspl{shape} to be
visible, unless there are independently painted \pathattrpl{marker}.)

There is a sample panel along the top (\menu{edit.path.fill_colour})
or the side (\menu{settings.styles}) illustrating the
effects of the \pathattr{fill-paint} setting that you choose.

\menudef{menu.edit.textarea.fill_colour}

The \menu{edit.textarea.fill_colour} menu item opens a dialog with a
\widget{fillpaintselector.title} panel in which you can change the
\inlineglsdef{index.text-attribute.fill-paint} for
\glspl{textarea} or \glspl{textpath} in \textattr{outline} mode. 
The sample panel at the top shows text using the style of the
currently selected \gls{textarea} or \gls{textpath}.

The \gls{menu.edit.textarea.fill_colour} menu item is only available if
the \menu{edit.textarea.outline} option is selected. 
...
However, be aware that marker caps are placed relative to the vertex
without taking the dash pattern into account.
\end{information}

\Figureref{fig:dashcapstyles} shows four lines with the same dash
pattern but different line caps. The gap is smaller with protruding
line caps. The fourth line has a butt cap with a \widget{arrow.rectanglecap} start
marker (shown in blue) and a \widget{arrow.roundcap} end marker (show in red). 
Note that the markers are placed at the vertex which may be in the
gap phase (as is the case for the end marker).

\FloatFig{fig:dashcapstyles}
{\includeimg{dashcapstyle}}
[Dash Pattern with Cap Styles]
{Dash pattern with cap styles: \subfigurefmt{a} butt, \subfigurefmt{b} round,
\subfigurefmt{c} square, \subfigurefmt{d} butt line cap with 
start and end marker caps.}
...
with thick lines (see \figureref{fig:capstyles}).
...
Markers can be specified by clicking on the \btn{choose}
...
When you click on the \btn{okay} button, the marker type
...
\begin{information}
In general, markers work best with the \pathattr{cap-style.butt} cap
style.
\end{information}
...
The \widget{arrow.indeptriangle2}, \widget{arrow.deptriangle2},
\widget{arrow.offsettriangle2} and \widget{arrow.stealth2}
markers are a new style of marker that have separate
\dgls{marker.length} and \dgls{marker.width} attributes (entered
into the \widget{arrow.length} and \widget{arrow.width} boxes). 

The \widget{arrow.indeptriangle2} marker is not
dependent on the \pathattr{line-width}, so if you change the line width you may
need to change the marker length and width attributes to compensate.
The \widget{arrow.deptriangle2} is dependent on the
\pathattr{line-width}. In this case, the actual marker dimensions
are the specified marker length plus half the line width for the
protrusion and the specified width plus the line width across the base.
In both cases, the base lies on the vertex and the marker protrudes forwards.

To illustrate the difference between the \widget{arrow.indeptriangle2} and
\widget{arrow.deptriangle2} markers, \figureref{fig:samplemarkertriangle2}
has four lines. The top two have a \widget{arrow.indeptriangle2} end marker
with the \widget{arrow.width} set to 5bp and the \widget{arrow.length}
set to 10bp. The bottom two have an \widget{arrow.deptriangle2} end marker
with the \widget{arrow.width} set to 5bp and the \widget{arrow.length}
set to 10bp. The two left hand lines have 1bp line width and the
two right hand lines have 10bp line width. (All lines have the cap
set to \widget{linestyle.cap.butt} and the marker \manmsg{colour}
set to red.)

The difference between the two markers is more noticeable with the
thicker line width. The \widget{arrow.indeptriangle2} marker (which
is independent of the line width) is now much narrower than the line
width. The  \widget{arrow.deptriangle2} marker (which does take the
line width into account) is still wider than the line width.

\FloatFig{fig:samplemarkertriangle2}
{\includeimg{samplemarkertriangle2}}
[\glsentrytext{arrow.indeptriangle2} and \glsentrytext{arrow.deptriangle2} Markers]
{\widget{arrow.indeptriangle2} and \widget{arrow.deptriangle2} markers:
the left two lines have 1bp line width,
the right two lines have 10bp line width,
the top two lines have a \widget{arrow.indeptriangle2} end marker
with 5bp width and 10bp length, and
the bottom two lines have a \widget{arrow.deptriangle2} end marker
with 5bp width and 10bp length.}

The \widget{arrow.offsettriangle2} marker is similar to the
\widget{arrow.deptriangle2} marker but is shifted back so that the
protuberance is minimised and is just enough to cover a butt 
\pathattr{cap-style}. To illustrate, \figureref{fig:offsetdeptriangle2markers}
has lines with 20bp line width and butt \pathattr{cap-style}.
The first line has a \widget{arrow.deptriangle2} end marker and the
second has an \widget{arrow.offsettriangle2} end marker. Both end
markers have width 15bp and length 20bp with the \manmsg{colour} set
to red with 60\% alpha, which allows the end of the line to show
through the marker. The \widget{arrow.deptriangle2} is flush
against the end of the line but the \widget{arrow.offsettriangle2}
is further back. If the \pathattr{cap-style} were to be changed to
either rectangle or round, it would protrude into the marker.

The third line in \figureref{fig:offsetdeptriangle2markers}
illustrates the \widget{arrow.stealth2} marker, which is similar to
the \widget{arrow.offsettriangle2} but has a \qt{stealth arrow}
shape. The indent is 40\% of the total length. Note that if the line
width is much thicker than the length, the offset may be further
forward than the vertex, which will cause a gap to appear between
the end of the line and the rear indentation of the arrow head.

\FloatFig{fig:offsetdeptriangle2markers}
{\includeimg{offsetdeptriangle2markers}}
[\glsentrytext{arrow.deptriangle2},
\glsentrytext{arrow.offsettriangle2} and
\glsentrytext{arrow.stealth2} Markers]
{\widget{arrow.deptriangle2},
\widget{arrow.offsettriangle2}, and
\widget{arrow.stealth2} markers:
\subfigurefmt{a} \widget{arrow.deptriangle2};
\subfigurefmt{b} \widget{arrow.offsettriangle2}
\subfigurefmt{c} \widget{arrow.stealth2}.}

\begin{information}
There are other triangle markers in other categories but
the ones in \widget{arrow.tab.arrows} are designed as \qt{pointing} style
markers.
\end{information}
...
The \dgls{marker.size} governs the protrusion. The marker width is set to
the \pathattr{line-width}. This means that the cap markers will be
narrow on thin \glspl{path}. Increasing the marker size will simply make
the cap markers long and thin.
...
Unlike the line \pathattr{cap-style}, the marker caps are always
positioned with their base on the vertex (although they may be
shifted due to the offset or secondary settings). This means they
don't take the dash phase into account if a \pathattr{dash-pattern}
is applied (see \figureref{fig:dashcapstyles} in
\sectionref{sec:dashpattern}).

\FloatTable[htbp]{tab:arrowmarkers}
{%
\begin{tabular}{@{}lcccc@{}}
\bfseries Marker & \bfseries Dimensions? &
\bfseries \tablecellstack{Line Width\\Dependent?} &
\bfseries \tablecellstack{\Manmsg{centred} on\\Vertex?} &
\bfseries Sample\\
\inlineglsdef{arrow.pointed} & \widget{arrow.size} & Dependent & No &
\includeimg{markerpointed}\\
\inlineglsdef{arrow.pointed60} & \widget{arrow.size} & Dependent & No &
\includeimg{markerpointed60}\\
\inlineglsdef{arrow.pointed45} & \widget{arrow.size} & Dependent & No &
\includeimg{markerpointed45}\\
\inlineglsdef{arrow.cusp} & \widget{arrow.size} & Dependent & No &
\includeimg{markercusp}\\
\inlineglsdef{arrow.latex} & N/A & Dependent & No &
\includeimg{markerlatex}\\
\inlineglsdef{arrow.altlatex} & N/A & Dependent & No &
\includeimg{markeraltlatex}\\
\inlineglsdef{arrow.altlatex_open} & N/A & Dependent & No &
\includeimg{markeraltlatexopen}\\
\inlineglsdef{arrow.triangle} & \widget{arrow.size} & Dependent & No &
\includeimg{markertriangle}\\
\inlineglsdef{arrow.triangle_open} & \widget{arrow.size} & Dependent & No &
\includeimg{markertriangleopen}\\
\inlineglsdef{arrow.equilateral_filled} & \widget{arrow.size} & Independent
& No & \includeimg{markerequilateralfilled}\\
\inlineglsdef{arrow.equilateral_open} & \widget{arrow.size} & Independent
& No & \includeimg{markerequilateralopen}\\
\fnsym{1}\inlineglsdef{arrow.indeptriangle2}
  & \widget{arrow.length} \& \widget{arrow.width} & Independent
 & No & \includeimg{markerindeptriangle2}\\
\fnsym{1}\inlineglsdef{arrow.deptriangle2}
  & \widget{arrow.length} \& \widget{arrow.width} & Dependent
 & No & \includeimg{markerdeptriangle2}\\
\fnsym{1}\inlineglsdef{arrow.offsettriangle2}
  & \widget{arrow.length} \& \widget{arrow.width} & Dependent
 & No & \includeimg{markeroffsettriangle2}\\
\fnsym{1}\inlineglsdef{arrow.stealth2}
  & \widget{arrow.length} \& \widget{arrow.width} & Dependent
 & No & \includeimg{markerstealth2}\\
\inlineglsdef{arrow.hooks} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhooks}
\end{tabular}
\tablefns{\fnsymtext{1}{Sample has marker length 12bp and marker width 8bp}}
}
...
\bfseries \tablecellstack{\Manmsg{centred} on\\Vertex?} &
\bfseries Sample\\
\inlineglsdef{arrow.hook_up} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhookup}\\
\inlineglsdef{arrow.hook_down} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhookdown}\\
\inlineglsdef{arrow.halfpointed_up} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfpointedup}\\
\inlineglsdef{arrow.halfpointed_down} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfpointeddown}\\
\inlineglsdef{arrow.halfpointed60_up} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfpointed60up}\\
\inlineglsdef{arrow.halfpointed60_down} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfpointed60down}\\
\inlineglsdef{arrow.halfpointed45_up} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfpointed45up}\\
\inlineglsdef{arrow.halfpointed45_down} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfpointed45down}\\
\inlineglsdef{arrow.halfcusp_up} & \widget{arrow.size} & Dependent & No &
\includeimg{markerhalfcuspup}\\
\inlineglsdef{arrow.halfcusp_down} & \widget{arrow.size} & Dependent & No &
...
{%
\inlineglsdef{arrow.dotfilled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerdotfilled}\\
\inlineglsdef{arrow.dotopen} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerdotopen}\\
\inlineglsdef{arrow.boxfilled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerboxfilled}\\
\inlineglsdef{arrow.boxopen} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerboxopen}\\
\inlineglsdef{arrow.cross} & \widget{arrow.size} & Independent & Yes &
\includeimg{markercross}\\
\inlineglsdef{arrow.plus} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerplus}\\
\inlineglsdef{arrow.star} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerstar}\\
\inlineglsdef{arrow.asterisk} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerasterisk}\\
\inlineglsdef{arrow.star5_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerstar5open}\\
\inlineglsdef{arrow.star5_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerstar5filled}\\
\inlineglsdef{arrow.star6_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerstar6open}\\
\inlineglsdef{arrow.star6_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerstar6filled}\\
\inlineglsdef{arrow.triangle_up_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markertriangleupfilled}\\
\inlineglsdef{arrow.triangle_up_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markertriangleupopen}\\
\inlineglsdef{arrow.triangle_down_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markertriangledownfilled}\\
\inlineglsdef{arrow.triangle_down_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markertriangledownopen}\\
\inlineglsdef{arrow.rhombus_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerrhombusfilled}\\
\inlineglsdef{arrow.rhombus_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerrhombusopen}\\
\inlineglsdef{arrow.pentagon_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerpentagonfilled}\\
\inlineglsdef{arrow.pentagon_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerpentagonopen}\\
\inlineglsdef{arrow.hexagon_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerhexagonfilled}\\
\inlineglsdef{arrow.hexagon_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerhexagonopen}\\
\inlineglsdef{arrow.octagon_filled} & \widget{arrow.size} & Independent & Yes &
\includeimg{markeroctagonfilled}\\
\inlineglsdef{arrow.octagon_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markeroctagonopen}\\
\inlineglsdef{arrow.semicircle_filled} & \widget{arrow.size} & Independent & No &
\includeimg{markersemicirclefilled}\\
\inlineglsdef{arrow.semicircle_open} & \widget{arrow.size} & Independent & No &
...
\bfseries \tablecellstack{\Manmsg{centred} on\\Vertex?} &
\bfseries Sample\\
\inlineglsdef{arrow.square} & N/A & Dependent & No&
\includeimg{markersquare}\\
\inlineglsdef{arrow.bar} & N/A & Dependent & Yes &
\includeimg{markerbar}\\
\inlineglsdef{arrow.round} & N/A & Dependent & No &
\includeimg{markerround}\\
\inlineglsdef{arrow.brace} & N/A & Dependent & No &
\includeimg{markerbrace}\\
\inlineglsdef{arrow.altsquare} & \widget{arrow.size} & Dependent & No&
\includeimg{markeraltsquare}\\
\inlineglsdef{arrow.altbar} & \widget{arrow.size} & Dependent & Yes &
\includeimg{markeraltbar}\\
\inlineglsdef{arrow.altround} & \widget{arrow.size} & Dependent & No &
\includeimg{markeraltround}\\
\inlineglsdef{arrow.altbrace} & \widget{arrow.size} & Dependent & No &
...
{%
\inlineglsdef{arrow.rectanglecap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerrectanglecap}\\
\inlineglsdef{arrow.roundcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerroundcap}\\
\inlineglsdef{arrow.trianglecap} & \widget{arrow.size} & Dependent & No &
\includeimg{markertrianglecap}\\
\inlineglsdef{arrow.inverttrianglecap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerinverttrianglecap}\\
\inlineglsdef{arrow.chevroncap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerchevroncap}\\
\inlineglsdef{arrow.invertchevroncap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerinvertchevroncap}\\
\inlineglsdef{arrow.fastcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerfastcap}\\
\inlineglsdef{arrow.invertfastcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerinvertfastcap}\\
\inlineglsdef{arrow.ballcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerballcap}\\
\inlineglsdef{arrow.leafcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleafcap}\\
\inlineglsdef{arrow.doubleleafcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleaf2cap}\\
\inlineglsdef{arrow.tripleleafcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleaf3cap}\\
\inlineglsdef{arrow.clubcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerclubcap}\\
\inlineglsdef{arrow.tripleleafforwardcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleaf3forcap}\\
\inlineglsdef{arrow.tripleleafbackcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleaf3backcap}\\
\inlineglsdef{arrow.doubleleafforwardcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleaf2forcap}\\
\inlineglsdef{arrow.doubleleafbackcap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerleaf2backcap}\\
\inlineglsdef{arrow.bulgecap} & \widget{arrow.size} & Dependent & No &
\includeimg{markerbulgecap}\\
\inlineglsdef{arrow.cutoutbulgecap} & \widget{arrow.size} & Dependent & No &
...
\bfseries \tablecellstack{\Manmsg{centred} on\\Vertex?} &
\bfseries Sample\\
\inlineglsdef{arrow.circle} & \widget{arrow.size} & Dependent & No &
\includeimg{markercircle}\\
\inlineglsdef{arrow.diamond} & N/A & Dependent & No &
\includeimg{markerdiamond}\\
\inlineglsdef{arrow.circle_open} & \widget{arrow.size} & Dependent & No &
\includeimg{markercircleopen}\\
\inlineglsdef{arrow.diamond_open} & N/A & Dependent & No &
\includeimg{markerdiamondopen}\\
\inlineglsdef{arrow.scissors_up_filled} & \widget{arrow.size} & Dependent & No &
\includeimg{markerscissorsupfilled}\\
\inlineglsdef{arrow.scissors_down_filled} & \widget{arrow.size} & Dependent & No &
\includeimg{markerscissorsdownfilled}\\
\inlineglsdef{arrow.scissors_up_open} & \widget{arrow.size} & Dependent & No &
\includeimg{markerscissorsupopen}\\
\inlineglsdef{arrow.scissors_down_open} & \widget{arrow.size} & Dependent & No &
\includeimg{markerscissorsdownopen}\\
\inlineglsdef{arrow.heart_right_filled} & \widget{arrow.size} & Independent & No &
\includeimg{markerheartrightfilled}\\
\inlineglsdef{arrow.heart_right_open} & \widget{arrow.size} & Independent & No &
\includeimg{markerheartrightopen}\\
\inlineglsdef{arrow.heart_filled} & \widget{arrow.size} & Independent & No &
\includeimg{markerheartfilled}\\
\inlineglsdef{arrow.heart_open} & \widget{arrow.size} & Independent & No &
\includeimg{markerheartopen}\\
\inlineglsdef{arrow.snowflake} & \widget{arrow.size} & Independent & Yes &
\includeimg{markersnowflake}\\
\inlineglsdef{arrow.starchevron_open} & \widget{arrow.size} & Independent & Yes &
\includeimg{markerstarchevronopen}\\
\inlineglsdef{arrow.starchevron_filled} & \widget{arrow.size} & Independent & Yes &
...
resizeable markers that don't have a separate width and length. 
This box can be used to vary the 
...
Resizeable markers that have a separate width and length attribute
will instead use the \widget{arrow.width} and \widget{arrow.length}
boxes.

\widgetdef{arrow.width}

The marker \inlineglsdef{index.marker.width} is given by the value
in the \widget{arrow.width} box. If an independent width isn't
supported, the marker width will either depend on the line width or
on the \widget{arrow.size} setting.

\widgetdef{arrow.length}

The marker \inlineglsdef{index.marker.length} is given by the value in the
\widget{arrow.length} box. This is how far the marker protrudes from its
base.
...
[Marker \Glsentrytext{manual.colour} May Be Independent of the Line
\Glsentrytext{manual.colour}]
...
The selected font may not support the chosen attributes.
For example, if \widget{font.shape} is set to 
\widget{font.shape.sc} the text on the \gls{canvas} won't be
\rendered\ in small-caps, but changing the \widget{font.shape}
will also change the associated \LaTeX\ command (\gls{upshape} for
small-caps) so the text may be rendered as small-caps if the image
is exported to a \LaTeX\ file. If it still doesn't show as
small-caps in the \LaTeX\ document, check the log file for font
messages.
...
\widgetdef{font.shape.em}

The \widget{font.shape.em} item indicates emphasized and
will automatically set the \LaTeX\ alternative to \gls{em}.
This setting may appear as italic or slanted when the text is
\rendered\ on the \gls{canvas} depending on the
font.

\widgetdef{font.shape.italic}

The \widget{font.shape.italic} item indicates italic and
will automatically set the \LaTeX\ alternative to \gls{itshape}.
This setting may appear as slanted rather than italic when the text is
\rendered\ on the \gls{canvas} depending on the
font.

\widgetdef{font.shape.slanted}

The \widget{font.shape.slanted} item indicates slanted (oblique) and
will automatically set the \LaTeX\ alternative to \gls{slshape}.
This setting may appear as italic when the text is
\rendered\ on the \gls{canvas} depending on the
font.

\widgetdef{font.shape.sc}

The \widget{font.shape.slanted} item indicates small-caps and
will automatically set the \LaTeX\ alternative to \gls{scshape}.
...
\chapter{\TeX/\LaTeX}\label{sec:latexfunctions}
...
\section{Adding Commands to the Preamble and Providing Document Text}
\label{sec:preamblepanels}
...
(The document title is obtained from the \gls{imagedescription}, see
...
The \widget{texeditor.latexcodeblock} panel has five panels.
...
\menudef{menu.texeditor.settings}
The \menu{texeditor.settings} function opens the \dialog{texeditorui}
dialog. This has the same layout as the \dialog{texeditorui} panel
of the \dialog{configui} (see \sectionref{sec:texeditorui}).
...
you are using plain \TeX\ you would need to do:
\begin{codebox*}
\gls{input} myshapepar
This is the start of the paragraph...
\end{codebox*}
or if you are using \LaTeX\ you would need to do:
\begin{codebox}
\gls{input}\marg{myshapepar}\comment{}
This is the start of the paragraph...
\end{codebox}
Remember to include the \sty{shapepar} package:
\begin{itemize}
\item \code{\gls{input} shapepar.sty} (plain \TeX)
\item \code{\gls{usepackage}\marg{shapepar}} (\LaTeX)
\end{itemize}
...
\texdocref{flowfram}
A brief summary of \sty{flowfram} is provided in \sectionref{sec:flowframesummary}.

Before assigning \gls{flowframe} data to any of the \glspl{object}
on the \gls{canvas}, you must first setup the \gls{typeblock}.
This can either be done with the \dialog{typeblock} dialog, which can be
opened with the \menu{tex.flowframe.set_typeblock} menu item
(described in \sectionref{sec:typeblock}), or you can use the 
\dialog{flfwizard.typeblock} page of the \dialog{flfwizard} dialog
(described in \sectionref{sec:flfwizard}).

\begin{warning}
If you assign \gls{flowframe} data to a \gls{group}, the data will be lost
if you later \hyperref[sec:grouping]{ungroup} it.
\end{warning}

Once the \gls{typeblock} has been set, you can then assign
\gls{flowframe} data to an \gls{object}. This can also be done
with the \dialog{flfwizard} dialog. Alternatively, select the
applicable \gls{object} and use the \menu{tex.flowframe.set_frame}
menu item to open the \dialog{flowframe} dialog (described in
\sectionref{sec:framedef}).

If you need to adjust the \gls{flowframe} data for an \gls{object}
that has already been assigned \gls{flowframe} data, select the
\gls{object} and use the \dialog{flowframe} dialog. It is also
possible to use the \dialog{flfwizard} dialog but if you only need
to make a minor adjustment, it's quicker to use the
\dialog{flowframe} dialog.
...
\FloatFig{fig:flowframe-layers}
{%
  \includeimg{flowframe-layers1}
  \includeimg{flowframe-layers2}
}
{Static, Flow and Dynamic Layers}
...
can be a comma separated list of pages or page ranges (for example,
...
Each type of frame has its own layer (see
\figureref{fig:flowframe-layers}).  The \sty{flowfram} package
stacks the frames on the page in the following order: static, flow
and dynamic, each in the order in which they were defined. For
example, if you define a flow frame \manmsg{labelled} \optfmt{left},
then define a static frame \manmsg{labelled} \optfmt{title}, then a
dynamic frame \manmsg{labelled} \optfmt{header} and lastly a flow
frame \manmsg{labelled} \optfmt{right}, then the \sty{flowfram}
package will stack the frames in the following order:
\optfmt{title}, \optfmt{left}, \optfmt{right} and \optfmt{header}.
This means that the static frames can be used for background
effects.

\FlowframTk\ doesn't have layers.
...
In order to export to a package or class file, you must first define
the \gls{flowframe} data, but the \gls{typeblock} must be defined
first. The \sty{flowfram} package defines each frame position
relative to the bottom left corner of the \gls{typeblock}. This is
because of the way that the output routine works. So when
\FlowframTk\ exports \gls{flowframe} data it needs the paper
\gls{paper.size} and orientation and the \gls{typeblock} not only to
set the page geometry but also to convert its own \gls{canvas}
co-ordinates into co-ordinates relative to the \gls{typeblock}.
So the \menu{tex.flowframe.set_frame} menu item only becomes
available once the \gls{typeblock} has been set.

\FlowframTk\ now provides two ways of defining the \gls{flowframe}
data: you can first set the \gls{typeblock} with 
\menu{tex.flowframe.set_typeblock} and then, for each \gls{object}
that needs to represent a static, flow or dynamic frame, select the
\gls*{object} and use \menu{tex.flowframe.set_frame} to set (or
edit) the \gls{flowframe} data.

The second, newer method, is to use the \dialog{flfwizard} dialog.
This guides you through the process step-by-step (see
\sectionref{sec:flfwizard}).
...
You can use the \menu{edit.undo} menu item to undo this action.
...
\subsection{The Flow Frame Wizard}\label{sec:flfwizard}

\menudef{menu.tex.flowframe.wizard}

The \menu{tex.flowframe.wizard} menu item opens the \dialog{flfwizard} dialog,
which guides you step-by-step through the process of setting up 
\gls{flowframe} data, in preparation to export to a \LaTeX\ document
class or package (see \sectionref{sec:exportsty}). This is only
available with the \tool{select}.

\widgetdef{flfwizard.title}

If the \gls{typeblock} hasn't yet been set, the \dialog{flfwizard} will
start at the beginning (the \dialog{flfwizard.cls} page) otherwise
it will skip ahead and start with the \dialog{flfwizard.frametype}
page. You can go back to the previous page or forward to the next
page using the \btn{flfwizard.previous} and
\btn{flfwizard.next} buttons at the bottom of the
\dialog{flfwizard} dialog. You can cancel at any time by clicking on
the \btn{cancel} button.

\widgetdef{flfwizard.previous}
The \btn{flfwizard.previous} button will have its text set to the
title of the previous page (panel). You can use this button to go back. The
button is not available on the first page.

\widgetdef{flfwizard.next}
The \btn{flfwizard.next} button will have its text set to the
title of the next page (panel). You can use this button to go forwards. On
the final page it will change to \widget{flfwizard.finish.title}.

\subsubsection{Wizard: Class and Normal Size}\label{sec:flfwizardclass}

\widgetdef{flfwizard.cls.title}

\FloatFig{fig:flfwizard-class}
[\setkeys{Gin}{width=0.5\linewidth}]
{\includeimg{flfwizard-class}}
{Flow Frame Wizard: Class and Normal Size}

The first page of the \dialog{flfwizard} dialog
(\figureref{fig:flfwizard-class}) reminds you of your current
settings.  The underlying class if you export to a document class
should be specified with either \widget{flfwizard.cls.default_cls}
(\cls{article}, \cls{extarticle} or \cls{a0poster}, depending on the
normal font size) or \widget{flfwizard.cls.custom_cls}
(you provide the class name, for example \cls{book} or
\cls{scrbook} or \cls{memoir}).
This setting is also used if you export to a multi-page PDF.

The default class font size should be set with
\widget{flfwizard.cls.normalsize}. This setting is needed not only
to set the default if you export to a class file but also to know
the default \gls{baselineskip} if it's needed for any calculations
(such as those required for the \widget{typeblock.adjust_height}
button in the \dialog{flfwizard.typeblock} panel or the
\widget{flowframe.shape_parshape} or
\widget{flowframe.shape_shapepar} settings).

These and the \widget{flfwizard.cls.relative_fontsize} and
\widget{flfwizard.cls.update} widgets are the same as the
corresponding settings in the \dialog{clssettings} tab of the
\dialog{texconfig} dialog (described in
\sectionref{sec:configuredialog}).

The \widget{flfwizard.labelpages.relative} and
\widget{flfwizard.labelpages.absolute} settings are the same as the
corresponding \widget{flfsettings.pages_opt} widgets in the
\dialog{flfsettings} tab of the \dialog{texconfig} dialog (see
\sectionref{sec:texconfigflf}).  If the
\widget{flfwizard.labelpages.absolute} button is selected, all page
lists should be given in terms of the absolute page number.  If the
\widget{flfwizard.labelpages.relative} button is selected, all page
lists should be given in terms of the numeric (not formatted) value
of the \ctr{page} counter.

If any of these settings are incorrect, you can change them at this
point.  Any changes you make in the \dialog{flfwizard.cls} panel
will be applied when you move onto the next panel.

Additionally, there's a reminder of the current \gls{grid} unit,
the storage unit, and the paper size. If any of these are incorrect, you will need to
cancel the \dialog{flfwizard} with the \btn{cancel} button and
change them. (Ideally the storage unit should be set before you
start creating your image to reduce rounding errors.)

The \gls{grid} setting can be changed with 
\menu{settings.grid.settings} or you can double-click on the grid unit
in the top left corner of the \gls{canvas} between the
\glspl{ruler} (see \figureref{fig:griddialog} in \sectionref{sec:gridmenu}).

The \hyperref[sec:controlsettings]{storage unit} can be set in the
\dialog{controls} tab of the \dialog{config} dialog (see
\sectionref{sec:controlsettings}) or you can double-click on the
storage unit in the \gls{statusbar} (see \figureref{fig:griddialog}
in \sectionref{sec:gridmenu}).  When exporting data, most dimensions
will be written in terms of the storage unit, regardless of the unit
you specify in the various settings.

The paper setting can be changed with the \menu{settings.paper}
menu (see \sectionref{sec:papermenu}). It's important to ensure that
the paper size is correct before setting the \gls{typeblock}.
You may have to reset the \gls{typeblock} if you change the paper
size afterwards.

Once you have confirmed all your class settings, click on the
\btn{flfwizard.next} button to apply your changes and move to the
\dialog{flfwizard.typeblock} panel.

\subsubsection{Wizard: Typeblock}\label{sec:flfwizardtypeblock}

\widgetdef{flfwizard.typeblock.title}

\FloatFig{fig:flfwizard-typeblock}
[\setkeys{Gin}{width=0.5\linewidth}]
{\includeimg{flfwizard-typeblock}}
{Flow Frame Wizard: Typeblock}

The \dialog{flfwizard.typeblock} panel is the second page of the
\dialog{flfwizard} dialog and has the same interface as the
\dialog{typeblock} dialog (see \sectionref{sec:typeblock}).

You can either set the document page margins using the 
\widget{flowframe.typeblock_margins} panel (the default unit is
obtained from the current \gls{grid}) or you can select one or
more items in the \widget{typeblock.select_object} list
and click on the \widget{typeblock.compute_from_objects} button.

\begin{information}
The \gls{typeblock} margins, shown as an outlined \manmsg{grey}
rectangle on the \gls{canvas}, are not connected to the 
\glspl{printer-margin} (shown as a thick \manmsg{grey} border around
the edge of the page area of the \gls{canvas} if the
\menu{settings.paper.margins} menu item is selected).
\end{information}

The \widget{flowframe.margins.width} and
\widget{flowframe.margins.height} fields show the \gls{typeblock}
width and height based on the dimensions supplied in the
\widget{flowframe.typeblock_margins} panel and the current
paper size. These values are non-editable. They will be updated if the supplied
margins change but they may not still have those values if you
change the paper size before you use the export function.

Each item in the \widget{typeblock.select_object} list represents an \gls{object} on the
\gls{canvas}. A single mouse click will just select an item in the
list. A double-click will also select the corresponding \gls{object}
on the \gls{canvas}, which can be useful to confirm that you have
selected the correct item in the list. You can select more than one
item in the list by holding down \keys{\keyref{shift}} (block select)
or \keys{\keyref{ctrl}} (add to selection or deselect if already
selected).

The \widget{typeblock.compute_from_objects} button will calculate
the minimum bounds that encompass all selected \glspl{object} and
will update the \widget{flowframe.typeblock_margins} panel.

If you want a two-sided document, you can enter the horizontal shift
for even pages in the \widget{flowframe.even_x_shift} field.
The \gls{evensidemargin} will be calculated as this
amount added to the \gls{oddsidemargin}.
If you want a symmetric layout, click on the \widget{typeblock.compute_sym_shift}
button to calculate the appropriate shift.

You may use the \widget{typeblock.adjust_width} button to adjust the
right margin so that the typeblock width is an integer
multiple of the supplied dimension but remember that this depends on the current
paper width. Likewise you can use the \widget{typeblock.adjust_height}
button to adjust the bottom margin but again the
calculation uses the current paper height. Rounding errors may occur
when converting between different units.

\begin{important}
The \widget{typeblock.adjust_width} and \widget{typeblock.adjust_height}
buttons don't round the margin values. They round the width or
height and then adjust the right or bottom margin to fit.
\end{important}

The \btn{flowframe.margins.xbalance} button may be used to average
the left and right margins so that they are equal but the total remains the same.
The \btn{flowframe.margins.ybalance} button may be used to average
the top and bottom margins so that they are equal but the total remains the same.

Any changes you make in the \dialog{flfwizard.typeblock} panel will
be applied when you click on the \btn{flfwizard.next} button to move
onto the \dialog{flfwizard.frametype} panel.

\subsubsection{Wizard: Frame Type}\label{sec:flfwizardframetype}

\widgetdef{flfwizard.frametype.title}

\FloatFig{fig:flfwizard-frametype}
[\setkeys{Gin}{width=0.5\linewidth}]
{\includeimg{flfwizard-frametype}}
{Flow Frame Wizard: Frame Type}

The \dialog{flfwizard.frametype} panel is the third page of the
\dialog{flfwizard} dialog. This allows you to setup the
\gls{flowframe} data for an \gls{object}. From this point onwards,
the changes you make won't be applied until you click on the
\widget{flfwizard.finish.title} button on the final page.

You must first select the applicable \gls{object} from the
\widget{flfwizard.frametype.object} list. A single mouse click will
select an item from the list. A double-click will both select an
item from the list and also select the corresponding \gls{object} on
the \gls{canvas}, which allows you to check that you have selected
the correct \gls{object}.

If you already had one or more \glspl{object} selected on the
\gls{canvas}, the list item that corresponds to the
\glsdisp{backobject}{backmost} of the selection will be
automatically selected.

Once an item has been selected, the frame type buttons will be
enabled. There are three types of frame: static, flow or dynamic.
The \inlineglsdef{flfwizard.frametype.flow} button is the default, but
if you want the selected object to represent a static frame, 
ensure that the \inlineglsdef{flfwizard.frametype.static} button is
selected.

If you want the selected object to represent a dynamic frame,
you can select the \inlineglsdef{flfwizard.frametype.dynamic} button.
However, there are some special dynamic frames that are identified
by a special label. These are the header, footer and thumbtab
frames. If you can't remember the special label, you can click on
the appropriate special button instead. These are:

\widgetdef{flfwizard.frametype.header}
Select the \widget{flfwizard.frametype.header} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.header} label. 
If the document is two-sided and there is an even header frame, this
frame will only be used for odd pages. If there is no even header
frame it will be used for all pages.

The \widget{flfwizard.frametype.header} button will be disabled if
there is already an object with the special \framelabelref{header} label.
If you are editing existing \gls{flowframe} data with this label, the 
\widget{flfwizard.frametype.dynamic} button will be selected
instead. The \widget{flfwizard.frametype.header} button will not be
required in this case, as the label has already been set.

\widgetdef{flfwizard.frametype.evenheader}
Select the \widget{flfwizard.frametype.evenheader} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.evenheader} label. 
If the document is two-sided this frame will be used instead of the
\framelabelref{header} frame. Using this button will not only fill in the
frame's label in the next panel but will also set the page list to
\qt{Even}.

The \widget{flfwizard.frametype.evenheader} button will be disabled if
there is already an object with the special \framelabelref{evenheader} label.
If you are editing existing \gls{flowframe} data with this label, the 
\widget{flfwizard.frametype.dynamic} button will be selected
instead. The \widget{flfwizard.frametype.evenheader} button will not be
required in this case, as the label has already been set.

\widgetdef{flfwizard.frametype.footer}
Select the \widget{flfwizard.frametype.footer} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.footer} label. If the
document is two-sided and there is an even footer frame, this frame
will only be used for odd pages. If there is no even footer frame it
will be used for all pages.

The \widget{flfwizard.frametype.footer} button will be disabled if
there is already an object with the special \framelabelref{footer} label.
If you are editing existing \gls{flowframe} data with this label, the 
\widget{flfwizard.frametype.dynamic} button will be selected
instead. The \widget{flfwizard.frametype.footer} button will not be
required in this case, as the label has already been set.

\widgetdef{flfwizard.frametype.evenfooter}
Select the \widget{flfwizard.frametype.evenfooter} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.evenfooter} label. If
the document is two-sided this frame will be used instead of the
\framelabelref{footer} frame. Using this button will not only fill
in the frame's label in the next panel but will also set the page
list to \qt{Even}.

The \widget{flfwizard.frametype.evenfooter} button will be disabled if
there is already an object with the special \framelabelref{evenfooter} label.
If you are editing existing \gls{flowframe} data with this label, the 
\widget{flfwizard.frametype.dynamic} button will be selected
instead. The \widget{flfwizard.frametype.evenfooter} button will not be
required in this case, as the label has already been set.

\widgetdef{flfwizard.frametype.thumbtab}
Select the \widget{flfwizard.frametype.thumbtab} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.thumbtabn} label. The value of \meta{n}
will automatically be calculated.

If the document is two-sided and there is an
\framelabelref{eventhumbtabn} frame, this frame will only be used for
odd pages. If there is no corresponding even frame, this frame will
be used for all pages.

\widgetdef{flfwizard.frametype.eventhumbtab}
Select the \widget{flfwizard.frametype.eventhumbtab} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.eventhumbtabn} label.
The value of \meta{n} will automatically be calculated.  If the
document is two-sided, this frame will be used instead of
\framelabelref{thumbtabn} on even pages. Using this button will not
only fill in the frame's label in the next panel but will also set
the page list to \qt{Even}.

\begin{information}
Unless you have a non-symmetric shape as the thumbtab frame, there's
usually no need for an even thumbtab frame. The even page offset can
be calculated automatically.
\end{information}

\widgetdef{flfwizard.frametype.thumbtabindex}
Select the \widget{flfwizard.frametype.thumbtabindex} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.thumbtabindexn}
label. The value of \meta{n} will automatically be calculated.

\begin{information}
Unless you want a different \manmsg{colour} or shape for the
thumbtab index frames, you don't need to create a thumbtab index.
\FlowframTk\ will automatically create the \framelabelref{thumbtabindexn} frames
based on the corresponding \framelabelref{thumbtabn} frame.
\end{information}

\widgetdef{flfwizard.frametype.eventhumbtabindex}
Select the \widget{flfwizard.frametype.eventhumbtabindex} button for a dynamic frame
with the special \inlineglsdef{opt.frame-label.eventhumbtabindexn}
label. The value of \meta{n} will automatically be calculated.

\begin{information}
Unless you have a non-symmetric shape as the thumbtab index frame, there's
usually no need for an even thumbtab index frame. The even page offset can
be calculated automatically.
\end{information}

Once you have selected the appropriate frame type, click on the
\btn{flfwizard.next} button to continue to the
\dialog{flfwizard.labelpages} panel.

\subsubsection{Wizard: Label and Pages}\label{sec:flfwizardlabelpages}

\widgetdef{flfwizard.labelpages.title}

\FloatFig{fig:flfwizard-label+pages}
[\setkeys{Gin}{width=0.5\linewidth}]
{\includeimg{flfwizard-label+pages}}
{Flow Frame Wizard: Label and Pages}

The \dialog{flfwizard.labelpages} panel
(\figureref{fig:flfwizard-label+pages}) confirms the selected frame
type in the \widget{flfwizard.labelpages.frame_type} field (Static,
Flow or Dynamic). This is non-editable. You will have to go back to
the previous page to change it, if it is incorrect.

\widgetdef{flfwizard.labelpages.frame_label}

The \widget{flfwizard.labelpages.frame_label} field will
automatically be filled in for special frames or if the
\gls{flowframe} data was already set and you are just modifying it.
Otherwise, you will need to fill it in. The label must be unique for
the given frame type and should follow the usual rule for \LaTeX\
labels, so avoid \TeX\ special characters and avoid commas.

\widgetdef{flfwizard.labelpages.pagelist}

The frame's page list may be a keyword, which can be obtained from
the \gls{dropdown} list, or a number, list or range
(for example, \qtt{1,4,7-12,14}). The number is either the absolute
page (if the \widget{flfwizard.labelpages.absolute} setting is on)
or the numeric value of the \ctr{page} counter
(if the \widget{flfwizard.labelpages.relative} setting is on).

Click on the \btn{flfwizard.next} button to move onto the
\dialog{flfwizard.style} panel.

\subsubsection{Wizard: Border, Style and Content}\label{sec:flfwizardstyle}

\widgetdef{flfwizard.style.title}

\FloatSubFigs{fig:flfwizard-style}
[\setkeys{Gin}{width=0.4\linewidth}]
{
  {fig:flfwizard-style-flow}{\includegraphics{flfwizard-style-flow}}{},
  {fig:flfwizard-style-static}{\includegraphics{flfwizard-style-static}}{},
  {fig:flfwizard-style-dynamic}{\includegraphics{flfwizard-style-dynamic}}{},
  {fig:flfwizard-style-special}{\includegraphics{flfwizard-style-special}}{}
}
[Flow Frame Wizard: Border and Style]
{Flow Frame Wizard border and style panel:
\subfigref{fig:flfwizard-style-flow} flow;
\subfigref{fig:flfwizard-style-static} static;
\subfigref{fig:flfwizard-style-dynamic} dynamic;
\subfigref{fig:flfwizard-style-special} special.}

The \dialog{flfwizard.style} panel provides a way of setting up the
frame border, style and content. However, not all settings are
available as they are particular to different types of frame
(see \figureref{fig:flfwizard-style}).

\widgetdef{flfwizard.style.border}

The object you have selected to use for the \gls{flowframe} data may
just be a simple rectangle that marks out the location and
dimensions of the frame. In which case, the \widget{flfwizard.style.border}
checkbox should be unselected.

If the object (such as a \gls{shape} or \gls{bitmap}) should be used
as a border or background for the frame, make sure that the
\widget{flfwizard.style.border} checkbox is selected.

\begin{information}
All \glspl{textarea} are considered to be a part of the
...
When assigning \gls{flowframe} data (rather than editing
existing data) the \widget{flfwizard.style.border} setting will
default to off if the current object is an unfilled \gls{path}.
Otherwise it will default to on. Change it as applicable.

\widgetdef{flfwizard.style.cmds}
The \widget{flfwizard.style.cmds} setting is only available for
dynamic frames. The \sty{flowfram} package only allows a control
sequence name (such as \qtt{textbf} or \qtt{bfseries}) for the style
attribute.  \FlowframTk\ allows either a control sequence name or
one or more declarations. These can be entered into the
\widget{flfwizard.style.cmds} field (which will be trimmed).

If the provided value contains a backslash, \FlowframTk\ will add
code to the exported file that defines a command with the provided
value as the definition. The name of that command can then be used
for the style attribute (which \FlowframTk\ will do automatically). 
If the provided value doesn't contain a
backslash, \FlowframTk\ will assume it's the control sequence name
(unless the value is empty, in which case no style is set).

The final command may be a text-block command (such as \gls{textbf})
but in general declarations (such as \gls{bfseries}) work better.

\widgetdef{flfwizard.style.marginpos}
The \widget{flfwizard.style.marginpos} selector is only available for flow frames.
This identifies the location of the side area used by
\gls{marginpar}: \widget{flowframe.marginpar.right} indicates to the
right of the flow frame, \widget{flowframe.marginpar.left} indicates to the
left of the flow frame, \widget{flowframe.marginpar.inner} indicates to the
right of the flow frame for even pages and to the left of the flow
frame for odd pages, \widget{flowframe.marginpar.outer} indicates to
the left of the flow frame for even pages and to the right of the
flow frame for odd pages. 

\widgetdef{flfwizard.style.clear}

The \widget{flfwizard.style.clear} checkbox is only available for static and dynamic frames.
If selected, the frame content will be automatically cleared after
each page.

\begin{information}
This setting should not be used for the special frames
which require special content. If cleared, the frame would lose its
special function. Therefore the \widget{flfwizard.style.clear}
checkbox is not visible for a dynamic frame with a special label.
\end{information}

\widgetdef{flfwizard.style.valign}
The \widget{flfwizard.style.valign} selector is only available for static and dynamic frames.
This corresponds to the \optfmt{valign} frame attribute.

\widgetdef{flfwizard.style.shape}
The \widget{flfwizard.style.shape} selector is only available for static and dynamic frames
that are assigned to a \gls{shape} (see \sectionref{sec:frameshape}). 
It will not be visible for flow frames and will be visible but disabled for
static or dynamic frames that aren't a \gls{shape} (such as a
\gls{group} or \gls{bitmap}).

\widgetdef{flfwizard.content.edit}
The \widget{flfwizard.content.edit} button is only available for
static and dynamic frames and will open \FlowframTk's \TeX\ editor
window (described in \sectionref{sec:texeditor}) in which you can
type the frame content. This button is not available for flow frames
(their content is the document text) or for dynamic frames with a
special label (which require special content to function correctly).

Once set, the content will be shown in the area below the 
\btn{flfwizard.content.edit} button. Click on the button again if
you need to change it.
The content will be shown on the \gls{canvas} if the
\widget{annotations.frame_contents.show} setting is on
(see \sectionref{sec:annotations}), but you can only change
the contents by clicking on the \widget{flowframe.edit}
button in either the \dialog{flfwizard} or in the
\dialog{flowframe} dialog.

Use the \btn{flfwizard.next} button to move onto the
\dialog{flfwizard.dimensions} panel.

\subsubsection{Wizard: Padding and Offsets}\label{sec:flfwizarddimensions}

\widgetdef{flfwizard.dimensions.title}

\FloatFig{fig:flfwizard-dimensions}
[\setkeys{Gin}{width=0.5\linewidth}]
{\includegraphics{flfwizard-dimensions}}
{Flow Frame Wizard: Padding and Offsets}

The final page is the \dialog{flfwizard.dimensions} panel (see
\figureref{fig:flfwizard-dimensions}).

If the frame has the border set, the \widget{flowframe.frame_margins}
may be used to insert padding between the object's \gls{bbox}
and the area for the frame's content. If the object is a simple
rectangle with a thin line width, this may be quite small, but a
thick line width or a shape that deviates significantly from its
\gls{bbox} may need more padding to prevent the frame's text from
overlapping the shape's outline. The padding isn't required if the
frame doesn't have a border.

The actual frame dimensions are given by the area within the inner
padding. The frame width is calculated as the object's \gls{bbox}
width less the left and right padding. The frame height is calculated
as the object's \gls{bbox} height less the top and bottom padding.
The frame's $x$ and $y$ position is at the bottom left of the inner
padding.

The \widget{flowframe.frame_margins} panel can be shown or hidden by
selecting or deselecting the \widget{flowframe.margins.none}
checkbox. It will default to deselected if the 
\widget{flfwizard.style.border} was not selected.
The \widget{flowframe.frame_margins} panel is not available if the
frame has been assigned a non-standard
\widget{flfwizard.style.shape}.
If the \widget{flowframe.frame_margins} panel is hidden, all the
margins will be set to zero.

The width and height of the area defined by the inner padding is
calculated and shown below the inner padding fields. Since this
setting is designed for frames with a border, the calculations take
any \gls{line-style} into account. For example, if the selected
object is a rectangle with control points at
(100\gls{bp},100\gls{bp}), (200\gls{bp},100\gls{bp})
(200\gls{bp},200\gls{bp}) and (100\gls{bp},200\gls{bp}) but the line
width is 1\gls{bp} (and there are no \pathattrpl{marker}) then with
zero inner padding, the width and height will both be 101\gls{bp}
not 100\gls{bp}.


\widgetdef{flfwizard.dimensions.even_x_shift}

For two-sided documents, the \widget{flfwizard.dimensions.even_x_shift}
determines the frame's horizontal position on even pages.
\FlowframTk\ uses this value to calculate the \optfmt{evenx} attribute.

\begin{information}
The frame's location is relative to the bottom left corner of the
\gls{typeblock}. If the \gls{typeblock} has a non-zero
shift for even pages, given in \widget{flowframe.even_x_shift}
on the \dialog{flfwizard.typeblock} panel, then the frame's location
will move with it on even pages. Use the  
\widget{flfwizard.dimensions.even_x_shift} field if the frame needs
to be in a different relative location on even pages.
For example, a frame that needs to be on the right of odd pages but
on the left of even pages.
\end{information}

\widgetdef{flfwizard.dimensions.compute_sym_x_shift}

If you want to calculate the shift required to provide a symmetric
layout, use the \widget{flfwizard.dimensions.compute_sym_x_shift}
button. Note that this doesn't make the object symmetric.
If you require a different shape for even pages, you will need to
create two different objects and set the 
\widget{flfwizard.labelpages.pagelist} for one to 
\widget{displaypage.odd} and the other to 
\widget{displaypage.even}.

\widgetdef{flfwizard.dimensions.even_y_shift}
It's less likely to have a frame with a different vertical position
on odd and even pages, but this effect can be achieved by setting
the \widget{flfwizard.dimensions.even_y_shift} dimension.

\widgetdef{flfwizard.finish.title}
Click on \widget{flfwizard.finish.title} to make your frame selections have an
effect. You will then be asked if you want to start over
with the next \gls{object}.  Click on \widgetfmt{Yes} to go back to the
\dialog{flfwizard.frametype} page.  The next \gls{object} will be
automatically selected in the \widget{flfwizard.frametype.object} list, 
but you can change the selection to another
one. Click on \widgetfmt{No} to close the \dialog{flfwizard} dialog.
...
The \inlineglsdef{typeblock} is the main area of the page where the
text goes.  The dimensions of the \gls{typeblock} are given by the
lengths \gls{typeblockwidth} and \gls{typeblockheight}
(which are defined and initialised by the \sty{flowfram} package to
\gls{textwidth} and \gls{textheight}). The \gls{typeblock} provides
a frame of reference and, unless the special header and footer
dynamic frames are created, the page header and footer will be
placed above and below the \gls{typeblock} according to \LaTeX's
standard page layout design.

\begin{information}
Static and dynamic frames may be positioned outside of the
\gls{typeblock}, but it's best to ensure that all flow frames are
within the \gls{typeblock} bounds.
\end{information}
...
The \gls{typeblock} can be set with the new \dialog{flfwizard}, but it
can also be set with the older \dialog{typeblock} dialog box, which can be
opened with the \menu{tex.flowframe.set_typeblock} menu item.

\widgetdef{typeblock.title}

In the \dialog{typeblock} dialog you can enter the margins between
the paper edge and the \gls{typeblock}.
The \gls{typeblock} width and height are calculated from these
values and the current paper size.

\widgetdef{flowframe.typeblock_margins}
You can type the margin lengths into the supplied fields
in the \widget{flowframe.typeblock_margins} area. 
The \gls{typeblock} width and height, based on the current paper size,
are shown below this area and will be updated whenever the fields in
the \widget{flowframe.typeblock_margins} area are changed.

If you later change the paper (via the \menu{settings.paper} menu)
the margins will remain the same but the \gls{typeblock} width and
height will change accordingly.

\widgetdef{typeblock.compute_from_objects}
If you prefer, you can get \FlowframTk\ to
calculate the margins from the bounds of one or more \glspl{object}.
Ensure that you have selected the applicable object or objects
from the \widget{typeblock.select_object} list and click on the
\widget{typeblock.compute_from_objects} button to update the
\widget{flowframe.typeblock_margins} fields. This button will be
disabled if there are no selected items in the \widget{typeblock.select_object} list.

\widgetdef{typeblock.select_object}

If one or more \gls{object} was already selected when you opened the
\dialog{typeblock} dialog, the row or rows corresponding to the
\gls*{canvas} \selection\ will automatically be selected in the
\widget{typeblock.select_object} list. If more than one row in the
list is selected, the \widget{typeblock.compute_from_objects} button
will calculate the margins based on the bounds that minimally
encompass all the selected items in the list.

If you didn't already have an object selected or you want to use a
different one, you can change the list selection.  A single mouse
\gls{click} will select an item in the list.  A \gls{doubleclick}
will both select an item in the list and select the corresponding
\gls{object} on the \gls{canvas}, which allows you to check that
you have selected the correct object.  If you hold down the
\keys{\keyref{shift}} key while clicking on an item, it will select
all items in a block from the currently selected item.  If you hold
down the \keys{\keyref{ctrl}} key while clicking on an item, it will
add the item to the current selection.
The textual representation of each \gls{object} in the list is the
same as for the \dialog{findbydescription} dialog box. 
...
\widget{typeblock.compute_from_objects} button was clicked
which then filled in the values for the margins.

\FloatFig{fig:typeblocka}
{\includeimg{typeblocka}}
{A Rectangle Drawn on a Tschichold Grid}
...
\widget{flowframe.margins.top} fields.

Additionally, you might want to adjust the right margin so that the
\gls{typeblock} width is rounded to, say, the nearest whole mm or
whole or half \gls{pc} or adjust the bottom margin so that the
\gls{typeblock} height is rounded to, say, the nearest baselineskip.

\widgetdef{typeblock.adjust_width}
To automatically round the \gls{typeblock} width, type in the
required rounding value in the \widget{typeblock.adjust_width_label}
field (for example 1\gls{pc}) and click on the
\widget{typeblock.adjust_width} button. For example, I changed the
left margin to 66\gls{pt} and the top margin to 94\gls{pt}. The
\widget{typeblock.adjust_width_label} was already set to 1\gls{pc}, so I
left it at that and clicked on the \widget{typeblock.adjust_width}
button. This filled in the right margin as shown in
\figureref{fig:typeblockc}. Note that this has changed the unit for
the right margin, but all the margins (and most other dimensions)
will be converted to the storage unit when exporting.

The width is calculated based on the current paper size and the
value is shown below the margin panel.

\begin{important}
The \widget{typeblock.adjust_width} and \widget{typeblock.adjust_height}
buttons don't round the margin values. They round the width or
height and then adjust the right or bottom margin to fit. Make sure you set the
paper size first.
\end{important}

\widgetdef{typeblock.adjust_height}
...
\begin{warning}
Rounding errors may occur that are caused by the conversion between
units. If you later change the \gls{normal-font-size}
(\sectionref{sec:normalsize}),
you may need to readjust the \gls{typeblock} height.
\end{warning}

The adjustment actions only change either the right or bottom
margin, which may be appropriate for an off-\manmsg{centred}
symmetric even and odd layout but not if you're interested in a
one-sided \manmsg{centred} layout.

\widgetdef{flowframe.margins.xbalance}
The \widget{flowframe.margins.xbalance} button will average the left
and right margins so that the total remains the same but will then
be the same on either side. If the left and right margin fields have
different units, the left will be adjusted to match the right.

\widgetdef{flowframe.margins.ybalance}
The \widget{flowframe.margins.ybalance} button will average the top
and bottom margins so that the total remains the same but will then
be the same both above and below. If the top and bottom margin fields have
different units, the top will be adjusted to match the bottom.

\widgetdef{typeblock.hshift}
...
\begin{compactcodebox}
\cmd{setlength}\gls{evensidemargin}\marg{\gls{oddsidemargin}}
\cmd{addtolength}\gls{evensidemargin}\margm{h-shift}
\end{compactcodebox}%
...
\gls{typeblock} may not be displayed correctly when you display even
pages in \FlowframTk. Remember that \gls{evensidemargin} is
ignored if you use the \opt{oneside} class option.

\widgetdef{typeblock.compute_sym_shift}
...
[The typeblock is shown as a light \glsentrytext{manual.grey} rectangle]
...
If you are unfamiliar with setting \gls{flowframe} data, you may
prefer to use the \dialog{flfwizard}. The \dialog{flowframe} dialog
is faster as you don't have to progress through multiple steps but
...
\begin{warning}
Make sure you set the paper size and \gls+{typeblock} before you set any frames.
...
You can change the font or position of the annotation in the
\dialog{annotations} tab of the \dialog{configui} dialog
(see \sectionref{sec:annotations}).
...
There are some dynamic frame labels that have a special meaning for
\FlowframTk: \framelabelref{header},
\framelabelref{footer},
\framelabelref{evenheader} and
\framelabelref{evenfooter}. These indicate that the frame should be converted
...
class file, \FlowframTk\ will use commands provided by
\sty{flowframtkutils} to convert these frames in dynamic header and
footers. Make sure that you have at least version 2.0 of
\sty{flowfram} installed to ensure that \sty{flowframtkutils} is
available.

\begin{important}
The special labels only have a special significance for dynamic frames.
\end{important}
...
There are also special labels for thumbtabs:
\framelabelref{thumbtabn},
\framelabelref{eventhumbtabn},
\framelabelref{thumbtabindexn}, and
\framelabelref{eventhumbtabindexn}.
These form a complete set for the \meta{n}th thumbtab. Unless you
want a different style for even or index thumbtabs, it's usually
sufficient to just define \framelabelref{thumbtabn}
and the index thumbtab will be based on that.

\begin{important}
Unlike the \dialog{flfwizard} dialog, the \dialog{flowframe} dialog
doesn't hide dynamic frame settings that aren't applicable to
special frames. Don't set the contents of these frames and don't
switch on the \widget{flowframe.clear} setting as it will interfere
with the special frame function.
\end{important}
...
When assigning \gls{flowframe} data (rather than editing existing
data) the \widget{flowframe.border} setting will default to
\widget{flowframe.border_none} if the current object is an unfilled
\gls{path}.  Otherwise it will default to
\widget{flowframe.border_as_shown}. Change it as applicable.
...
\widgetdef{flowframe.frame_margins}

The frame may have inner padding between the border and the area in which
the contents are typeset. The padding is usually only relevant
if you have specified the \widget{flowframe.border_as_shown}
border option. If the \widget{flowframe.margins.none} checkbox
is selected, the inner padding fields are hidden. Deselect
\widget{flowframe.margins.none} to show them.
This setting is not available for non-standard
paragraph shapes (\sectionref{sec:frameshape}).
There's no need to use \sty{flowfram}['s] \optfmt{offset} attribute
as \FlowframTk\ calculates the correct position.
...
\widgetdef{flowframe.style_cmds}
The \widget{flowframe.style_cmds} setting is only available for
dynamic frames. The \sty{flowfram} package only allows a control
sequence name (such as \qtt{textbf} or \qtt{bfseries}) for the style
attribute.  \FlowframTk\ allows either a control sequence name or
one or more declarations. These can be entered into the
\widget{flowframe.style_cmds} field (which will be trimmed).
The final command may be a text-block command (such as \gls{textbf})
but in general declarations (such as \gls{bfseries}) work better.

If \FlowframTk\ detects one or more backslash in the \widget{flowframe.style_cmds}
field it will create a command (via \sty{flowframtkutils}) in the
exported file whose name can be used for the
\optfmt{style} attribute. Otherwise it will assume the supplied
value (if not empty) is a control sequence name.
...
or environments provided by the \sty{flowfram} package (but not for
the special dynamic frames). For your
...
The contents will be shown on the \gls{canvas} if the
\widget{annotations.frame_contents.show} setting is on
(see \sectionref{sec:annotations}), but you can only change
the contents by clicking on the \widget{flowframe.edit}
button in either the \dialog{flfwizard} or in the
...
\begin{important}
You can't use verbatim text in the contents of dynamic frames.
\end{important}
...
\glspl*{object}. Remember that the \gls{line-style} contributes to a
\gls*{shape}['s] \gls{bbox}.

\subsection{Align Objects to Typeblock}\label{sec:aligntotypeblock}

\menudef{menu.tex.flowframe.align_to_typeblock}

The \menu{tex.flowframe.align_to_typeblock} sub-menu is similar to
the \menu{transform.align_to_page} sub-menu, but in this case the
\selected\ object or objects are aligned to the \gls{typeblock}.

Remember that the \gls{line-style} contributes to a
\gls*{shape}['s] \gls{bbox}. So, for example, if you are aligning a
path with a non-transparent \pathattr{line-paint} then the outer
edge of the path's outline will be aligned, not the outermost
\gls{controlpt}.

This method does not recursively descend into sub groups so if you want
to shift several \glspl{object} while retaining their relative
positions to each other, you will need to group them first.

\menudef{menu.tex.flowframe.align_to_typeblock.left}

The \menu{tex.flowframe.align_to_typeblock.left} menu item aligns
the selected \glspl{object} to the left of the \gls{typeblock}. If
the selection includes \glspl{textarea}, the horizontal
\textattr{anchor} will automatically be updated if the
\widget{textconfig.anchor} setting is on.

\menudef{menu.tex.flowframe.align_to_typeblock.centre}

The \menu{tex.flowframe.align_to_typeblock.centre} menu item
moves all selected objects so that they are
\manmsg{centred} horizontally within the \gls{typeblock}. If
the selection includes \glspl{textarea}, the horizontal
\textattr{anchor} will automatically be updated if the
\widget{textconfig.anchor} setting is on.

\menudef{menu.tex.flowframe.align_to_typeblock.right}

The \menu{tex.flowframe.align_to_typeblock.right} menu item aligns
the selected \glspl{object} to the right of the \gls{typeblock}. If
the selection includes \glspl{textarea}, the horizontal
\textattr{anchor} will automatically be updated if the
\widget{textconfig.anchor} setting is on.

\menudef{menu.tex.flowframe.align_to_typeblock.top}

The \menu{tex.flowframe.align_to_typeblock.top} menu item aligns
the selected \glspl{object} to the top of the \gls{typeblock}. If
the selection includes \glspl{textarea}, the vertical
\textattr{anchor} will automatically be updated if the
\widget{textconfig.anchor} setting is on.

\menudef{menu.tex.flowframe.align_to_typeblock.middle}

The \menu{tex.flowframe.align_to_typeblock.middle} menu item
moves all selected objects so that they are
\manmsg{centred} vertically within the \gls{typeblock}. If
the selection includes \glspl{textarea}, the vertical
\textattr{anchor} will automatically be updated if the
\widget{textconfig.anchor} setting is on.

\menudef{menu.tex.flowframe.align_to_typeblock.bottom}

The \menu{tex.flowframe.align_to_typeblock.bottom} menu item aligns
the selected \glspl{object} to the bottom of the \gls{typeblock}. If
the selection includes \glspl{textarea}, the vertical
\textattr{anchor} will automatically be updated if the
\widget{textconfig.anchor} setting is on.
...
It is possible to filter out \glspl{object} with \gls{flowframe}
data from the \gls{canvas} rendering based on their page list. The
\menu{tex.flowframe.display_page} menu item will open the
\dialog{displaypage} dialog box.

\widgetdef{displaypage.title}

In this context, a \qt{frame} is an \gls{object} that has \gls{flowframe} data
assigned to it.

\begin{information}
\Glspl{object} that have not been assigned \gls{flowframe} data will
always be displayed.  The title bar will indicate how many objects
have been hidden.
\end{information}
...
\figureref{fig:housesavetex}. Remember to use the
\sty{pgf} package:
\begin{codebox}
\cmd{usepackage}\marg{pgf}
\end{codebox}
Alternatively use \sty{flowframtkutils} which automatically loads
the \sty{pgf} package but also supplies some additional commands:
\begin{codebox}
\cmd{usepackage}\marg{flowframtkutils}
\end{codebox}
The image can be included in the document with \gls{input}. For
example:
\begin{codebox}
\cbeg{figure}
\cmd{centering}
\gls{input}\marg{house}
\cmd{caption}\marg{A House}
\cend{figure}
\end{codebox}
Alternatively, you can use \gls{includeteximage} provided by
\sty{flowframtkutils}:
\begin{codebox}
\cbeg{figure}
\cmd{centering}
\gls{includeteximage}\marg{house}
\cmd{caption}\marg{A House}
\cend{figure}
\end{codebox}
If the image turns out to be too large for your document, you can
use the optional argument to scale it. For example:
\begin{codebox}
\gls{includeteximage}\oarg{scale=0.75}\marg{house}
\end{codebox}
...
you can get brown from 40\% Red, 8\% Green and 0\% Blue.
Alternatively, click on the closest \manmsg{colour} button
or use the \btn{choose_colour} button to open the
\dialog{choose_colour} dialog. Create the
rectangle shown in \figureref{fig:lettuce1}: switch to the
\tool{rectangle}, \gls{click} at the co-ordinate 
(100\gls{bp},150\gls{bp}),
move the mouse to co-ordinate (250\gls{bp},300\gls{bp}) 
and \gls{click} to complete.
...
Select the top segment (by clicking on the segment's start
\gls{controlpt} or cycle round with \accelerator{editpath.next_control} until
the segment is selected). The segment will then be highlighted in red
(\figureref{fig:lettuce2b}).

\begin{information}
If you created the rectangle by starting at a different corner, the
\glspl{controlpt} will be in a different order.
\end{information}
...
to change the curvature (\figureref{fig:lettuce4a}).
I've moved them to (220\gls{bp},90\gls{bp}) and
(130\gls{bp},90\gls{bp})

\begin{information}
Click on a \gls{controlpt} to select it. Make sure that it has
changed \manmsg{colour} to indicate that it has been selected
before you start dragging or you will end up moving the wrong \gls{controlpt}.
\end{information}

If you have the \gridlock\ enabled, you may find it easier to
temporarily disable it while you are editing the curvature control
points or use the \dialog{segmentinfo} dialog, which can be opened with
\menu{editpath.info}.

Deselect the edit path tool (\menu{edit.path.edit} or
\accelerator{menu.edit.path.edit}) to exit
the \editpathmode\ (\figureref{fig:lettuce4b}).
...
\cbeg{figure}
\cmd{centering}
\gls{input}\marg{lettuce}
\cmd{caption}\marg{Lettuce on Toast}
\cend{figure}
\end{codebox}
Alternatively, load the \sty{flowframtkutils} package (which
automatically loads \sty{pgf}):
\begin{codebox}
\cmd{usepackage}\marg{flowframtkutils}
\end{codebox}
and use \gls{includeteximage}:
\begin{codebox}
\cbeg{figure}
\cmd{centering}
\gls{includeteximage}\marg{lettuce}
\cmd{caption}\marg{Lettuce on Toast}
\cend{figure}
\end{codebox}
...
either with the mouse or with \accelerator{navigate.add_next}
...
For this example, I have the storage unit set to mm and the major
grid unit to cm with 4 subdivisions. The storage unit can be set
in the \dialog{controls} panel of the \dialog{config} dialog
(which can be opened with the \menu{settings.config} menu item).
Alternatively, you can double-click on the unit in the
\gls{statusbar}. The grid can be set with \dialog{grid} dialog,
which can either be opened with the \menu{settings.grid.settings}
menu item or by double-clicking on the grid unit in the top left
corner between the \glspl{ruler}.
...
\figureref{fig:neuron3b}. 

Make sure that the \gls{controlpt} is selected before you start to
drag it with the mouse or you may end up moving the wrong \gls{controlpt}.
If you have enabled the \gridlock, you may find it easier to disable
it while you are editing the curvature \glspl{controlpt}. If you
have trouble selecting a \gls{controlpt}, try cycling round with
\accelerator{editpath.next_control}. Alternatively, use the
\dialog{segmentinfo} dialog, which can be opened with
\menu{editpath.info}.
...
[
    ...
    \mathstool\ (\sectionref{sec:textareas}), and add
...
If the \texttool\ had been used instead of the \mathstool, then
the \mathsmode\ mappings would not be applied and the \LaTeX\ text
will include the Unicode subscript characters. If this has happened,
make sure that the \widget{edittext.latextext.different} button is selected
and type in the appropriate \LaTeX\ code or insert \glssymbol{mshift}
at the start and end of the alternative \LaTeX\ text and click on
the \widget{edittext.latextext.remap} button.
...
\item Select the \mathstool\ (\sectionref{sec:textareas}) and start a
...
\cbeg{figure}
\cmd{centering}
\gls{input}\marg{neuron}
\cmd{caption}\marg{An Artificial Neuron}
\cend{figure}
\end{codebox}
(Remember to use the \sty{pgf} or \sty{flowframtkutils} package.) The image will appear in
...
{fig:bus6}
{\includeimg{bus6}}
{Bus Example\dash Convert Line Segment to a Curve and Combine Paths}
[
...
the \pathattr{fill-paint} to red using the \menu{edit.path.fill_colour}
...
\cmd{usepackage}\marg{shapepar}
\cbeg{document}
\gls{input}\marg{busshape}\cmd{frenchspacing}
...
something like:
\begin{codebox}
\gls{input} shapepar.sty
\codepar
\gls{input} busshape.tex
\cmd{frenchspacing}
The wheels on the bus go round and round...
\codepar
\cmd{bye}
...
\dialog{typeblock} dialog box or the \dialog{flfwizard}.)
...
using the \menu{edit.path.fill_colour} menu item to open
...
\menu{tex.flowframe.set_frame} menu item. This will open up the
\dialog{flowframe} dialog box shown in \figureref{fig:poster5}.  Set
the type to \widget{flowframe.static}, and call it \qt{title}.
Unselect the \widget{flowframe.margins.none} checkbox to show the
inner padding fields and set them as desired.  I used 0.1in for all
the margins. You could increase the left inner padding to avoid the
\gls{bitmap} but this would affect the \manmsg{centre} alignment
that will be applied to the title.  (The alignment is relative to
the inner area not to the border.)
...
\cmd{centering}
Insert figure here!
\cmd{caption}\marg{A Sample Figure}
\cmd{label}\marg{fig:sample}
\cend{staticfigure}
\end{compactcodebox}%
The \qt{table} frame's contents are:
\begin{compactcodebox}
\cbeg{statictable}
\cmd{caption}\marg{A Sample Table}
\cmd{label}\marg{tab:sample}
\cmd{centering}
Insert table here!
\cmd{par}
\cend{statictable}
\end{compactcodebox}%
The frame contents are shown on the \gls{canvas}. (This is
configurable, see \sectionref{sec:annotations}.) Bear in mind that
this is just the \LaTeX\ code and is not formatted but simply shown
ragged right in a monospaced font within the bounds marking the
inner padding.
...
package.) With modern \LaTeX\ kernels, UTF-8 is now the default, so
the first line may be omitted.
...
If you want to quickly check how the layout will look, use
\menu{file.export} and select the \widget{export.flfpdf} button.
This will create a temporary file and use \pdfLaTeX\ to compile it.
As no document body has been set, empty content will be inserted to
create a page. In \figureref{fig:poster-flfpdfexport}, I've change
the processor to use \app{latexmk} instead of \pdfLaTeX. This is
useful if multiple runs are required, but make sure to add the
\code{-pdf} switch in the command line options.
You may omit this step and just move onto the next one.

\FloatFig{fig:poster-flfpdfexport}
[\setkeys{Gin}{width=0.8\textwidth}]
{\includeimg{poster-flfpdfexport}}
{Poster Example\dash Export to PDF Document}


\item Use the \menu{file.export} menu item to create
a new \LaTeX\ package or class that defines these frames. To create
a package, select the \widget{export.sty} button. To create a class,
select the \widget{export.cls} button. In this example, 
I've selected \widget{export.sty} to create a package.
I called my file \filefmt{poster.sty} (\figureref{fig:poster7}).
...
\cmd{documentclass}\marg{article}
\codepar
\comment{use new package created in this example:}
\cmd{usepackage}\marg{poster}
\codepar
\cbeg{document}
\codepar
This is the main body of the poster. This text will
appear in the first of the two flow frames. Once it
has reached the end of the first flow frame, it 
will then continue in the second flow frame.
\codepar
\comment{Lots of text omitted}
\codepar
\cend{document}
\end{codebox}

Now suppose I've exported to a class by using the
\widget{export.cls} button in the export dialog. Then the start of my
\filefmt{poster.tex} file just uses this class:
\begin{codebox}
\comment{use new class created with this example:}
\cmd{documentclass}\marg{poster}
\codepar
\cbeg{document}
This is the main body of the poster. This text will
appear in the first of the two flow frames. Once it
...
Another possibility is to put the document content into the 
\widget{texeditor.latexcodeblock.document} tab, as shown in
\figureref{fig:posterdoc}, and then use the \widget{export.flfpdf}
export option. The resulting PDF is the same as shown in
\figureref{fig:posterfinal}, provided that the \widget{processes.pdflatex}
processor has been replaced with \app{latexmk}, as described
earlier, since a second run is required to resolve the
cross-references. Alternatively, you can use \app{arara} instead,
but you will need to add the directives to the
\widget{texeditor.latexcodeblock.magic_comments} tab, as shown in 
\figureref{fig:poster-arara}.

\FloatFig{fig:posterdoc}
[\setkeys{Gin}{width=0.8\textwidth}]
{\includeimg{posterdoc}}
{Poster Example\dash Adding Document Body}

\FloatFig{fig:poster-arara}
[\setkeys{Gin}{width=0.8\textwidth}]
{\includeimg{poster-arara}}
{Poster Example\dash Using Arara to Create PDF}
...
\item The 12\gls{pt} font size has a corresponding \gls{baselineskip}
of 14.5\gls{pt}. For this example, it is more practical to have a grid
...
use \menu{settings.grid.settings} to open the \dialog{grid} dialog
(or double-click on the grid unit in the corner between the
\glspl{ruler}), and enter the values as shown in \figureref{fig:news2}.
...
open the \dialog{config} dialog box (or double-click on the storage
unit in the \gls{statusbar}) and change the storage unit in
...
area. I used 58\gls{pt} margins on all sides. To do this, use the
...
shown in \figureref{fig:news5a}: static type, label \qt{title} and
no inner padding. The \widget{flowframe.border} field has been set to
...
\begin{compactcodebox*}
\cbeg{center}
\gls{bfseries}\gls{Huge}
Fairy Tale Times
\cend{center}
\cmd{hfill} Issue 2. 7 December 2005.
\end{compactcodebox*}
...
\figureref{fig:news7}: dynamic type, label \qt{leftheading}, no
border and no inner padding. The frame contents contains the section
command:
\begin{compactcodebox}
\cmd{section}\marg{Killer Wolf on the Loose}
\end{compactcodebox}
This is again set using the \widget{flowframe.edit} button
to open the \TeX\ editor.

\begin{information}
Numbered section headings are problematic in dynamic frames as the
contents aren't processed until the page is shipped out. In this
example, the page numbering will be switched off by changing
the \ctr{secnumdepth} counter, so it won't be a problem.
\end{information}
...
section command:
\begin{compactcodebox}
\cmd{section}\marg{Tragic Wall Accident}
\end{compactcodebox}
...
shown in \figureref{fig:news9}: dynamic type, label \qt{lefttext},
and shape set to \widget{flowframe.shape_parshape}. The border is
set to none but this field will be disabled as soon as the shape is
changed as the border option is only available for the default
standard shape.

Note that the \widget{flowframe.align} field has been set to
...
devoured an old lady. He then disguised himself as the
old lady in order to deceive her granddaughter. 
Luckily for the little girl a woodsman arrived in time 
to rescue her. Parents are being cautioned not to let 
their children wander about on their own, and to 
remind them not to talk to strangers.

The next day the wolf struck again, this time 
targeting two pig brothers who had most incautiously 
made their dwellings on the cheap using inadequate 
materials. The wolf also made an attempt on the third 
pig brother, but was unable to break into his house.

Police are appealing to the public for witnesses, and 
remind people to keep their doors securely fastened 
at all times.

``Always ask to see identification,'' said one police 
advisor, ``and invest in improving the general 
...
The \widget{annotations.frame_contents.show} setting uses the frame
rectangular inner area as a guide to location and dimensions. It
doesn't take the shape into account. This means that the frame
contents has lines stretching across to the right. This can result
in a cluttered \gls{canvas}, so you may prefer to switch off the
feature at this point. You can do this from the \dialog{annotations}
tab of the \dialog{configui} dialog, which can be opened with 
the \menu{settings.configui} menu item. Uncheck the
\widget{annotations.frame_contents.show} checkbox and click on the
\btn{okay} button.
...
{Newspaper Example\dash Added Image (Frame Contents Now Hidden)}

\item Set the \gls{flowframe} data shown in \figureref{fig:news11} to
this \gls{bitmap}: static type, label \qt{eggimage}, and border
shown. No content is required for this frame.

\begin{information}
When using a \gls{bitmap} as a frame, you must set the
...
\figureref{fig:news13}: dynamic type, label \qt{righttext}.
The \widget{flowframe.shape} field has been set to
\widget{flowframe.shape_parshape} (which automatically disables the
\widget{flowframe.border} field) and the
...
particularly those vulnerable members of the 
population suffering from eggshell syndrome.

\small\em
Exclusive interview with one of the King's men on 
page 6.
\end{verbatim}
...
\figureref{fig:news15}: static type and label \qt{Lbar}.  Make sure that you have set the
\widget{flowframe.border} field to
\widget{flowframe.border_as_shown}. Since this is decorative, the
...
\item Switch to the \tool{select}, \select\
this line (use \accelerator{navigate.skip} if you have difficulty
selecting it with the mouse) and assign the \gls{flowframe} data shown in
\figureref{fig:news17}: static type, label \qt{hline} and border shown.
...
\figureref{fig:news19}: dynamic type, label \qt{bottomheading}, no
border, and middle alignment. The frame contents is another sectioning
command:
\begin{compactcodebox}
\cmd{section}\marg{Relief as Missing Sheep Finally Return Home}
\end{compactcodebox}
...
\figureref{fig:news21}: flow type, label \qt{left} and no border. 
Similarly for the right hand lower rectangle: flow type, label
\qt{right} and no border.
...
\figureref{fig:news23}: static type, label \qt{sheepimage}, and
border set to \widget{flowframe.border_as_shown}.
...
To do this, select the \tool{closed_line}, and construct a polygon
...
\cmd{usepackage}\marg{lmodern}
\end{compactcodebox}%
(see \figureref{fig:newspreamble}) and the late preamble to
\begin{compactcodebox*}
\comment{suppress section numbering}
\cmd{setcounter}\marg{\ctr{secnumdepth}}\marg{0}
\comment{set the paragraph indentation for static}
\comment{and dynamic frames}
...
\item This next step can be done using the \dialog{flowframe}
dialog as in the earlier steps, but let's use the \dialog{flfwizard}
instead, which can be opened with the \menu{tex.flowframe.wizard}
menu item (make sure you switch to the \tool{select} first).

If you haven't already selected the new rectangle, select it from
the \widget{flfwizard.frametype.object} list. Since it's the most
recent object to be added, it will be the last in the list. If
you're not sure, double-click on the list item and the rectangle
should be selected on the \gls{canvas}, which you can double-check
without having to close the \dialog{flfwizard}.

\FloatFig{fig:news28}
[\setkeys{Gin}{width=\linewidth}]
{\includeimg{news28}}
{Newspaper Example\dash Assigning the Footer Frame Data using the
Wizard}

Select the \widget{flfwizard.frametype.footer} button, as shown in 
\figureref{fig:news28}. The \btn{flfwizard.next} button should have
the text \widget{flfwizard.labelpages.title}. Click on it to move to
the \dialog{flfwizard.labelpages} page, as shown in \figureref{fig:news29}.

\FloatFig{fig:news29}
[\setkeys{Gin}{width=\linewidth}]
{\includeimg{news29}}
{Newspaper Example\dash Footer Label and Pages}

Since this is a special dynamic frame, the label has automatically
been filled in, so just click on the \btn{flfwizard.next} button to
move onto the \dialog{flfwizard.style} panel, shown in
\figureref{fig:news30}. The \widget{flfwizard.style.border} checkbox
should be unchecked, change the \widget{flfwizard.style.valign}
field to \widget{flowframe.align_middle} and set the \widget{flfwizard.style.cmds}
to just \code{itshape}. Since it's just a single declaration, the
leading backslash may be omitted. If you want extra declarations,
such as \gls{large}, you will need the leading backslashes.

\FloatFig{fig:news30}
[\setkeys{Gin}{width=\linewidth}]
{\includeimg{news30}}
{Newspaper Example\dash Footer Style}

Click on the \btn{flfwizard.next} button to
move onto the \dialog{flfwizard.dimensions} panel.
Click on the \widget{flfwizard.dimensions.compute_sym_x_shift}
button to calculate the horizontal shift for even pages
(see \figureref{fig:news31}).

\FloatFig{fig:news31}
[\setkeys{Gin}{width=\linewidth}]
{\includeimg{news31}}
{Newspaper Example\dash Footer Paddings and Offsets}

The text on the \btn{flfwizard.next} should now be
\widget{flfwizard.finish.title}. Click on it to finish.
You will be asked if you want to start over with another object. 
Click \qt{No} to close the dialog.

\item Save the image as \filefmt{newspaper.jdr} and then select the
\menu{file.export} menu item. Select the
\widget{export.cls} button, and save as \filefmt{newspaper.cls}.

\item Use your favourite text editor to create a file called
\filefmt{news.tex} that looks like:
\begin{codebox}
\cmd{documentclass}\marg{newspaper}
\codepar
\cbeg{document}
\comment{set the paragraph shape}
\gls{input}\marg{sheepcutout}
\comment{suppress paragraph indentation}
\cmd{noindent}
There was much celebration yesterday morning when 
Little Bo Peep's sheep finally returned home. They 
had been missing for more than a week.
\codepar
``I just didn't know where to find them,'' the 
shepherdess stated, ``but I was told to leave them 
alone and they'd come home.''
\codepar
\comment{lots of text omitted}
\cend{document}
\end{codebox}
...
\begin{codebox}
\cmd{documentclass}[twoside,12pt]\marg{article}
\cmd{usepackage}\marg{newspaper}
\codepar
\cbeg{document}
\comment{set the paragraph shape}
\gls{input}\marg{sheepcutout}
...
I've use the \dialog{typeblock} and \dialog{flowframe} dialog boxes,
but you may prefer to use the \dialog{flfwizard} instead.

\begin{enumerate}
\item For this example I'm going to start with the
\gls{grid.tschichold} grid on A4 portrait paper.  
To set the paper size and orientation, use the \menu{settings.paper}
sub-menu.

To set the grid, use \menu{settings.grid.settings} or double-click
on the grid unit in the top left corner between the \glspl{ruler} to
...
\item Either use \menu{settings.config} to open the
\dialog{config} dialog (and select the \dialog{controls} tab) 
or double-click on the storage unit in the \gls{statusbar}.
Set the image \gls{storageunit} to \gls{pt}, as shown in
...
\widget{typeblock.compute_from_objects} button. (If this button
is disabled, select the rectangle from the list.) This will fill the margins, but it looks a bit untidy
...
\figureref{fig:brochure9}. You can move the \gls*{path} with the
mouse or via \menu{edit.moveby} or use
\menu{transform.align_to_page.right} and
...
\item Use \menu{edit.path.fill_colour} to change the 
...
Set the \widget{flowframe.style_cmds} to:
\begin{compactcodebox*}
\gls{bfseries}\cmd{color}\marg{white}\gls{Huge}
\end{compactcodebox*}%
as shown in \figureref{fig:brochure11}.

\begin{information}
If you prefer to use the \dialog{flfwizard} dialog, select the
\widget{flfwizard.frametype.footer} button and the label will
automatically be filled.
\end{information}

\FloatFig{fig:brochure11}
{\includeimg{brochure11}}
...
[Brochure Example\dash \glsentrytext{displaypage.all}]
...
margins to 10\gls{pt}. Set the \widget{flowframe.style_cmds} field
to:
\begin{compactcodebox}
\gls{bfseries}\gls{Large}
\end{compactcodebox}%
as shown in \figureref{fig:brochure20}.

\FloatFig{fig:brochure20}
{\includeimg{brochure20}}
...
\widget{export.cls} button and export the image as
...
\begin{codebox}
\cmd{documentclass}[twoside]\marg{brochure}
\codepar
\cmd{usepackage}[T1]\marg{fontenc}
\cmd{usepackage}[utf8]\marg{inputenc}
\cmd{usepackage}\marg{lmodern}
\codepar
\cmd{usepackage}\marg{blindtext}
\codepar
\cbeg{document}
\cmd{Blinddocument}
\cend{document}
...
\item To complete the rectangle, press \accelerator{tools.finish}
or use \mnemonictrail{tools.finish}.
...
\mnemonictrail{edit.path.fill_colour}. This will open up
...
\mnemonictrail{edit.path.fill_colour}.
...
\mnemonic{edit.path.fill_colour} to set the \pathattr{fill-paint}
...
\widget{edittext.latextext.different} button (either \keys{\keyref{tab}} to it and
press \keys{\keyref{space}} or use \mnemonic{edittext.latextext.different}.) This will enable
...
{No Mouse Example\dash Text is Now \Glsentrytext{manual.centred}}
...
a plain text format. The \gls*{AJR} format has less precision than the
\gls*{JDR} format. The current version number for both formats
is~2.2.

\begin{information}
As from \FlowframTk\ v0.8.8, any \glspl{object} that are copied
from the \gls{canvas} to the clipboard are stored as a complete
image in the latest \gls{JDR} format with MIME type
\code{application/x-flowframtk-jdr}.
For example, if a \gls{path} and a \gls{textarea} are selected and
copied to the clipboard, the two objects will be copied to a
temporary \gls{group} which represents an image that's stored
on the clipboard in the binary \gls{JDR} format (with no settings).
\end{information}

The first line of the plain text \gls*{AJR} format must be:
\begin{jdrversion}{2.2 onwards}
\begin{compactcodebox}
AJR \meta{version} \meta{encoding}
\end{compactcodebox}
\end{jdrversion}
\begin{jdrversion}{1.0--2.1}
\begin{compactcodebox}
AJR \meta{version}
\end{compactcodebox}
\end{jdrversion}
where \meta{version} is the version number and \meta{encoding}
(only present from 2.2) is the character encoding name. For example:
\begin{verbatim}
AJR 1.8
\end{verbatim}
or
...
newline or space character). Similarly for the encoding name (for
AJR 2.2 onwards).

\begin{information}
All the markup in an \gls{AJR} file is within the ASCII range, but
textual content (any \glspl{textarea}, \glspl{textpath},
descriptions, tags or static/dynamic frame content, preamble or
document body) may contain non-ASCII characters so assume UTF-8 
rather than ASCII for pre 2.2 versions.
\end{information}
...
\code{java.io.DataOutputStream} class. The \gls{AJR} version writes
...
\code{java.io.DataOutputStream} class. The \gls{AJR} version is the
...
\end{verbatim}
where \code{dout} is a \code{java.io.DataOutputStream} object
and \code{text} is a \code{java.lang.String}. For the \gls{AJR} format,
the length is written as an integer followed by a \emph{single}
white space character. (Avoid inserting a line break immediately
after \meta{n} as the end-of-line character for some operating systems consists of two
...
The HSB support in \FlowframTk's \dialog{paintselector} panel 
wasn't working correctly and has been removed as the
\btn{choose_colour} can now be used instead, but that will convert
to \gls{index.colour.rgb} or \gls{index.colour.cmyk} or
\gls{index.colour.greyscale}.
...
\begin{itemdesc}
Linear gradient paint specs are saved as:
\begin{jdrversion}{2.2 onwards}
\begin{syntaxline}
\meta{start-paint} \meta{mid-flag} [\meta{mid-paint}]
\meta{end-paint} \meta{direction}
\end{syntaxline}
\end{jdrversion}
\begin{jdrversion}{1.0--2.1}
\begin{syntaxline}
\meta{start-paint} \meta{end-paint} \meta{direction}
\end{syntaxline}
\end{jdrversion}
where \meta{start-paint} is the start \type{paint},
\meta{mid-flag} is a \type{boolean} indicating whether or not
the \meta{mid-paint} is present,
\meta{mid-paint} is the mid \type{paint} (if \meta{mid-flag} is true),
\meta{end-paint} is the end \type{paint},
and \meta{direction} an \type{integer} indicating the
...
\begin{itemdesc}
Radial gradient paint specs are saved as:
\begin{jdrversion}{2.2 onwards}
\begin{syntaxline}
\meta{start-paint} \meta{mid-flag} [\meta{mid-paint}]
\meta{end-paint} \meta{location}
\end{syntaxline}
\end{jdrversion}
\begin{jdrversion}{1.3--2.1}
\begin{syntaxline}
\meta{start-paint} \meta{end-paint} \meta{location}
\end{syntaxline}
\end{jdrversion}
where \meta{start-paint} is the start \type{paint},
\meta{mid-flag} is a \type{boolean} indicating whether or not
...
\meta{mid-paint} is the mid \type{paint} (if \meta{mid-flag} is true),
\meta{end-paint} is the end \type{paint},
and \meta{location} is an \type{integer} indicating the
...
(although white space may be acceptable at that point in the
...
\item Versions 2.1 and above include the origin shift.
...
{Grid Identifiers}
  ...
  The grid specification (1.6 onwards) is as follows
    ...
    \begin{jdrversion}{2.2 onwards}
    \begin{syntaxline}
    \meta{unit} \meta{major} \meta{major-y} \meta{minor} \meta{minor-y}
    \end{syntaxline}
    \end{jdrversion}
    \begin{jdrversion}{1.6--2.1}
    \begin{syntaxline}
    \meta{unit} \meta{major} \meta{minor}
    \end{syntaxline}
    \end{jdrversion}
    where
     ...
     \item \meta{unit} is the \type{unit-id} representing the grid unit.
       ...
       \item \meta{major} is a \type{double} representing the major
        grid division (in terms of the grid unit). For version 2.2
        onwards this is just for the horizontal $x$-axis.

       \item \meta{major-y} is a \type{double} representing the major
        grid division (in terms of the grid unit) for the vertical
        $y$-axis.

       \item \meta{minor} is an \type{integer} representing the grid subdivision.
        For version 2.2 onwards this is just for the horizontal $x$-axis;

       \item \meta{minor-y} is an \type{integer} representing the grid subdivision
...
\item\label{jdr:cliptag}From v2.2 onwards the \gls{clippingtag} comes
after the \TeX\ settings.
\begin{jdrversion}{2.2 onwards}
\begin{syntaxline}
\meta{clip-tag}
\end{syntaxline}
\end{jdrversion}
The \meta{clip-tag} is stored as a \type{string} and may be empty or
null.
...
\gls+{typeblock} and the \meta{description} represents the \gls{imagedescription}.
The \meta{tag} is also included for version 2.1 and above which
represents the \inlineglsdef{imagetag}. This tag should be empty except for any
image stored in the clipboard. In which case, the image tag is set
to the filename that was current when the data was copied to the
clipboard. (This filename may simply be \widget{label.untitled}
for a new unsaved image.) The \gls{imagetag} in the clipboard is simply
used to determine whether or not to offset the content when pasted
into a \gls{canvas}.
...
For example, in the \gls{AJR} version
\begin{verbatim}
3 6 9 -1
\end{verbatim}
...
{%
    \begin{tabular}{llll}
    0 & \widget{arrow.nomarker} &
       11 & \widget{arrow.boxfilled}\\
    1 & \widget{arrow.pointed} &
       12 & \widget{arrow.boxopen}\\
    2 & \widget{arrow.triangle} &
       13 & \widget{arrow.cross} \\
    3 & \widget{arrow.circle} &
       14 & \widget{arrow.plus}\\
    4 & \widget{arrow.diamond} &
       15 & \widget{arrow.star}\\
    5 & \widget{arrow.square} &
       16 & \widget{arrow.triangle_up_filled}\\
    6 & \widget{arrow.bar} &
       17 & \widget{arrow.triangle_up_open}\\
    7 & \widget{arrow.latex} &
       18 & \widget{arrow.triangle_down_filled}\\
    8 & \widget{arrow.round} &
       19 & \widget{arrow.triangle_down_open}\\
    9 & \widget{arrow.dotfilled} &
       20 & \widget{arrow.rhombus_filled} \\
    10 & \widget{arrow.dotopen} &
       21 & \widget{arrow.rhombus_open}
  \end{tabular}
}
...
{%
  ...
  22 & \widget{arrow.pentagon_filled} & 41 & \widget{arrow.halfcusp_down}
     & 60 & \widget{arrow.semicircle_open}\\
  23 & \widget{arrow.pentagon_open} & 42 & \widget{arrow.altlatex}
     & 61 & \widget{arrow.semicircle_filled} \\
  24 & \widget{arrow.hexagon_filled} & 43 & \widget{arrow.altlatex_open}
     & 62 & \widget{arrow.star5_open}\\
  25 & \widget{arrow.hexagon_open} & 44 & \widget{arrow.triangle_open}
     & 63 & \widget{arrow.star5_filled}\\
  26 & \widget{arrow.octagon_filled} & 45 & \widget{arrow.circle_open}
     & 64 & \widget{arrow.asterisk}\\
  27 & \widget{arrow.octagon_open} & 46 & \widget{arrow.diamond_open}
     & 65 & \widget{arrow.scissors_down_filled}\\
  28 & \widget{arrow.pointed60} & 47 & \widget{arrow.brace}
     & 66 & \widget{arrow.scissors_up_filled}\\
  29 & \widget{arrow.pointed45} & 48 & \widget{arrow.rectanglecap}
     & 67 & \widget{arrow.scissors_down_open}\\
  30 & \widget{arrow.hooks} & 49 & \widget{arrow.chevroncap}
     & 68 & \widget{arrow.scissors_up_open}\\
  31 & \widget{arrow.hook_up} & 50 & \widget{arrow.fastcap}
     & 69 & \widget{arrow.heart_right_filled}\\
  32 & \widget{arrow.hook_down} & 51 & \widget{arrow.roundcap}
     & 70 & \widget{arrow.heart_right_open}\\
  33 & \widget{arrow.halfpointed_up} & 52 & \widget{arrow.trianglecap}
     & 71 & \widget{arrow.heart_filled}\\
  34 & \widget{arrow.halfpointed_down} & 53 & \widget{arrow.inverttrianglecap}
     & 72 & \widget{arrow.heart_open}\\
  35 & \widget{arrow.halfpointed60_up} & 54 & \widget{arrow.invertchevroncap}
     & 73 & \widget{arrow.snowflake}\\
  36 & \widget{arrow.halfpointed60_down} & 55 & \widget{arrow.invertfastcap}
     & 74 & \widget{arrow.starchevron_open}\\
  37 & \widget{arrow.halfpointed45_up} & 56 & \widget{arrow.altbar}
     & 75 & \widget{arrow.starchevron_filled}\\
  38 & \widget{arrow.halfpointed45_down} & 57 & \widget{arrow.altround}
     & 76 & \widget{arrow.star6_filled}\\
  39 & \widget{arrow.cusp} & 58 & \widget{arrow.altsquare}
     & 77 & \widget{arrow.star6_open}\\
  40 & \widget{arrow.halfcusp_up} & 59 & \widget{arrow.altbrace}
     & 78 & \widget{arrow.equilateral_filled}\\
   & & & & 79 & \widget{arrow.equilateral_open}
  \end{tabular}
}
...
{%
  \begin{tabular}{llll}
  80 & \widget{arrow.ballcap} & 85 & \widget{arrow.tripleleafforwardcap} \\
  81 & \widget{arrow.leafcap} & 86 & \widget{arrow.tripleleafbackcap} \\
  82 & \widget{arrow.doubleleafcap} & 87 & \widget{arrow.doubleleafforwardcap} \\
  83 & \widget{arrow.tripleleafcap} & 88 & \widget{arrow.doubleleafbackcap} \\
  84 & \widget{arrow.clubcap} & 89 & \widget{arrow.bulgecap}\\
  90 & \widget{arrow.cutoutbulgecap} 
  \end{tabular}
}
{Additional Marker IDs (Version 1.6 Onwards)}

\FloatTable{tab:jdrarrowid2.1}
{%
  \begin{tabular}{llll}
  90 & \widget{arrow.indeptriangle2} & 91 & \widget{arrow.deptriangle2}\\
  92 & \widget{arrow.offsettriangle2} & 93 & \widget{arrow.stealth2} 
  \end{tabular}
}
{Additional Marker IDs (Version 2.1 Onwards)}

\FloatTable{tab:jdrarrowid2.2}
{%
  \begin{tabular}{llll}
  95 & \widget{arrow.indep_centred_triangle2_filled} &
  96 & \widget{arrow.indep_centred_triangle2_open}\\
  97 & \widget{arrow.indep_centred_rectangle2_filled} &
  98 & \widget{arrow.indep_centred_rectangle2_open} \\ 
  99 & \widget{arrow.indep_centred_ellipse2_filled} &
  100 & \widget{arrow.indep_centred_ellipse2_open} \\ 
  101 & \widget{arrow.indep_centred_diamond2_filled} &
  102 & \widget{arrow.indep_centred_diamond2_open} \\ 
  103 & \widget{arrow.indep_centred_triangleup2_filled} &
  104 & \widget{arrow.indep_centred_triangleup2_open} \\ 
  105 & \widget{arrow.indep_centred_triangledown2_filled} &
  106 & \widget{arrow.indep_centred_triangledown2_open} 
  \end{tabular}
}
{Additional Marker IDs (Version 2.2 Onwards)}
  ...
  available for version 1.6 onwards, markers listed in
  \tableref{tab:jdrarrowid2.1} are available for version 2.1
  onwards, and markers listed in
  \tableref{tab:jdrarrowid2.2} are available for version 2.2 onwards.

  The \meta{marker data} is stored as follows:
  \begin{jdrversion}{1.4--2.0}
  \begin{syntaxline}
  \meta{size} \meta{repeat} \meta{is reversed} \meta{orient data}
  ...
  \begin{jdrversion}{2.1 onwards}
  \begin{syntaxline}
  \meta{size} \meta{width-flag} [\meta{width}]
  \meta{repeat} \meta{is reversed} \meta{orient data}
  \meta{\manmsg{colour} data} \meta{overlay}
  [\meta{user offset flag} [\meta{user offset}]
  \meta{repeat offset flag} [\meta{repeat offset}]]
  \meta{composite data}
  ...
  are true, respectively.
    ...
    This value is present even for markers that don't have a 
    \gls{index.marker.size} attribute.

    \item For version 2.1 onwards, there is now support for markers
    that have a \gls{index.marker.width} in addition to the size (which is considered
    the marker length). The marker width attribute is not the same
    as the pen width. If the marker supports an independent width
    attribute, the \type{boolean} \meta{width-flag} should be true
    and should be followed by \meta{width} which is a \type{length}.
    If the marker doesn't support an independent width attribute,
    \meta{width-flag} should be false and \meta{width} should be
    omitted.
...
The \meta{modifier specs} are as follows:
  ...
  \meta{outline-flag} [\meta{outline-fill paint}], which is
  the same as for \glspl*{textarea} (\hyperref[jdr:textarea]{\code{T}}).
  Version 2.2 supports the new show path option. The boolean
  \meta{show-path-flag} indicates whether or not the underlying path
  should be shown. Regardless of this value, \meta{show-path-flag}
  is followed by the \pathattr{line-paint}, \pathattr{fill-paint}
  and \gls{line-style} for when the underlying path is shown.
  (This allows the style to be retained if the setting was temporarily 
  switched off when the image was saved.
  So the \meta{object-specs} are:
  \begin{jdrversion}{2.2 onwards}
  \begin{syntaxline}
  \meta{outline-flag} [\meta{outline-fill paint}] \meta{underlying shape specs}
  \meta{show-path-flag} \meta{line paint} \meta{fill paint} \meta{line style}
  \end{syntaxline}
  \end{jdrversion}
  \begin{jdrversion}{1.8--2.1}
  \begin{syntaxline}
  \meta{outline-flag} [\meta{outline-fill paint}] \meta{underlying shape specs}
  \end{syntaxline}
  \end{jdrversion}
...
\end{jdrversion}
  ...
  where:
  \begin{enumerate}
  \item The \type{boolean} value \meta{single} indicates that the 
  shape should be drawn as a single path.
...
may be empty or a control sequence name, such as \code{textbf} or 
consist of \LaTeX\ font changing commands where the last may be 
a text-block command, such as \code{\cmd{large}\cmd{textbf}}.)
This value should be omitted for other frame types.
...
\FlowframTk\ depends on the following third party libraries:
\begin{itemize}
   \item \TeX\ Java Help \filefmt{texjavahelplib.jar}
   (GPL, \url{https://github.com/nlct/texjavahelp}).
   \item \TeX\ Java Parser \filefmt{texparserlib.jar}
