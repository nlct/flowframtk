...
You can specify a tag that identifies \glspl{path} that should clip rather
than be stroked or filled when exporting to a file that contains
\sty{pgf} drawing code. The path will show normally on the
\gls{canvas}.
...
That is, instead of writing \code{\gls{pgfusepath}\marg{stroke,fill}}
\FlowframTk\ will write \code{\gls{pgfusepath}\marg{clip}}.

The clip tag is saved in the image file, so existing images will use
whatever tag was set in the file. If you want to prevent clipping,
you can simply set the \widget{clipping.tag} field to empty if you don't
want to remove the tag from all the \glspl{path} in the current
image.
...
\begin{information}
If you have multiple paths with the designated clip tag, the clipping area won't
...
\cmd{renewcommand}\gls{flowframtkencapobject}\oarg{8}\marg{\comment{}
 \cmd{ifstrequal}\marg{\#2}\marg{JDRGroup}
  \marg{\cbeg{pgfscope}\#8\cend{pgfscope}}\marg{\#8}}
\end{codebox}
...
group.) The clipping tag has been set to \qt{clip} (as shown in
...
The \meta{tag} is the object's tag (\sectionref{sec:objecttag}).
...
dialog from which you assign a tag to all
...
You can designate a particular tag as a clip tag (see
...
\section{Clipping Shapes}\label{sec:clipping}
...
The shape clipping function alters \glspl{shape} by removing
...
If you want to retain the original shapes, then set up a clipping
tag instead (see \sectionref{sec:cliptag}).
...
fit.
This splitting process doesn't always work correctly.
...
\item\label{jdr:cliptag}From v2.2 onwards the clip tag comes
