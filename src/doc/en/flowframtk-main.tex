...
If any \glspl{path} have been selected or if any
\glslink{text-attribute.outline}{outlined} \glspl{textarea}
...
\gls{text-attribute.anchor} or set the image \glspl{image.typeblock}
...
The \LaTeX\ export functions (see \sectionref{sec:exportpgf}) use the \sty{pgf} text
...
\figureref{fig:edittextb}). Note that the vertical \gls{text-attribute.anchor}
...
\begin{important}
The alternative text may not be used by
\hyperref[sec:exportimage]{export} functions if
the \gls*{textpath} has been made an
\gls{text-attribute.outline} or if the
...
function. The text from the original \gls{textarea} will then follow
...
If the original \gls*{textarea} was in \gls{text-attribute.outline} mode, the new
\gls*{textpath} will also be in \gls{text-attribute.outline} mode, possibly with a fill
paint, depending on the original \gls*{textarea} and \gls*{path}
fill paint. (If the original \gls*{textarea} didn't have a fill
paint, the \gls*{path} fill paint will be applied.)
...
\section{Reducing to \Glsfmttext{manual.grey} Scale}\label{sec:reduce}
...
\inlineglsdef{text-attribute.family}
...
\inlineglsdef{text-attribute.size}
...
\inlineglsdef{text-attribute.series}
...
\inlineglsdef{text-attribute.shape}
...
\inlineglsdef{text-attribute.transformation-matrix}
...
\inlineglsdef{text-attribute.anchor}
