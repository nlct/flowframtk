...
\LaTeX\ lengths \gls{typeblockwidth} and \gls{typeblockheight}
(which are initialised by the \sty{flowfram} package to
...
the paper edge and the \gls{typeblock}. You can either type the
lengths into the supplied fields or you can get \FlowframTk\ to
calculate the margins for a \gls{typeblock} by clicking on the
\widget{typeblock.compute_from_objects} button.

The \widget{typeblock.compute_from_objects} button will calculate
the minimum bounds required to encompass all \glspl{object} that
have been selected in the \widget{typeblock.select_object} list.
This button will be disabled if no items in the list are selected.

If any \glspl{object} were already selected when you opened the
\dialog{typeblock} dialog, they will be automatically selected in the
\widget{typeblock.select_object} list. You can scroll up or down
this list and select different \glspl{object} if you prefer. A
single mouse click will select an item in the list.  A double-click
...
have selected the correct object.  If you hold down the
...
down the \keys{\keyref{ctrl}} key while clicking on an item, it
will add the item to the current selection.
...
The \widget{typeblock.adjust_width} and \widget{typeblock.adjust_height}
buttons don't round the margin values. They round the width or
height and then adjust the margins to fit. Make sure you set the
paper size first.
\end{important}
