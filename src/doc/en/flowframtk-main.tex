...
\hyperref[sec:lines]{line path} or \hyperref[sec:curves]{curve
...
you can delete the previous segment using the \accelerator{action.delete_last} key.
\end{warning}
...
\accelerator{action.constructclick} will emulate a single mouse
...
\gls*{textarea}: the text tool and the \manmsg{maths} tool. These
...
With either the text or \manmsg{maths} tool,
...
the text and \manmsg{maths} tools.  (If you do switch between the
...
text\slash\manmsg{maths} tool is selected) you will simply create a new
overlapping \gls*{textarea}.

If your operating system supports \gls{drag-and-drop}, you can
also \gls{drag-and-drop} text onto the \gls{canvas}. If the drop point is
inside the \gls{textarea} construction field, the dropped text will be
inserted into the field (newline characters will be converted to
spaces). If the drop point is outside the \gls{textarea} construction
field, a new text area will be created for each line of dropped
text. The mapping applied to the dropped text will be according to
the currently selected text tool.  If text is dropped onto the
\gls{canvas} in select mode, the text mappings (not \manmsg{maths}
mappings) will be applied. If a file is
dropped onto the \gls{canvas} in text- or \manmsg{maths}-mode, the file
path (or URI) will be added as a new \gls{textarea}.
...
{
 {fig:dndtexta}{\includeimg[height=0.2\textheight]{dndtext1}}{},
 {fig:dndtextb}{\includeimg[height=0.2\textheight]{dndtext2}}{},
 {fig:dndtextc}{\includeimg[height=0.2\textheight]{dndtext3}}{},
 {fig:dndtextd}{\includeimg[height=0.2\textheight]{dndtext4}}{},
 {fig:dndtexte}{\includeimg[height=0.2\textheight]{dndtext5}}{}
}
[Drag and Drop Text]
{Drag and Drop Text: \subfigref{a} drag initiated on some
selected text in another application; \subfigref{b} selected text
dragged onto canvas; \subfigref{c} the text that was dropped onto
the canvas in \subfigref{b} has been converted into two text areas;
\subfigref{d}
the selected text from \subfigref{a} is now dragged onto the text
area construction field; \subfigref{e} the dropped text has been
inserted into the construction field.}

Whilst a text area is under construction, you can activate the
\menu{selectedtext} \dgls{popupmenu}, illustrated in \figureref{fig:textpopup},
with a \gls{menuclick} (or use the context-menu key).
