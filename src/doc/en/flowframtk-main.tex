...
Images can be imported from the \gls{Acorn-DrawFile-format}.
\FlowframTk's experimental mode may provided additional import options, but
...
\begin{itemize}
    ...
    \item (only if \widget{processes.support_eps_svg} is on)
     a PostScript file or \pgls{svg} file.
\end{itemize}
...
\begin{important}
You can't import back from the files you can export to.
\end{important}
...
clutter in the \dialog{export} dialog, the export functions that
rely on the DVI format are no longer supported by default.  If you
still require EPS or \gls{svg}, you need to enable support by
ensuring that the \widget{processes.support_eps_svg} checkbox is
selected. This will enable additional fields for \app{latex} (DVI
format), \app{dvips} and \app{dvisvgm}. The \file{libgs} library is
only applicable with \app{dvisvgm}, so you only need it if you
intend to export to \gls{svg}.
...
normal (non-experimental) mode, and that is the
...
applied. Note that when importing from a
\glslink{Acorn-DrawFile-format}{DrawFile}, any text
