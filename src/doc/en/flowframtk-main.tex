...
The original \gls{JpgfDraw} application was inspired by \gls{Acorn Computers}['s] Draw
...
\glslink{DrawFile}{Acorn-DrawFile-format}, any text
...
The \widget{arrow.triangle2} marker is a new style of marker that
has separate \dgls{marker.length} and \dgls{marker.width} attributes (entered into the
\widget{arrow.length} and \widget{arrow.width} boxes). This marker is not
...
The base lies on the vertex and the marker protrudes
forwards.

Note that there are other triangle markers in other categories but
...
\end{tabular}
\tablefns{\fnsymtext{1}{Sample has marker length 8bp and marker width 4bp}}
}
...
{%
  ...
  90 & \widget{arrow.cutoutbulgecap} & 90 & \fnsym{1}\widget{arrow.triangle2}
  \end{tabular}
  \tablefns{\fnsymtext{1}{Version 2.1 onwards only (has width)}}
}
