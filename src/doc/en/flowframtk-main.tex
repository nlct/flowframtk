...
\gls{clipping.intersection} function (see
\sectionref{sec:pathintersect}) or the clipping shapes function (see
...
that tag in its \gls{taglist}, then \FlowframTk\ will write
\code{\gls{pgfusepath}\marg{clip}} instead.
...
(space-separated) tag list that contains this tag will be written to
...
No clipping is applied on the \gls{canvas} (\figureref{fig:cliptaga})
but clipping is applied when the image is exported to a file
where it is rendered with \env{pgfpicture}. The export had the
\widget{export.markup.encap} setting on.

In \figureref{fig:cliptagb}, no adjustment was made to the encap
...
outside of any clipping. (It's rendered after the end of the scope
...
into account. If you want to crop the image, the best solution is to
create a rectangle at the \gls{back} and both set the tag and set
the \gls{image.typeblock} to the rectangle's \gls{bbox} (see
\sectionref{sec:typeblock}).  Make sure to then select the
...
clutter in the \dialog{export} dialog, the export to \gls{eps} is no
...
To restore all the original mappings, quit \FlowframTk\ and then delete
...
The \meta{Java class name} is the Java class name of the \gls{object}:
...
\code{JDRText} or \code{JDRTextPath}, as applicable, not
\code{JDRPath}.
\end{information}

The \meta{description} is the image's description
(\sectionref{sec:imagedescription}) when \meta{n} is 0 otherwise
it's the \gls{object}['s] description
(\sectionref{sec:objectdescription}). This argument will be empty if
no description has been set.

The \meta{tag} is the object's tag or space-separated \gls{taglist} 
(\sectionref{sec:objecttag}).
This argument will be empty if no tag has been set.
...
\meta{Java class name} will be \code{JDRText} or \code{JDRTextPath}).
...
will be exported as though they had been converted to a path (except that the 
\meta{Java class name} will be \code{JDRTextPath}).

\widgetdef{export.textareaoutline}

If the export format doesn't support \gls{textarea} outlines, the
\widget{export.textareaoutline} panel provides options to indicate
what should be done if the image contains any \glspl{textarea} with 
the \textattr{outline} mode on. Note that this option doesn't change
the image itself but just governs the way the image is saved in the export
file.
...
If \widget{export.textareaoutline.ignore} is selected, all
\glspl{textarea} will be exported as though the \textattr{outline} mode
was off.

\widgetdef{export.textareaoutline.to_path}

If \widget{export.textareaoutline.to_path} is selected, all
\glspl{textarea} with the \textattr{outline} mode attribute set 
...
process if required. The window will usually close after the
operation has been completed but will remain open if any warnings or
...
Path \gls{clipping.intersection} can be used to clip a closed
...
represents the image's tag. This tag should be empty except for any
...
for a new unsaved image.) The image tag in the clipboard is
