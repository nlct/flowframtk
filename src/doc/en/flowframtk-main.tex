...
This can either be done with the \dialog{typeblock}, which can be
...
Once the \gls{typeblock} can be set, you can then assign
...
The \menu{tex.flowframe.wizard} opens the \dialog{flfwizard} dialog,
...
page using the \widget{flfwizard.previous} and
\widget{flfwizard.next} buttons at the bottom of the
...
title of the previous page. You can use this button to go back. The
...
title of the next page. You can use this button to go forwards. On
the final page it will change to \widget{flfwizard.finish.title}.

\widgetdef{flfwizard.cls.title}

The first page of the \dialog{flfwizard} dialog reminds you of your
current settings. The \widget{flfwizard.cls.default_cls},
\widget{flfwizard.cls.custom_cls},
\widget{flfwizard.cls.relative_fontsize},
\widget{flfwizard.cls.normalsize}, and
\widget{flfwizard.cls.update} are the same as the corresponding
settings in the \dialog{clssettings} tab of the \dialog{texconfig}
dialog (described in \sectionref{sec:configuredialog}).

The \widget{flfwizard.labelpages.absolute} and
\widget{flfwizard.labelpages.relative} settings are the same as the
corresponding \widget{flfsettings.pages_opt} settings in the
...
point.
Any changes you make in the \dialog{flfwizard.cls} panel won't be
applied until you reach the \dialog{flfwizard.frametype} panel.
...
or \keys{\keyref{control}} (add to selection or deselect if already
selected).
...
left and right margins so that the typeblock width is rounded to the
supplied dimension but remember that this depends on the current
...
calculation uses the current paper height.
...
height and then adjust the margins to fit.
\end{important}

Any changes you make in the \dialog{flfwizard.typeblock} panel won't be
applied until you click on the \btn{flfwizard.next} button and reach the 
\dialog{flfwizard.frametype} panel.
...
\gls{flowframe} data for an \gls{object}. You must first select the
applicable \gls{object} from the \widget{flfwizard.frametype.object}
list. A single mouse click will select an item from the list. A
double-click will both select an item from the list and also
select the corresponding \gls{object} on the \gls{canvas}, which
allows you to check that you have selected the correct \gls{object}.

If you already had one or more \glspl{object} selected, the list
item that corresponds to the \glsdisp{backobject}{backmost} of the
...
\btn{flfwizard.next} button to continue to the next page.

\widgetdef{flfwizard.labelpages.title}

The \dialog{flfwizard.labelpages} panel confirms the selected frame
type in the \widget{flfwizard.labelpages.frame_type} field. This is
non-editable. You will have to go back to the previous page to
...
Otherwise, you will need to fill it in. The label must be unique
and should follow the usual character rule for labels in general, so
avoid \TeX\ special characters.

\begin{important}
Unlike the \sty{flowfram} package which only requires that the label
be unique for the given frame type, \FlowframTk\ requires the label
be unique across all types.
\end{important}
...
sequence name (such as \qtt{textbf} or \qtt{bfseries}) for the
the style attribute.
\FlowframTk\ allows either a control sequence name or one or more
declarations. These can be entered into the \widget{flfwizard.style.cmds}
field (which will be trimmed).
...
left of the flow frame, \widget{flowframe.marginpar.outer} indicates to the
right of the flow frame for even pages and to the left of the flow
frame for odd pages, \widget{flowframe.marginpar.inner} indicates to
...
\widget{flfwizard.style.shape}. If the panel is hidden, all the
...
down the \keys{\keyref{control}} key while clicking on an item, it
