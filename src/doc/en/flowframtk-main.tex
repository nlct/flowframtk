...
\newcommand{\texttool}{\gls{menu.tools.textarea} tool}
\newcommand{\mathstool}{\gls{menu.tools.math} tool}
...
except the select tool. The tools can be selected using either the
...
\hyperref[sec:editpath]{edit path function} to convert
...
\menu{navigate.goto} which will display the dialog box shown in
\figureref{fig:goto}. Enter the $x$ and $y$ co-ordinates in the
\widget{coordinates.x} and \widget{coordinates.y} fields if you are
using a rectangular grid, or enter the angle and radius in the
\widget{coordinates.angle} and \widget{coordinates.radius} fields,
...
with the \hyperref[sec:editpath]{edit path function}.
...
\glspl*{controlpt} using the
\hyperref[sec:editpath]{edit path function}.
...
to create a circle, I recommend that you first
\hyperref[mi:gridlock]{lock} the grid or use the
...
onto the \gls{canvas} in select mode, the text mappings (not
...
\gls{index.menu.selectedtext}, illustrated in
...
the \gls{menu.tools.select} tool (and no shapes are
...
\gls{bitmap} file onto the \gls{canvas} (in \gls{menu.tools.select} mode) and it will
...
\LaTeX\ path, or transformation matrix), \hyperref[sec:selectobjects]{select}
...
In order to edit an \gls{object}, you must be in select mode. To do
...
\item Use Control-click (i.e.\ \gls{click} whilst holding down the control
...
\item Use \menu{edit.select_all} to select all
\glspl{object}.
\end{itemize}

When an \gls{object} is selected, a dashed red\slash\manmsg{grey} rectangle will
be displayed around it. (This rectangle may optionally have
\hyperref[mi:hotspots]{hotspot} regions.) Note that
individual elements of a \gls{group} can not be selected
independently of the group.  When you select a group, you will only
see a dashed red\slash\manmsg{grey} rectangle around the \gls{bbox} of the group,
not around the individual elements of the group.

To deselect an individual \gls*{object}, click on that object whilst
depressing the shift key. To deselect all \glspl*{object}, click on
an empty part of the \gls{canvas}, or use \menu{edit.deselect_all}.
Selecting another tool will also deselect all \glspl*{object}.

In select mode you can \gls{drag-and-drop} \gls{JDR} or \gls{AJR}
...
If text is dropped onto the \gls{canvas} in select mode, the text
...
in select mode, the file path (or URI) will be added as a new \gls{textarea}.

\section{Selection Popup Menus}
\label{sec:selectpopups}

In select mode, you can also \gls{menuclick} to activate the
...
is the same as using the \menu{bitmap.insert} menu item.
...
If a path is selected, the following options will be available
(\figureref{fig:selectpopups.path}).

\menudef{menu.selectedpath.path_description}

To assign a description to the path, use
\menu{selectedpath.path_description}. This is the same as using the
...
the \menu{edit.edit.path.edit} menu item from the main menu bar.
This option won't be available if there is more than one
path selected.
...
To change the path's line \manmsg{colour}, use
\menu{selectedpath.line_colour}. This is the same as using the
\menu{edit.line_colour} menu item from the main menu bar.

\menudef{menu.selectedpath.fill_colour}

To change the path's fill \manmsg{colour}, use
...
\menudef{menu.selectedpath.pattern}

To change the line pattern (dash or solid) for the selected paths,
use \menu{selectedpath.pattern}. This is the same as using the
\menu{edit.path.style.pattern} menu item from the main menu bar.
...
If a \gls{textarea} is selected, the following options will be available
(\figureref{fig:selectpopups.text}).

\menudef{menu.selectedtext.textarea_description}

To assign a description to the \gls{textarea}, use
\menu{selectedtext.textarea_description}. This is the same as using the
\menu{edit.object_description} menu item from the main menu bar.
This option won't be available if there is more than one
...
To change the \gls{textarea}['s] pen \manmsg{colour}, use
...
To outline the \gls{textarea}, use \menu{selectedtext.outline}.
This is the same as using the \menu{edit.textarea.outline} menu item
from the main menu bar.

\menudef{menu.selectedtext.fill_colour}

To change the \gls{textarea}['s] fill \manmsg{colour}, use
...
To change just the anchor (horizontal, vertical or both) for the
...
\FloatFig
  ...
  {Popup Menu when only a Text-Path is Selected},

If a text-path is selected, the following option will be available
(\figureref{fig:selectpopups.textpath}).

\menudef{menu.selectedtextpath.textpath_description}

To assign a description to the text-path, use
\menu{selectedtextpath.textpath_description}. This is the same as using the
\menu{edit.object_description} menu item from the main menu bar.
...
If a \gls{bitmap} is selected, the following options will be available
(\figureref{fig:selectpopups.bitmap}).

\menudef{menu.selectedbitmap.bitmap_description}

To assign a description to the \gls{bitmap}, use
\menu{selectedbitmap.bitmap_description}. This is the same as using the
\menu{edit.object_description} menu item from the main menu bar.
This option won't be available if there is more than one
\gls{bitmap} selected.

\menudef{menu.selectedtext.properties}
...
\FloatSubFigs{fig:selectpopups}
{%
  {fig:selectpopups.bitmap}{\includeimg{selectPopupBitmap}}{only \glspl{bitmap}},
  {fig:selectpopups.other}{\includeimg{selectPopupOther}}{groups or mixture}
}
{Popup Menus in Select Mode Depend on What Objects Have Been Selected}
...
\inlineglsdef{goto.title} dialog in
which you can enter $x$ and $y$ coordinates. If your system allows
applications to move the cursor, the cursor will be moved to the
entered co-ordinates (scrolling the \gls{canvas} if necessary).

\menudef{menu.navigate.select}

The \menu{navigate.select} menu item 
selects the next object in the \gls{stack}. (Starting from the
...
selected object closest to the \gls{back}, and select
the next object in the \gls{stack} (heading towards the \gls{back}). This
...
the next object in the \gls{stack} to the selection (starting from
...
cursor, the cursor will be moved to the top left corner of the
...
\section{\Glsfmttext{manual.colour} Space Conversions}\label{sec:convertcolspace}
...
\section{Line \Glsfmttext{manual.colour}}\label{sec:linepaint}

TODO


\section{Fill \Glsfmttext{manual.colour}}\label{sec:fillpaint}
...
\subsubsection{Marker \Glsfmttext{manual.colour}}\label{sec:markerpaint}
...
\section{Text \Glsfmttext{manual.colour}}\label{sec:textpaint}
