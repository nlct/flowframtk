...
\begin{itemize}
\item \xmlelem{svg} (the main container);
...
\item \xmlelem{text} (may produce a single \gls{textarea} or a
\gls{group} of \glspl{textarea} depending on the attributes and
content);
...
\item \xmlelem{a} (restricted to within \xmlelem{text});
\item \xmlelem{textPath} (offset and spacing ignored);
\item shapes: \xmlelem{rect}, \xmlelem{ellipse},
\xmlelem{circle}, \xmlelem{line}, \xmlelem{polyline},
\xmlelem{polygon}, \xmlelem{path};
\item \xmlelem{linearGradient}, \xmlelem{radialGradient} \xmlelem{stop}
(limited as \FlowframTk's \dgls{gradient-paint} only supports two
\manmsgpl{colour} with fixed starting points or directions);
\item \xmlelem{marker} (very limited, markers may be omitted or
replaced with a circle, box, triangle or cross);
\item \xmlelem{style} (only limited CSS support);
\item \xmlelem{defs}, \xmlelem{use} (referenced element must be
defined within the file);
\item \xmlelem{g} (will add a \gls{group} if its content causes more
than one \FlowframTk\ \gls{object} to be created).
\end{itemize}

If the \xmlelem{svg} area (given by the \xmlattr{viewBox}) is larger than the current
paper size, the paper size will be changed to the next predefined
size that encompasses the given area.
