...
\FloatSubFigs{fig:scaledialog}
{
  {fig:scaledialoga}{\includeimg{scaledialoga}}{},
  {fig:scaledialogb}{\includeimg{scaledialogb}}{}
}
[Scale Selection Dialog Box]
{Scale Selection Dialog Box:
\subfigref{fig:scaledialoga} no typeblock;
\subfigref{fig:scaledialogb} typeblock set}
...
factor is the same for both dimensions otherwise the number field
next to the \widget{scale.keep_aspect} check box will become
available and you can set that to
the required vertical scaling. The other value (next to the
...
line style, such as line width and markers. For example, if the shape is a rectangle with
co-ordinates at (100\gls{bp},100\gls{bp}), (200\gls{bp},100\gls{bp})
(200\gls{bp},200\gls{bp}) and (100\gls{bp},200\gls{bp}) then the
width and height are both 100\gls{bp}.

However, if the selected object is a group containing one or more
...
rectangle bounds by 0.5\gls{bp} in each direction, which adds
1\gls{bp} to the width and 1\gls{bp} to the height. So even if all
the other objects within the group are within the rectangle bounds, 
the group's width and height will be slightly larger than the actual
rectangle co-ordinates which will change the calculations.

The reason for this difference is that the feature is mainly
...
\figureref{fig:scaledialoga}). Otherwise, a \gls{dropdown} will be
available where you can choose either the paper width or
\gls{typeblock} width (see \figureref{fig:scaledialogb}).
...
to calculate the $x$ scale factor. For example, if the selected
object's width is 100\gls{bp} and the \gls{typeblock} width is
400\gls{bp} then the $x$ scale factor required to make the object's
width 50\% of the \gls{typeblock} width is 2.

\begin{information}
The \widget{scale.calc_x} button doesn't automatically apply the
scaling to the selection on the \gls{canvas}. It simply updates the
applicable scaling field at the top of the dialog.
\end{information}
...
\gls{typeblock}, enter the percentage value (an integer) into the
\widget{scale.height} field. If no \gls{typeblock} has been set
the percentage can only be of the paper height (see
\figureref{fig:scaledialoga}). Otherwise, a \gls{dropdown} will be
available where you can choose either the paper height or
\gls{typeblock} height (see \figureref{fig:scaledialogb}).

\widgetdef{scale.calc_y}
Once you have entered the required percentage value in the
\widget{scale.height} field, click on the \btn{scale.calc_y}
to calculate the $y$ scale factor. For example, if the selected
object's height is 100\gls{bp} and the \gls{typeblock} height is
400\gls{bp} then the $x$ scale factor required to make the object's
height 50\% of the \gls{typeblock} height is 2.

\begin{information}
The \widget{scale.calc_y} button doesn't automatically apply the
scaling to the selection on the \gls{canvas}. It simply updates the
applicable scaling field at the top of the dialog.
\end{information}

If the \widget{scale.y} button is selected, the calculated value
will be placed in the \widget{scale.y} field. If the
\widget{scale.both} button is selected, the calculated value will be
placed in the field next to the \widget{scale.keep_aspect} checkbox.
If that checkbox is selected, the value will also be placed in the
field next to the \widget{scale.both} button.
