...
Each type of frame has its own layer (see \figureref{fig:flowframe-layers}).
...
lengths into the supplied fields or, if you have one or more
\glspl{object} selected, you can get \FlowframTk\ to calculate the
margins for a \gls{typeblock} that fits the bounding rectangle that
encompasses all the selected \glspl{object} by clicking on the
\widget{typeblock.compute_from_objects} button.  (This button is
disabled if no \gls*{object} is selected.)
