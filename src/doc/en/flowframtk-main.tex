...
class and preamble.  The generated \ext{tex} file has the form:
\begin{compactcodebox}
...
is selected in the \dialog{texconfig.title} dialog.
...
the command or the \glsname{preamble.mid} or \gls{preamble.latex} can include the command
definition.

\menudef{menu.tex.set_preamble}

The \glsname{preamble.early}, \glsname{preamble.mid} and
\gls{preamble.latex} code can be edited in the
\inlineglsdef{preamble} panel attached to the side of the
...
\inlineglstextdef{preamble.mid}, which may also be used to define
commands for use in any \gls{textarea}['s] alternative \LaTeX\ text.
...
\menudef{menu.texeditor.selectAll}

The \menu{texeditor.selectAll} function selects all the text in
...
The \dialog{find} dialog has a text field where you can enter you
search term. 
...
The \widget{find.find} button will find the next occurrence of the
search term.
