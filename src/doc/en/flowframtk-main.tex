...
early-preamble can include the code to load the package that defines
the command or the mid- or late-preamble can include the command
definition.  If the \glspl{textarea-mapping} are enabled (see
\sectionref{sec:texconfigtext}), any
required packages for mapped symbols will automatically be
added to the early \gls{preamble}.

\menudef{menu.tex.set_preamble}

The early-, mid- and late-\gls{preamble} code can be edited in the
...
The \gls{preamble} panel has three tabbed panes, for the
early-, mid- and late-\gls{preamble} code. Each pane has the same layout
with a horizontal toolbar and an editor area below it in which you
can type the code. The buttons on the toolbar have the following functions:
