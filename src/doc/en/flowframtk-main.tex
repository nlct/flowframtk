...
added to the \code{/Title} attribute. The only other attribute
that is set is the \code{/CreationDate} attribute. This setting is
...
of the \widget{texconfig.title} dialog also
uses these settings.
...
\begin{itemize}
\item the list of \hyperref[sec:recent]{recent files}
...
the \hyperref[sec:export]{export} functions
...
In \hyperref[sec:edit]{select mode} (but not when you're editing or
distorting a shape) you can \gls{drag-and-drop} image files or
text onto the canvas to add to the current image. In
\hyperref[sec:text]{text or \manmsg{maths} mode} you can only drag
...
using the \widget{borders.title} tab in the \widget{configui.title}
...
\gls{doubleclick} on this area to open the
\widget{goto.title} dialog.
...
\gls{doubleclick} on this area to toggle the grid lock setting.
...
bar, using the \widget{configui.title} dialog. You can also use this
