% arara: lualatex
% arara: bib2gls if missing("glstex")
% arara: lualatex
% arara: bib2gls
% arara: lualatex
% arara: lualatex
...
\convswitchdef{bounds}

For output formats that support setting the image bounds,
this option indicates whether to use the paper size (not applicable
when \convswitch{export-latex} isn't used with \convswitch{doc}),
the image bounding box or (if set) the
\gls{typeblock}. Allowed values are: \optfmt{paper}, \optfmt{image}
and \optfmt{typeblock}.

\convswitchdef{crop}

Equivalent to \code{\convswitch{bounds} image}.

\convswitchdef{no-crop}

This option is deprecated. Use \code{\convswitch{bounds} paper} instead.

\convswitchdef{use-typeblock}

Equivalent to \code{\convswitch{bounds} typeblock}.

\convswitchdef{no-use-typeblock}

This option is deprecated. Use \code{\convswitch{bounds} image} instead.

\convswitchdef{use-flowframtksty}

Assume that the \sty{flowframtkutils} package is installed and use that
...
\begin{information}
The \sty{flowframtkutils} package is a supplementary package provided
with the \sty{flowfram} package (v1.18+) specifically for use
with \FlowframTk. If you have an older version of \sty{flowfram}
installed, the \sty{flowframtkutils} package won't be available.
\end{information}

\convswitchdef{no-use-flowframtksty}

The \sty{flowframtkutils} package isn't installed 
