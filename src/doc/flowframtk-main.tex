...
The \menu{menu.settings.grid} submenu allows you to change
the \gls{grid} settings:

\menudef{menu.settings.grid.show}

Toggles between displaying the \gls{grid} on the \gls{canvas} and  
...
Toggles between locking and unlocking the grid. If the grid is
locked, mouse \glspl{click} will be translated to the nearest tick mark.
This means that if you use a mouse \gls{click} to set the location of
a \gls{controlpt} when constructing a \gls{path}, the
point will be placed at the nearest tick mark. This also means that
when you move a point while in 
edit mode (\sectionref{sec:editpath}), the point will be moved
...
\menutrail{navigate.goto} or emulate a \gls{primaryclick}.
